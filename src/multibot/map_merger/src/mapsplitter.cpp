...
nav_msgs::OccupancyGrid *getMapPart(nav_msgs::OccupancyGrid *tmp, int start_x,
...
void callback(const nav_msgs::OccupancyGrid::ConstPtr &msg)
{
    ...
    ros::Publisher pub_map =
        n.advertise<nav_msgs::OccupancyGrid>(pub_topic_map, 3);
    ros::Publisher pub_meta =
        n.advertise<nav_msgs::OccupancyGrid>(pub_topic_meta, 3);
    // get the message
    ...
    {
        for (int collum = 0; collum < local_map.info.width - size;
             collum += size)
        {
            // ROS_INFO("row:%icollum:%i:",row,collum);
            nav_msgs::OccupancyGrid *t =
                getMapPart(&local_map, row, collum, size, size);
            int sum_elements =
            ...
            ROS_INFO("Published Mapframe:%s||r:%i;c:%i", pub_frame.c_str(), row,
                     collum);
                                    ...
                                    int start_y, int width, int height)
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        {
            part->data.push_back(tmp->data.at(
