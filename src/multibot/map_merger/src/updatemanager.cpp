...
{
    ROS_DEBUG("in addToupdateList,indexOfMap:%i,size of values:%zu", indexOfMap,
              values.size());
    std::vector<int> *tmp = updateInformation->at(indexOfMap);
    ROS_DEBUG("in addToupdateList size tmp:%zu", tmp->size());
    bool add = true;
    if (values.size() == 1 && tmp->size() > 1)
    {
        ...
        if (i == j)
        {
            ...
            ROS_DEBUG("adding revision number %i, to list at index:%i",
                      values.at(0), indexOfMap);
            add = false;
        }
    }
    ...
    if (tmp->size() > 1)
    {
        // if the latest value is smaller then the value before, i need to sort
        // my list
...
{
    // ROS_DEBUG("Returning list of updates, size of
    // list:%lu",updateInformation->at(indexOfMap)->size());
    return updateInformation->at(indexOfMap);
}
int updateManager::getLatestUpdateVersionOfRobot(int indexOfMap)
{
    return getUpdateListOfrobot(indexOfMap)
        ->at(getUpdateListOfrobot(indexOfMap)->size() - 1);
}
...
{
    if (getUpdateListOfrobot(indexOfMap)->size() ==
        getLatestUpdateVersionOfRobot(indexOfMap) + 1)
        return false;
    else
        return true;
}
