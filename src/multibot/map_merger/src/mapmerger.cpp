...
MapMerger::MapMerger()
{
    ...
    nodeHandle->param<int>("seconds_recompute_transform",
                           seconds_recompute_transform, 5);
    ...
    nodeHandle->param<std::string>("static_mc_group_name", mc_group,
                                   "mc_default");
    nodeHandle->param<std::string>("local_map_topic", local_map_topic, "/map");
    nodeHandle->param<std::string>("local_map_metadata_topic",
                                   local_map_metadata_topic, "/map_metadata");
    nodeHandle->param<std::string>("local_map_frame_id", local_map_frame_id,
                                   "map");
    // nodeHandle->param<std::string>("meta_topic_over_network",map_meta_topic,"/map_meta");
    nodeHandle->param<std::string>("map_topic_over_network", topic_over_network,
                                   "map_other");
    nodeHandle->param<std::string>("position_local_robot_topic",
                                   position_local_robot_topic, "slam_out_pose");
    nodeHandle->param<std::string>("position_other_robots_topic",
                                   position_other_robots_topic,
                                   "position_other_robots");
    ...
    ROS_INFO("Using %s as multicast group", mc_group.c_str());
...
{
    // ros::NodeHandle nodeHandle;
    ROS_INFO("Wait for map_meta_data, on topic:[%s]",
             local_map_metadata_topic.c_str());
    ros::Subscriber sub =
        nodeHandle->subscribe(local_map_metadata_topic, 1000,
                              &MapMerger::callback_map_meta_data_local, this);
    while (ros::ok())
    {
        ros::spinOnce();
...
void MapMerger::waitForRobotInformation()
{
    ...
    ros::Subscriber sub =
        nodeHandle->subscribe(robot_prefix + "/adhoc_communication/new_robot",
                              1000, &MapMerger::callback_got_robot_for_data,

                              this);

    ros::ServiceClient getNeighborsClient =
        nodeHandle->serviceClient<adhoc_communication::GetNeighbors>(
    ...
    {
        ROS_INFO(
            "%zu robots are already in the network, adding them,my name:%s",
            getNeighbors.response.neigbors.size(), robot_name.c_str());
...
void MapMerger::callback_new_robot(const std_msgs::StringConstPtr &msg)
{
    ...
    for (int i = 0; i < robots->size(); i++)
    {
        if (robots->at(i) == newRobotName)
        {
            ...
            sendMapOverNetwork(newRobotName, lUpdate, lastUpdate->getMinX(),
                               lastUpdate->getMaxX(), lastUpdate->getMinY(),
                               lastUpdate->getMaxX());
            delete lUpdate;
            return;
        }
    ...
    makeEmptyMapData(newRobotName, map_height, map_width,
...
void MapMerger::callback_control(
    const adhoc_communication::MmControlConstPtr &msg)
{
    ...
    if (tmp.update_numbers.size() == 0)
    {
        ...
        ROS_DEBUG(
            "Send map after control minx:%i \t maxx:%i \t miny:%i \t maxy:%i",
        ...
        sendMapOverNetwork(tmp.src_robot, containedUpdates, min_x, min_y, max_x,
                           max_y);
        // sendMapOverNetwork(tmp.src_robot,containedUpdates);
        ROS_INFO("Sended map after beeing asked to %s,containd %zu "
                 "updates,update_seq:%i",
                 tmp.src_robot.c_str(), containedUpdates->size(), update_seq);
    ...
    else
    {
        ...
        ROS_INFO("Sending %zu updates back to "
                 "%s\n\t\tminX:%i\tminY:%i\tmaxX:%i\tmaxY:%i",
                 tmp.update_numbers.size(), tmp.src_robot.c_str(), min_x, min_y,
                 max_x, max_y);
        for (int i = 0; i < tmp.update_numbers.size(); i++)
            ROS_INFO("Send %i", tmp.update_numbers.at(i));
        sendMapOverNetwork(tmp.src_robot, &tmp.update_numbers, min_x, min_y,
                           max_x, max_y);
    }
}

void MapMerger::callback_map_meta_data_local(
...
void MapMerger::callback_global_pub(const ros::TimerEvent &e)
{
    ...
    {
        ROS_DEBUG("Merging other maps into global map in "
                  "callback_global_pub,sizeTransforms:%zu,sizeRobots:%zu",
                  transforms->size(), robots->size());
        for (int i = 0; i < transforms->size(); i++)
        {
            if (new_data_maps->at(findRobotIndex(i)) == false)
            {
                int robotIndex = findRobotIndex(i);
                ROS_DEBUG("Skipping %s,no new Data",
                          robots->at(robotIndex).c_str());
                continue;
            }
            ...
            nav_msgs::OccupancyGrid *map_to_merge =
            ...
            ROS_DEBUG("In mergeMaps MapData:%i,Transform:%i|size "
                      "MapData:%zu,size Tranform:%zu",
                      i, findTransformIndex(i), map_data->size(),
                      transforms->size());
    ...
    ROS_DEBUG("Publishing global_map, contains data of local map and %zu other "
              "maps| i am having %zu other maps",
              transforms->size(), map_data->size());
...
void MapMerger::callback_recompute_transform(const ros::TimerEvent &e)
{
    ...
    else
    {
        ...
        std::copy(local_map->data.begin(), local_map->data.end(),
                  global_map->data.begin());
        for (int i = 0; i < map_data->size(); i++)
        {
            nav_msgs::OccupancyGrid *whole_map = map_data->at(i);
            nav_msgs::OccupancyGrid *map_to_merge =
...
void MapMerger::callback_send_map(const ros::TimerEvent &e)
{
    ...
    for (int row = 0; row < local_map->info.height; row += 2)
    {
        for (int collumn = 0; collumn < local_map->info.width; collumn += 2)
        {
            index = row * local_map->info.width + collumn;
    ...
    ROS_DEBUG("Found values\n min_x:%i|max_x:%i|min_y:%i|max_y:%i", min_x,
              max_x, min_y, max_y);
    ...
    // todo:mc_ROBOT
    for (int i = 0; i < robots->size(); i++)
        sendMapOverNetwork(robots->at(i), containedUpdates, min_x, min_y, max_x,
                           max_y);

    // sendMapOverNetwork(mc_group, containedUpdates, min_x, min_y, max_x,
    // max_y);
    delete containedUpdates;
    // sendMapOverNetwork("",min_x,min_y,max_x,max_y);
    // send_map_over_network("");
    ROS_DEBUG("Sended local map over network,adding updateentry for update "
              "number:%i\n\t\t\tminx:%i\tmaxx:%i\tminy:%i\tmaxy:%i",
              local_map->header.seq, min_x, max_x, min_y, max_y);
    update_list->push_back(
        new UpdateEntry(update_seq, min_x, min_y, max_x, max_y));
    update_seq++;
    ...
    const adhoc_communication::MmRobotPosition::ConstPtr &msg)
{
    ...
    // ROS_ERROR("Got position
    // from:%s,robotssize:%i",source_host.c_str(),robots->size());
    for (int i = 0; i < robots->size(); i++)
    {
        // ROS_ERROR("i:%i",i);
        ...
        if (name == source_host)
        {
            ...
            {
                // ROS_ERROR("Comparing %s to %s,
                // size:",positions->positions.at(j).sourceRobotName.c_str(),
                //         source_host.c_str()
                //          );
                if (positions->positions.at(j).src_robot == source_host)
                {
                    ...
                    adhoc_communication::MmPoint newPosition =
                        positions->positions.at(j);
                    int index_transform = findTransformIndex(i);
                    if (transforms->size() == 0)
                    {
                        ROS_WARN("no transformation for %s, position is not "
                                 "published",
                                 name.c_str());
                        return;
                    }
                    ...
                    cv::Point org_point(
                        map_width / 2 + tmp.pose.position.x / 0.05,
                        map_height / 2 + tmp.pose.position.y / 0.05);
                    ...
                    pos_pub_other->at(j)
                        .publish<visualization_msgs::MarkerArray>(
                            pos_array_other->at(j));
                }
            }
            // ROS_ERROR("after for");
            if (addNew)
            {
                ...
                {
                    ROS_WARN("no transformation for %s, position is not "
                             "published,index:%u, size:%zu",
                             name.c_str(), index_transform, transforms->size());
                }
            ...
            /*
                        if(robots_position_publisher->size() > 0 &&
               transforms->size() > 0)
                        {
                            ...
                            cv::Mat trans = transforms->at(index_transform);
                            cv::Point org_point(map_width / 2
               +tmp.pose.position.x / 0.05,
                                                map_height / 2
               +tmp.pose.position.y / 0.05);
                            cv::Point homogeneous;
                            std::vector<cv::Point> inPts,outPts;
                            ...
                            s.width = map_width ;// 0.05;
                            cv::transform(inPts,outPts,trans);
                            tmp.pose.position.x = (outPts.at(0).x - map_width /
               2) * 0.05;
                            tmp.pose.position.y = (outPts.at(0).y - map_height /
               2) * 0.05;
                            //_position_other_robots[i] = tmp;
                            tmp.header.frame_id = local_map_frame_id;
                            tmp.pose.orientation.w -=
               180./M_PI*atan2(trans.at<double>(0,1),trans.at<double>(1,1));
                            ros::Publisher pub_rob =
               robots_position_publisher->at(i);
                            pub_rob.publish(tmp);
                            return;
                        }
                        */
    ...
    /* ros::Publisher pub_robot =
     nodeHandle->advertise<geometry_msgs::PoseStamped>(robot_prefix+"/position/"+source_host,3);
     tmp.header.frame_id = local_map_frame_id;
...
void MapMerger::callback_map_other(
    const adhoc_communication::MmMapUpdateConstPtr &msg)
{
    ...
    if (has_local_map == false)
    {
        if (src_robot == robot_name)
        {
            ...
            if (local_map->data.size() == 0)
            {
                ...
                local_map->data.resize(local_map->info.height *
                                       local_map->info.width);
                // here recalc Indexes!!!
                ...
                ROS_INFO("Local map is %ix%i", toInsert->info.height,
                         toInsert->info.width);
            }
            else
            {
    ...
    if (index_robots == -1)
    {
        ROS_WARN("Got map with a not valid frame_id:%s| index_robot == -1| "
                 "size of data:%zu",
                 toInsert->header.frame_id.c_str(), toInsert->data.size());
        return;
    }
    else
    {
        ...
        ROS_DEBUG("GOT NOT LOCAL MAP,Process it, data size:%zu",
                  toInsert->data.size());
        ...
        {
            ROS_DEBUG("Missed some updates for %s",
                      robots->at(index_robots).c_str());
            std::vector<int> *listOfMissedUpdates =
                updateMan->getMissingUpdateOfRobot(index_robots);
            ...
            ROS_DEBUG("%zu updates are missing,asking for them",
                      listOfMissedUpdates->size());
...
void MapMerger::callback_map(const nav_msgs::OccupancyGrid::ConstPtr &msg)
{
    ...
    int index_robots = -1;
    // ROS_INFO("Got map, frame_id:%s|size of
    // data:%i",toInsert->header.frame_id.c_str(),toInsert->data.size());
...
void MapMerger::processLocalMap(nav_msgs::OccupancyGrid *toInsert, int index)
{
    ...
    if (global_map == NULL)
    {
        ...
        std::copy(toInsert->data.begin(), toInsert->data.end(),
                  local_map_old->data.begin());
...
void MapMerger::processPosition(geometry_msgs::PoseStamped *pose)
{
    ...
    exchange.request.dst_robot = mc_group;
...
void MapMerger::start()
{
    ...
    ros::Subscriber sub = nodeHandle->subscribe(local_map_topic, 1000,
                                                &MapMerger::callback_map, this);
    ros::Duration(0.1).sleep();
    ros::spinOnce();

    ros::Subscriber sub_control =
        nodeHandle->subscribe(robot_prefix + "/" + control_topic, 1000,
                              &MapMerger::callback_control, this);
    ros::Duration(0.1).sleep();
    ros::spinOnce();

    ROS_INFO("Init Sub_other_maps");
    ros::Subscriber sub_other_maps =
        nodeHandle->subscribe(robot_prefix + "/" + topic_over_network, 1000,
                              &MapMerger::callback_map_other, this);
    ros::Duration(0.1).sleep();
    ros::spinOnce();
    //#ifndef DEBUG

    ros::Subscriber sub_new_robot =
        nodeHandle->subscribe(robot_prefix + "/adhoc_communication/new_robot",
                              1000, &MapMerger::callback_new_robot, this);
    ros::Duration(0.1).sleep();
    ros::spinOnce();
    //#else
    //  ros::Subscriber sub_new_robot =
    //  nodeHandle->subscribe("/robot_1/new_robot",1000,&MapMerger::callback_new_robot,this);
    //#endif
    ros::Subscriber sub_position_local, sub_position_network;
    if (exchange_position)
    {
        ROS_INFO("Creating position stuff");
        sub_position_local = nodeHandle->subscribe(
            robot_prefix + "/" + position_local_robot_topic, 1000,
            &MapMerger::callback_got_position, this);
        sub_position_network = nodeHandle->subscribe(
            robot_prefix + "/" + position_other_robots_topic, 5,
            &MapMerger::callback_got_position_network, this);
        if (has_local_map)
            send_position = nodeHandle->createTimer(
                ros::Duration(seconds_send_position),
                &MapMerger::callback_send_position, this);
        list_of_positions_publisher =
            nodeHandle->advertise<adhoc_communication::MmListOfPoints>(
                robot_prefix + "/" + "all_positions", 3);
    }

    ROS_INFO("Init Subscriber");
    pub = nodeHandle->advertise<nav_msgs::OccupancyGrid>("global_map", 3);
    my_pos_pub = nodeHandle->advertise<visualization_msgs::MarkerArray>(
        "position_" + robot_name, 3);
    // because i hat to send them in a occupancy grid, i send them as int, so i
    // lose the numbers
    // after the comma
    // ros::Publisher pub_position =
    // nodeHandle->advertise<geometry_msgs::Pose>("position_over_network",3);

    if (seconds_recompute_transform != -1)
    {
        ROS_INFO("Create timer to recompute the transformations all %i seconds",
                 seconds_recompute_transform);
        recompute_transform_timer = nodeHandle->createTimer(
            ros::Duration(seconds_recompute_transform),
            &MapMerger::callback_recompute_transform, this);
    }
    global_timer_pub =
        nodeHandle->createTimer(ros::Duration(seconds_publish_timer),
                                &MapMerger::callback_global_pub, this);
    send_map = nodeHandle->createTimer(ros::Duration(seconds_send_timer),
                                       &MapMerger::callback_send_map, this);
    ros::ServiceServer transform_srv = nodeHandle->advertiseService(
        "transformPoint", &MapMerger::transformPointSRV, this);
    ros::ServiceServer log_output_srv = nodeHandle->advertiseService(
        "logOutput", &MapMerger::log_output_srv, this);
    //    ros::spin();
    ...
    ask_other_timer = nodeHandle->createTimer(
        ros::Duration(5), &MapMerger::callback_ask_other_robots, this);

    ros::waitForShutdown();
}

void MapMerger::mergeMaps(nav_msgs::OccupancyGrid *mapToMerge, int min_x,
                          int min_y, int max_x, int max_y)
{
    ...
    int start_x = min_x;
    int start_y = min_y;
    // loop to merge the maps, all of the maps are overlapping becouse they got
    // transformed in the
    // calcOffsets Method. if the cur_map_to_merge knows something the local map
    // do not know -> i
    // set the value
...
cv::Mat MapMerger::mapToMat(const nav_msgs::OccupancyGrid *map)
{
    ...
    {
        ROS_ERROR("Data size doesn't match width*height: width = %d, height = "
                  "%d, data size = %zu",
...
nav_msgs::OccupancyGrid *MapMerger::matToMap(const Mat mat,
                                             nav_msgs::OccupancyGrid *forInfo)
{
    ...
    for (size_t i = 2; i < toReturn->info.height * toReturn->info.width; i++)
    {
        ...
        // here it is <10 and not 0 (like in mapToMat), becouse otherwise we
        // loose much
        // walls etc. but so we get a little of wrong information in the unkown
        // area, what is
        // not so terrible.
                          ...
                          int index_of_transform)
{
    ...
    {
        ROS_WARN("No transformation available for:[%s]",
                 mapToUpdate->header.frame_id.c_str());
        return;
    }
    // convert the map to a mat, then transform the image, and then convert the
    // mat to a map
    // and merge them
...
nav_msgs::OccupancyGrid *MapMerger::getMapPart(int map_index, int start_x,
                                               int start_y, int width,
                                               int height)
{
    ROS_DEBUG("Start in getMapPart, creating map part, "
              "min_x:%i|max_x:%i|min_y:%i|max_y:%i|Index:%i",
              start_x, start_x + height, start_y, start_y + width, map_index);
    ...
    if (sizePart > tmp->data.size())
    {
        ...
        ROS_ERROR("TmpWidth:%i\tTmpHeight:%i\tstartx:%i\tstarty:%i",
                  tmp->info.width, tmp->info.height, start_x, start_y);
        ROS_ERROR("Part is too large \n\t\t\t\tsizePart:%i\ttmp "
                  "size:%zu\twidth:%i\theight:%i",
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        {
            part->data.push_back(tmp->data[(row + start_x) * tmp->info.width +
                                           (collum + start_y)]);
        }
    }
...
void MapMerger::updateMapArea(int map_index, nav_msgs::OccupancyGrid *newData,
                              bool clear)
{
    ...
    ROS_DEBUG("Updating map Area, new data from map:%s|x:%i|y:%i",
              newData->header.frame_id.c_str(), start_x, start_y);
    ...
    for (int row = 0; row < height; row++)
    {
        for (int collum = 0; collum < width; collum++)
        {
            ...
            int i_global =
                (row + start_x) * tmp->info.width + (collum + start_y);
...
{
    int sum_elements =
        std::accumulate(local_map->data.begin(), local_map->data.end(), 0);
    if (local_map->data.size() == 0 ||
    ...
    if (transIndex == -1)
    {
        StitchedMap mapStiched(img_local, img_other, max_trans_robot,
                               max_rotation_robot, 5, cv::Mat());
    ...
    {
        StitchedMap mapStiched(img_local, img_other, max_trans_robot,
                               max_rotation_robot, 5,
                               transforms->at(transIndex));
        worked = mapStiched.works;
        if (worked)
        {
            newTrans = mapStiched.H;
    ...
    // calculates the transformation between the local map and all the other
    // maps, and
    // then updates the map data.
    if (worked)
    {
        ROS_DEBUG("Storing new Transform for %s in array",
...
void MapMerger::computeTransform(int mapDataIndex)
{
    ...
    sum_elements =
        std::accumulate(whole_map->data.begin(), whole_map->data.end(), 0);
...
void MapMerger::makeEmptyMapData(string robot_name, int height, int width,
                                 float resolution)
{
    ...
    if (exchange_position)
    {
        ...
        pos_pub_other->push_back(
            nodeHandle->advertise<visualization_msgs::MarkerArray>(
                "position_" + robot_name, 3));
    }
}

void MapMerger::callback_robot_status(
    const nav_msgs::MapMetaData::ConstPtr &msg)
{
    for (int i = 0; i < robots->size(); i++)
    {
...
void MapMerger::sendMapOverNetwork(string destination,
                                   ...
                                   int start_row, int start_collumn,
                                   int end_row, int end_collumn)
{
    // ros::nodeHandle nodeHandle ("~");
    if (broadcast_map == true)
    {
        destination = "";
    }
    ros::ServiceClient client;
    ...
    if (end_row == -1 || end_collumn == -1)
    {
        ROS_DEBUG("Send whole map");
        end_row = map_height;
        end_collumn = map_width;
        start_row = 0;
        start_collumn = 0;
        isWholeMap = true;
    }
    ROS_DEBUG("Send map using:%s \n\t\t\t\t the normal way "
              "min_x:%i|max_x:%i|min_y:%i|max_y:%i",
              service.c_str(), start_row, end_row, start_collumn, end_collumn);
    /**
      todo:
      values for the mapPart are used wrong, always creates one great map part
      if i just send 688 x 688 packets,
      fix the problem
    */
    int height, width;
    height = size;
    ...
    {
        for (int collumn = start_collumn; collumn < end_collumn;
             collumn += size)
        {
            if (row + height > end_row)
                height = end_row - row;
            if (collumn + width > end_collumn)
                width = end_collumn - collumn;
            ROS_DEBUG("Creating map part t");
            nav_msgs::OccupancyGrid *t =
                this->getMapPart(-1, row, collumn, width, height);
            ROS_DEBUG("Checking data of  map part t,size:%zu", t->data.size());

            /*int sum_elements =
            std::accumulate(t->data.begin(),t->data.end(),0);
            ...
            */
            ...
            exchange.request.map_update.map.header.frame_id =
                robot_name.c_str();
            ...
            ROS_DEBUG("Send Map to %s(if nothing -> broadcast), frame id:%s| "
                      "topic:[%s]|data<_size:%zu",
                      destination.c_str(), t->header.frame_id.c_str(),
                      topic_over_network.c_str(),
                      exchange.request.map_update.map.data.size());
            if (client.call(exchange))
            {
                if (exchange.response.status)
                {
                    if (destination == "")
                    {
                        ROS_DEBUG("Sended Map to: all,topic:%s|r:%i;c:%i",
                                  exchange.request.topic.c_str(), row, collumn);
                    }
                    else
                    {
                        ROS_DEBUG("Sended Map to: %s,topic:%s|r:%i;c:%i",
                                  exchange.request.dst_robot.c_str(),
                                  exchange.request.topic.c_str(), row, collumn);
                    }
                }
                else
                {
                    ROS_WARN("Destination host unreachable [%s](if nothing -> "
                             "broadcast)",
                             hostname.c_str());
                }
            }
...
void MapMerger::sendMetaData(float res)
{
    ...
    client = nodeHandle->serviceClient<adhoc_communication::SendOccupancyGrid>(
        service);
...
int MapMerger::findTransformIndex(int robot_index)
{
    ...
    // ROS_DEBUG("Could not found TransformIndex for
    // robot_index:%i",robot_index);
    return -1;
}
    ...
    if (!boost::filesystem::exists(boost_log_path))
        try
        {
            if (!boost::filesystem::create_directories(boost_log_path))
                ROS_ERROR("Cannot create directory \"%s\".",
                          full_log_path.c_str());
        }
 ...
 * Saves log information to the hard drive. At this point log information may
 * include
 *  * global map
 *  * local map
 */
                               ...
                               map_merger::LogMaps::Response &res)
{
    ...
    if (req.log & LOG_GLOBAL_MAP)
    {
        ...
        else
        {
            ...
            circle(glo, Point(glo.rows / 2, glo.cols / 2), 5, Scalar(0, 0, 0),
                   2);
            if (global_map_ready)
    ...
    if (req.log & LOG_LOCAL_MAP)
    {
        ...
        else
        {
            ...
            circle(loc, Point(loc.rows / 2, loc.cols / 2), 5, Scalar(0, 0, 0),
                   2);
    ...
    if (req.log & LOG_LOCAL_MAP_PROGRESS)
    {
        ...
        {
            // log the local map progress, i.e., the number of free spaces in
            // the local map
    ...
    if (req.log & LOG_GLOBAL_MAP_PROGRESS)
    {
        ...
        {
            // log the global map progress, i.e., the number of free spaces in
            // the local map
    ...
    if (req.log & LOG_GLOBAL_MAP_PROGRESS || req.log & LOG_LOCAL_MAP_PROGRESS)
    {
        ...
        // fs << creation_time << "," << global_progress << "," <<
        // local_progress << std::endl;
        fs << ros::Time::now() << "," << global_progress << ","
           << local_progress << "," << cur_position->pose.position.x << ","
           << cur_position->pose.position.y << "," << update_list->size() << ","
           << transforms->size() << std::endl;
                                  ...
                                  map_merger::TransformPoint::Response &res)
{
    ...
    for (int i = 0; i < robots->size(); i++)
    {
        ...
        {
            ROS_DEBUG("Found %s with %s at %i", robots->at(i).c_str(),
                      req.point.src_robot.c_str(), i);
...
{
    ros::ServiceClient client =
        nodeHandle->serviceClient<adhoc_communication::SendMmControl>(
            robot_prefix + "/adhoc_communication/send_control_message");
...
bool MapMerger::getHasLocalMap() { return has_local_map; }

void MapMerger::callback_ask_other_robots(const ros::TimerEvent &e)
{
    ROS_INFO("Checking if robots are already in the network, "
             "service:%s/adhoc_communication/get_neighbors",
             robot_prefix.c_str());
