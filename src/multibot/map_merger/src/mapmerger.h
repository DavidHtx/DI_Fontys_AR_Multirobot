...
class MapMerger
{
  ...
  private:
    ...
    void
    callback_map_other(const adhoc_communication::MmMapUpdateConstPtr &msg);
    void callback_control(const adhoc_communication::MmControlConstPtr &msg);
    void
    callback_map_meta_data_local(const nav_msgs::MapMetaData::ConstPtr &msg);
    ...
    void callback_got_position_network(
    ...
    void makeEmptyMapData(string robot_name, int height, int width,
                          float resolution);
    void mergeMaps(nav_msgs::OccupancyGrid *mapToMerge, int min_x = 0,
                   int min_y = 0, int max_x = -1, int max_y = -1);
    ...
    void updateMap(nav_msgs::OccupancyGrid *mapToUpdate,
                   int index_of_transform);
    void updateMapArea(int map_index, nav_msgs::OccupancyGrid *newData,
                       bool clear = false);
    int findTransformIndex(int robot_index);
    int findRobotIndex(int transform_index);
    void sendMapOverNetwork(string destination,
                            std::vector<int> *containedUpdates,
                            int start_row = 0, int start_collum = 0,
                            int end_row = -1, int end_collum = -1);
    void sendMetaData(float res = 0.05);
    void sendBackAskedMapData(string robotName,
                              std::vector<int> missingUpdates);
    nav_msgs::OccupancyGrid *getMapPart(int map_index, int start_x, int start_y,
                                        int width, int height);
    nav_msgs::OccupancyGrid *matToMap(const cv::Mat mat,
    ...
    int map_width, map_height, size, seconds_meta_timer, seconds_publish_timer,
        seconds_send_timer, seconds_recompute_transform, seconds_send_position,
    ...
    // Private variables
    bool debug, splitted, has_local_map, exchange_position, local_map_new_data,
        force_recompute_all, global_map_ready, broadcast_map;
    //            convergente;
    cv::Mat lastTrans;
    ros::Publisher pub;
    ...
    int update_seq;
    std::string topic_over_network, local_map_frame_id, local_map_topic,
        local_map_metadata_topic, position_local_robot_topic, map_meta_topic,
        position_other_robots_topic, robot_prefix, robot_name, mc_group,
        control_topic,
    ...
    ros::Timer send_position, recompute_transform_timer, global_timer_pub,
        send_map, ask_other_timer;
