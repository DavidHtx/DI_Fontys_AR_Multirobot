...
McPosAckObj::McPosAckObj(RoutedFrame *frame, McTree *group)
{
    ...
    if (!compareMac(group->route_uplink_->next_hop, frame->eh_h_.eh_source) &&
        !group->root)
    {
    ...
    for (std::list<mac *>::iterator i = group->downlinks_l_.begin();
         i != group->downlinks_l_.end(); i++)
    {
        mac *m = *i;
...
{
    if (ack->mc_group_.compare(group_name) == 0 &&
        ack->header_.packet_id == packet_id &&
        ack->header_.frame_seq_num == sequence_num)
    {
        for (std::list<unsigned char *>::iterator i = missing_acks_l.begin();
             i != missing_acks_l.end();)
        {
