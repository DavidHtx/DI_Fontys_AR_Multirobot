 ...
 * \brief Main file for the ad hoc communication node including the main
 *function.
 *
 * Description:
 * This node implements a dynamic source routing protocol, which allows you to
 *send data through a network of independent nodes.
 * Its important that the node always run with root rights, because the protocol
 *is implement with raw sockets and without root rights you can't open a raw
 *socket.
 *
 * Node instruction:
 *
 ...
 *
 * Every service request has a field for the hostname of the destination and a
 *field for the topic to publish the message.
 * If you want to broadcast a message you must set the destination hostname as
 *an empty string.
 * NOTE: Broadcast don't include acknowledgments and can only be send to direct
 *neighbors and if just one frame of the broadcast packet will be lost over
 *transmission, the whole packet is useless.
 *
 * The node 'communication_tester' of this package demonstrate how you can use
 *the services.
 * There is also a tutorial to adapt this node to implement a service to send
 *your own ROS messages.
 *
 * The node have a lot of parameter and defines that can be changed. There is a
 *list of all parameters and defines in the docu foldere.
 *
 *Special Notes: (not important for users)
 *
 * For debugging reasons there is an option to set the hostname and the mac of
 *the node manually.
 ...
 *
 * I want to start a node with hostname "my_hostname" and the mac
 *"84:a6:c8:43:1e:3a". The node should only be able to reach two hosts:
 * neighbor_one 10:00:00:00:00:00
 * neighbor_two 20:00:00:00:00:00
 * To reach this you must start the node like this:
 *
 * rosrun ad_hoc_comm ad_hoc_communication wlan0 my_hostname 84:a6:c8:43:1e:3a
 *neighbor_one 10:00:00:00:00:00 neighbor_two 20:00:00:00:00:00
 *
 * NOTE: If you don't use this feature you can start the node normally and all
 *reachable neighbor will be detected automatically. If you want to use this
 *feature, make sure DEBUG is defined.
 */
                    ...
                    adhoc_communication::GetGroupState::Response &res)
{
    ...
    if (t != nullptr)
    {
        ...
        for (auto it = t->downlinks_l_.begin(); it != t->downlinks_l_.end();
             ++it)
        {
            mac *m = *it;
        ...
        {
            res.route_uplink =
                "Next hop:" + getHostnameFromMac(t->route_uplink_->next_hop) +
                " RD:" + getIntAsString(t->route_uplink_->root_distance) +
                " HOBS:" + getIntAsString(t->route_uplink_->hobs) + " CH:" +
                getIntAsString(t->route_uplink_->current_hop);
        }
        else
        {
            res.route_uplink = "";
 ...
 *
 * Implements the ROS service join_mc_group to tell a node to connect to a
 * multicast group.
 */
                 ...
                 adhoc_communication::ChangeMCMembership::Response &res)
{
    ...
    if (req.action)
    {
        ...
        if (t != NULL && t->connected && t->activated)
        {
            ...
            t->member = true;
            res.status = true;
            // ROS_DEBUG("CHANGE MC STATUS IN GROUP [%s] FROM CONNECTOR TO
            // MEMBER ", req.group_name.c_str());
            return true;
        }
        else if (t != NULL && t->outgoing_request_ != NULL &&
                 getMillisecondsTime() - t->time_stamp_ <
                     INTERVAL_WAIT_FOR_MCROUTES)
        {
            /* In this case the join request has been invoked by another node
             * and is now running */
        ...
        if (t == NULL || (t != NULL && t->outgoing_request_ == NULL))
        {
            ...
            while (join_attempts < MAX_INTERN_JOIN_ATTEMPS &&
                   res.status == false)
            {
                RouteRequest *rr = new RouteRequest(hostname, req.group_name,
                                                    hop_limit_min, true);
                ...
                ROS_INFO("SEND JOIN REQUEST: HOST[%s] ID[%u]",
                         req.group_name.c_str(), rr->header_.id);
                ...
                Logging::increaseProperty("num_mc_total_bytes_sent ",
                                          network_string.length());
#endif
                sleepMS(INTERVAL_WAIT_FOR_MCROUTES);
                lock_mc_groups.lock();

                res.status = !t->routing_entries_l_.empty() ||
                             (t->activated && t->connected);
            ...
            if (res.status && t->activateBestRoute(&rreq_logging))
            {
                ...
                /* Activate route*/
                McRouteActivationFrame r_act_frame = McRouteActivationFrame(
                    t->route_uplink_->next_hop, t->group_name_,
                    t->route_uplink_->id, t->route_uplink_->hostname_source);
                string network_string =
                    r_act_frame.getFrameAsNetworkString(src_mac);

                resendUnackLinkFrame("", r_act_frame.header_.id,
                                     r_act_frame.header_.mac_destination,
                                     network_string, FRAME_TYPE_MC_ACTIVATION);

                ROS_INFO("ACTIVATE MC ROUTE: MC GROUP[%s] NEXT HOP[%s]",
                         t->group_name_.c_str(),
                         getMacAsStr(t->route_uplink_->next_hop).c_str());
                ROS_INFO("SUCCESSFULLY JOINED GROUP MC: [%s]",
            ...
            {
                ROS_WARN("COULD NOT JOIN GROUP MC: [%s]",
                         req.group_name.c_str());
            }
        }
        else
    ...
    else
    {
        ...
        if (t != NULL)
        {
            if (t->downlinks_l_.empty())
            {
                if (t->root)
                {
                    ROS_ERROR("CANNOT DISCONNECT FROM OWN GROUP: [%s]",
                              req.group_name.c_str());
                    return true;
                }
                ROS_WARN("DISCONNECT FROM MC TREE: MC GROUP[%s] NEXT HOP[%s]",
                         t->group_name_.c_str(),
                         getMacAsStr(t->route_uplink_->next_hop).c_str());

                McDisconnectFrame disc_frame(t->route_uplink_->next_hop,
                                             req.group_name);
                disc_frame.disconnect_uplink = true;

                string network_string =
                    disc_frame.getFrameAsNetworkString(src_mac);
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
                ...
                resendUnackLinkFrame("", disc_frame.header_.id,
                                     disc_frame.header_.mac_destination,
                                     network_string, FRAME_TYPE_MC_DISCONNECT);
            ...
            {
                t->member = false;
                ROS_INFO("SET MEMBERSHIP TO FALSE: GROUP[%s]",
                         t->group_name_.c_str());
            }
        }
        else
                  ...
                  adhoc_communication::GetNeighbors::Response &res)
{
    ...
    for (std::list<hostname_mac>::iterator it = neighbors_l.begin();
         it != neighbors_l.end(); ++it)
    {
 ...
 */
    ...
    /*Call the function sendPacket and with the serialized object and the frame
     * payload type as parameter*/
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_QUATERNION, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendQuaternion", time_call, getMillisecondsTime(),
                             s_msg.size(), res.status);
#endif
    return true;
}
 ...
 */
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_MAP, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendOccupancyGrid", time_call,
                             getMillisecondsTime(), s_msg.size(), res.status);
#endif
    return res.status;
}
 ...
 */
    ...
    res.status =
        sendBroadcast(req.topic, req.data, FRAME_DATA_TYPE_ANY, req.hop_limit);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("BroadcastString", time_call,
                             getMillisecondsTime(), req.data.size(),
                             res.status);
#endif
    return res.status;
}

bool sendBroadcastCMgrRobotUpdate(
    adhoc_communication::BroadcastCMgrRobotUpdate::Request &req,
 ...
 */
    ...
    res.status = sendBroadcast(req.topic, s_msg, FRAME_DATA_TYPE_ROBOT_UPDATE,
                               req.hop_limit);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("BroadcastCMgrRobotUpdate", time_call,
                             getMillisecondsTime(), s_msg.size(), res.status);
#endif
    return res.status;
}
 ...
 */
    ...
    string s_msg = getSerializedMessage(req.update);
    res.status = sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_ROBOT_UPDATE,
                            req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendCMgrRobotUpdate", time_call,
                             getMillisecondsTime(), s_msg.size(), res.status);
#endif
    return res.status;
}
 ...
 */
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_TWIST, req.topic);
#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendTwist", time_call, getMillisecondsTime(),
                             s_msg.size(), res.status);
#endif
    return res.status;
}
                  ...
                  adhoc_communication::SendMmRobotPosition::Response &res)
{
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_POSITION, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendMmRobotPosition", time_call,
                             getMillisecondsTime(), s_msg.size(), res.status);
#endif
    return res.status;
}
                 ...
                 adhoc_communication::SendExpAuction::Response &res)
{
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_AUCTION, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendExpAuction", time_call, getMillisecondsTime(),
                             s_msg.size(), res.status);
#endif
    return res.status;
}
                 ...
                 adhoc_communication::SendExpCluster::Response &res)
{
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_CLUSTER, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendExpCluster", time_call, getMillisecondsTime(),
                             s_msg.size(), res.status);
#endif
    return res.status;
}
 ...
 */
    ...
    res.status =
        sendPacket(req.dst_robot, req.data, FRAME_DATA_TYPE_ANY, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS

    Logging::logServiceCalls("SendString", time_call, getMillisecondsTime(),
                             req.data.size(), res.status);
#endif
    return res.status;
}
 ...
 */
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_POINT, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendMmPoint", time_call, getMillisecondsTime(),
                             s_msg.size(), res.status);
#endif
    return res.status;
}
                        ...
                        adhoc_communication::SendMmControl::Response &res)
{
    ...
    res.status = sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_CONTROLL_MSG,
                            req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendMmControl", time_call, getMillisecondsTime(),
                             s_msg.size(), res.status);
#endif
    return res.status;
}
                   ...
                   adhoc_communication::SendMmMapUpdate::Response &res)
{
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_MAP_UPDATE, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendMmMapUpdate", time_call,
                             getMillisecondsTime(), s_msg.size(), res.status);
#endif
    return res.status;
}
 ...
 */
    ...
    res.status =
        sendPacket(req.dst_robot, s_msg, FRAME_DATA_TYPE_FRONTIER, req.topic);

#ifdef PERFORMANCE_LOGGING_SERVICE_CALLS
    Logging::logServiceCalls("SendExpFrontier", time_call,
                             getMillisecondsTime(), s_msg.size(), res.status);
#endif
    return res.status;
}
...
bool sendRouteRequest(string *dst, routing_entry *route)
{
    ...
    /*SEND route request*/
    ROS_INFO("SEND ROUTE REQUEST: HOST[%s] ID[%u]", dst->c_str(),
             rr.header_.id);
    string network_string = rr.getRequestAsNetworkString(src_mac);
    socketSend(network_string);
    ...
    // Wait for the route response
    {
        ...
        boost::thread resendRouteRequestT(resendRouteRequest,
                                          boost::ref(out_req),
                                          boost::ref(got_request_response));
...
bool gotAckRoutedFrame(routing_entry &route)
{
    unsigned long start_time = getMillisecondsTime();
    {
        ...
        boost::thread resendRoutedFrameT(resendRoutedFrame,
                                         boost::ref(got_rframe_ack), route);
    ...
    {
        //   ROS_ERROR("SUCCESSFULLY SENT ROUTED FRAME: ID[%u] DESTINATION
        //   HOST[%s]", unack_routed_frame->frame.header_.frame_id,
        //   route.hostname_destination.c_str());
    }
    else
    {
        ROS_WARN("Could not sent frame: ID[%u] destination host[%s]",
                 unack_routed_frame->frame.header_.packet_id,
                 route.hostname_destination.c_str());
        {
            loc.unlock();
    ...
    Logging::logTransportFrame(unack_routed_frame, &route, start_time, end_time,
                               unack_routed_frame->frame_is_ack);
    return unack_routed_frame->frame_is_ack;
}

bool sendPacket(std::string &hostname_destination, std::string &payload,
                uint8_t data_type_, std::string &topic)
{
    packet_id_++;
     ...
     * multicast: an entry for hostname_destination is in mc_handler, i.e., this
     * host joined the mc_group hostname_destination
     * unicast: otherwise
     */
    ...
    if (!bcast)
    {
        ...
        if (!send_successfully)
        {
            ...
            if (mc_t != NULL && mc_t->connected && mc_t->activated)
            {
                ...
                ROS_DEBUG("DESTINATION IS A MC GROUP: [%s]",
                          mc_t->group_name_.c_str());
                mc_frame = true;
                ...
                {
                    ROS_DEBUG("THERE ARE NO OTHER MEMBERS IN THE MC GROUP [%s]",
                              mc_t->group_name_.c_str());
                    return true;
                }
            }
    ...
    header_len +=
        mc_frame ? route.hostname_destination.length() : 0; // if multicast

    uint32_t payload_len = payload.length();
    uint32_t payload_space =
        (max_frame_size - header_len); // NOTE: payload space must be at least 1
                                       // to send data over network
    /*Calculate amount of needed frames*/
    float frames_count_f =
        payload_len / (float)payload_space; // Amount of frames with no payload
    uint32_t frames_count = frames_count_f; // get integer of the float (if
                                            // frames_count_f is 9.34,
                                            // frames_count is 9.00 )
    if (frames_count_f >
        frames_count) // example: if frames_count_f is 9.34 i need 10 frames
        frames_count++;

    uint32_t packet_size = payload_len + (frames_count * header_len);

    ROS_DEBUG("PACKET INFO: PL[%u] PL SPACE[%u] HEADER LEN[%u] SIZE[%u]",
              payload_len, payload_space, header_len, frames_count);
    /*Check if packet is not too large and the frame have also space for payload
     * data*/
    if (packet_size >= (unsigned)max_packet_size)
    {
        ROS_ERROR("Packet[%uBYTES] is too big! Try to increase the "
                  "'max_packet_size' parameter [%uBYTES]",
                  packet_size, max_packet_size);
        return false;
    }
    else if (header_len >= (unsigned)max_frame_size)
    {
        ROS_ERROR("Header so long that there is no space for payload! Try to "
                  "increase the 'max_frame_size' parameter!");
        return false;
    }
    ...
    /*  this is needed for multicast if a node will request a frame which has
     * not been sent yet */
    for (unsigned int i = 0; i < frames_count; i++)
    {
        ...
        pos += payload_space;
        RoutedFrame f =
            RoutedFrame(topic, p, data_type_, packet_id_, i, frames_count);
    ...
    if (packet.isNack())
    {
        ...
        packet.ts_ = getMillisecondsTime() +
                     packet.size_ * 10; // fake time stamp to prevent that the
                                        // packet would not be too early deleted
    ...
    for (unsigned int i = 0; i < frames_count && cannot_send_frame == false;
         i++)
    {
        ...
        if (packet.isMcFrame() && !packet.isNack())
        {
            McPosAckObj *ack_obj = NULL;
            {
                /* safe an McPosAckObj before sending, because after sending it
                   might be too late, because the acknowledgment of the receiver
        ...
        else if (!packet.isMcFrame())
        {
            /*SAFE routed frame as unacknowledged*/
            {
                ...
                memcpy(unack_routed_frame->frame.header_.mac_destination_,
                       route.next_hop, 6);
            ...
            // if (route.hobs > 2)
            {
                ...
                resendUnackLinkFrame(hostname, f.header_.frame_id,
                                     route.next_hop, network_string,
                                     FRAME_TYPE_TRANSPORT_DATA);
#ifdef PERFORMANCE_LOGGING_UC_LINK_SUMMARY
                Logging::increaseProperty("num_unique_data_frames_sent");

                Logging::increaseProperty("num_total_bytes_sent",
                                          network_string.length());
#endif
            }
        }
        ...
        socketSend(network_string);
// ROS_INFO("SEND ROUTED FRAME: ID[%u] DESTINATION HOST[%s] NEXT HOP[%s] ROUTE
// ID[%u]", f.header_.frame_id, route.hostname_destination.c_str(),
// getMacAsStr(route.next_hop).c_str(), route.id);
#ifdef DELAY
        if (simulation_mode)
            boost::this_thread::sleep(
                boost::posix_time::milliseconds(delay)); // usleep(delay);
#endif
        ...
        if (packet.isMcFrame() && !packet.isNack())
        {
            ...
            McPosAckObj *mc_ack =
                getMcAckObj(&f.mc_g_name_, f.header_.packet_id,
            ...
            {
                boost::thread resendMcAckFrameT(deliverMcAckFrame,
                                                boost::ref(*stc_rf),
                                                boost::ref(route), false);
            ...
            {
                ROS_WARN(
                    "Could not sent pos ack packet with ID[%u] to group[%s]",
                    stc_rf->frame.header_.packet_id,
                    hostname_destination.c_str());
        ...
        if (!bcast)
        {
            if (!gotAckRoutedFrame(route))
            {
                ...
                ROS_WARN("Could not sent unicast packet: ID[%u] destination "
                         "host[%s]",
                         unack_routed_frame->frame.header_.packet_id,
                         route.hostname_destination.c_str());
                return false;
    ...
    if (!bcast)
        ROS_INFO("SUCCESSFULLY SEND PACKET: ID[%u] DEST[%s]", packet_id_,
                 hostname_destination.c_str());
    else if (packet.isMcFrame())
    {
        send_successfully = true;
        ...
        {
            ROS_INFO(
                "SUCCESSFULLY SEND MC NEGATIVE ACK PACKET: ID[%u] GROUP[%s]",
                packet_id_, hostname_destination.c_str());
        }
        else
        {
            ROS_INFO(
                "SUCCESSFULLY SEND MC POSITIVE ACK PACKET: ID[%u] GROUP[%s]",
                packet_id_, hostname_destination.c_str());
        }
    }
    else
...
int main(int argc, char **argv)
{
    ...
    ROS_ERROR("HOSTNAME[%s] MAC[%s] INTERFACE[%s]", hostname.c_str(),
              getMacAsStr(src_mac).c_str(), interface);
    ROS_ERROR("num_link_retrans=%u num_e2e_retrans=%u num_rreq=%u",
              num_link_retrans, num_e2e_retrans, num_rreq);
    ROS_ERROR("max_packet_size=%u max_frame_size=%u hop_limit_min=%u "
              "hop_limit_max=%u",
              max_packet_size, max_frame_size, hop_limit_min, hop_limit_max);
    ROS_ERROR("hop_limit_increment=%u beacon_interval=%u", hop_limit_increment,
              beacon_interval);
    ...
    ros::ServiceServer sendRobotUpdateS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_robot_update", sendRobotUpdate);

    ros::ServiceServer sendTwistS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_twist", sendTwist);
    ros::ServiceServer sendMapS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_map", sendMap);
    ros::ServiceServer sendPostionS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_position", sendPosition);
    ros::ServiceServer sendStringS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_string", sendString);
    ros::ServiceServer getNeighborsS = n_pub->advertiseService(
        robot_prefix + node_prefix + "get_neighbors", getNeighbors);
    ros::ServiceServer sendPointS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_point", sendPoint);
    ros::ServiceServer sendMapUpdateS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_map_update", sendMapUpdate);
    ros::ServiceServer sendControlMsgS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_control_message",
        sendControlMessage);
    ros::ServiceServer joinMCGroupS = n_pub->advertiseService(
        robot_prefix + node_prefix + "join_mc_group", joinMCGroup);
    ros::ServiceServer sendFrontierS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_frontier", sendFrontier);
    ros::ServiceServer sendClusterS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_cluster", sendCluster);
    ros::ServiceServer sendAuctionS = n_pub->advertiseService(
        robot_prefix + node_prefix + "send_auction", sendAuction);
    ros::ServiceServer sendBcastS = n_pub->advertiseService(
        robot_prefix + node_prefix + "broadcast_robot_update",
        sendBroadcastCMgrRobotUpdate);
    ros::ServiceServer sendBcastStringS = n_pub->advertiseService(
        robot_prefix + node_prefix + "broadcast_string", sendBroadcastString);

    ros::ServiceServer shutDownRosS = n_pub->advertiseService(
        robot_prefix + node_prefix + "shut_down", shutDownRos);

    ros::ServiceServer getGroupStatusS = n_pub->advertiseService(
        robot_prefix + node_prefix + "get_group_state", getGroupStateF);

    publishers_l.push_front(n_pub->advertise<std_msgs::String>(
        robot_prefix + node_prefix + topic_new_robot, 1000, true));
    publishers_l.push_front(n_pub->advertise<std_msgs::String>(
        robot_prefix + node_prefix + topic_remove_robot, 1000, true));
    ...
    /*Tutorial*/
    // ros::ServiceServer sendQuaternionS =
    // n->advertiseService("send_quaternion", sendQuaternion);
    /*INIT THREADS*/

    boost::thread receiveFramesT(receiveFrames);
    boost::thread processIncomingFramesT(processIncomingFrames);
    // boost::thread resendFrameT(&resendFrames); // this thread re-sends frames
    // ineffective and too less accurate
    ...
    // boost::thread checkMutexAvailabilityT(&checkMutexAvailability,
    // &mtx_packets_incomplete, "packets incomplete");
    // boost::thread checkMutexAvailabilisastyT(&checkMutexAvailability,
    // &mtx_cached_mc_packets, "mtx_cached_mc_packets");

    list<ros::Subscriber> sub_robot_pos_l;

    boost::thread log_mem_consumption_packets(
        &Logging::logMemoryConsumptionPackets, 5000, &mtx_packets_incomplete,
        &packets_incomplete_l, &mtx_cached_mc_packets, &cached_mc_packets_l,
        &mtx_unack_link_frames, &unack_link_frames_l, &mtx_unack_cr_frames,
        &unack_cr_frames_l);
    ...
    if (simulation_mode)
    {
        ...
        const char *robot_number_pos =
            hostname.data() + 6; // Example: robot_1 or robot_2 -> robot_i
        try
        {
            my_sim_position->robot_number_ =
                boost::lexical_cast<uint32_t>(std::string(robot_number_pos));
        }
        catch (boost::bad_lexical_cast const &)
        {
            ROS_FATAL("If parameter simulation mode is set, the hostname of "
                      "the robot must be like the robot names of the stage "
                      "robot names, e.g., robot_0 or robot_1 etc.");
            return 0;
        }
        ...
        for (int i = 0; i < robots_in_simulation; i++)
        {
            if ((unsigned)i != my_sim_position->robot_number_)
            {
                ...
                sub->robot_number_ = i;
                sub_robot_pos_l.push_front(n_pub->subscribe(
                    topic_to_sub, 1, &PositionSubscriber::Subscribe, &*sub));
                robot_positions_l.push_front(sub);
            }
            else
                sub_robot_pos_l.push_front(
                    n_pub->subscribe(topic, 1, &PositionSubscriber::Subscribe,
                                     &*my_sim_position));
        }
    }
    ...
    {
        /* Create own mc group */
        ROS_INFO("Create own group! %s ",
                 std::string("mc").append("_").append(hostname).c_str());
        boost::unique_lock<boost::mutex> lock(mtx_mc_groups);
        mc_handler.createGroupAsRoot(
            &std::string("mc").append("_").append(hostname));
    }
    else
    {
        /* create default one */
        ROS_INFO("Create default group! %s ",
                 std::string("mc").append("_default").c_str());
        boost::unique_lock<boost::mutex> lock(mtx_mc_groups);
...
bool gotAck(AckLinkFrame *inc_frame)
{
     ...
     * If CR is enabled, also the unacknowledged CR frames will be managed by
     * this function
     *
     * Return:
     * 		(true) if got acknowledgment of unacknowledged link frame
     *
     * 		(false) if got acknowledgment of frame that is not in
     * unack_link_frames_l
     */
...

    {
        ...
        std::list<stc_frame>::iterator i = std::find(
            unack_link_frames_l.begin(), unack_link_frames_l.end(), f);
        if (i != unack_link_frames_l.end())
        {
#ifdef PERFORMANCE_LOGGING_UC_LINK_SUMMARY
            if (compareMac(inc_frame->header_.mac_confirmer,
                           inc_frame->eh_h_.eh_source))
                Logging::increaseProperty(
                    "num_acknowledgments_received_directly");
            else
                Logging::increaseProperty("num_acknowledgments_received_relay");

            Logging::increaseProperty("num_total_bytes_received",
                                      inc_frame->buffer_str_len_);
#endif

#ifdef PERFORMANCE_LOGGING_UC_LINK_FRAMES
            Logging::logUcLinkTransmission(*i);
#endif
            ROS_INFO("GOT ACK OF LINK FRAME: ID[%u] SOURCE MAC[%s] TYPE[%s]",
                     (*i).frame_id, getMacAsStr((*i).mac).c_str(),
                     frame_types[(*i).type].c_str());
...

    {
        ...
        for (std::list<ack_cr_info>::iterator i = unack_cr_frames_l.begin();
             i != unack_cr_frames_l.end();)
        {
            if ((*i).id == inc_frame->header_.frame_id &&
                (*i).source_host.compare(inc_frame->hostname_source_) == 0 &&
                compareMac((*i).frame_dst_mac,
                           inc_frame->header_.mac_confirmer))
            {
                if (compareMac((*i).frame_src_mac,
                               inc_frame->header_.mac_destination_))
                    ROS_INFO("GOT ACK OF CR FRAME: ID[%u] SOURCE MAC[%s]",
                             (*i).id,
                             getMacAsStr(inc_frame->eh_h_.eh_source).c_str());
                else if (compareMac(inc_frame->header_.mac_destination_,
                                    src_mac))
                {
                    ROS_INFO("GOT ACK OF RETRANSMITTED CR FRAME: ID[%u] SOURCE "
                             "MAC[%s]",
                             (*i).id,
                             getMacAsStr(inc_frame->eh_h_.eh_source).c_str());
                    sendLinkAck((*i).frame_src_mac, (*i).frame_dst_mac, (*i).id,
                                (*i).source_host, false, (*i).frame_type);
#ifdef PERFORMANCE_LOGGING_UC_LINK_SUMMARY
                    Logging::increaseProperty("num_acknowledgments_relayed");
                    Logging::increaseProperty("num_total_bytes_sent",
                                              AckLinkFrame::HEADER_FIXED_LEN +
                                                  (*i).source_host.length());
#endif
                }
            ...
            else if (getMillisecondsTime() - (*i).ts >=
                     MAX_TIME_CACHE_UNACK_CR_FRAMES)
            {
                ROS_WARN("DROP UNACKNOWLEGED CR FRAME: ID[%u] SOURCE MAC[%s]",
                         (*i).id,
                         getMacAsStr(inc_frame->eh_h_.eh_source).c_str());
...
void receiveFrames()
{
    ...
    /* Description:
     * This function receive all incoming frames on the interface and process
     * them.
     * Behavior of the function will be explained within the function.
     *
     * NOTE: This function runs as own thread.
     */
    unsigned char *buffer_incoming =
        new unsigned char[ETHER_MAX_LEN]; // ETHER_MAX_LEN]; /* Buffer for
    ...
    while (ros::ok())
    {
        ...
        /*
         * 	int i = recvfrom(raw_socket, buffer_incoming, ETHER_MAX_LEN, 0,
         NULL, NULL);
         if (i == -1 && ros::ok())
         {
         perror("recvfrom():");
        ...
        /*CHECK if the incoming frame has the Ethernet type of the routing
         * protocol*/
         ...
         * It can be defined how many percentages of all incoming protocol
         * frames will be dropped. (PERCENTAGE_OF_PACKET_LOST) -> defines.h
         *
         */
        ...
        if (loss_ratio > 0)
        {
            if (incoming_frame_count == 0)
            {
                ...
                ROS_ERROR("----------------------------------------------------"
                          "--------------------------");
                ROS_ERROR("----------------------LOSE FRAMES IS "
                          "ACTIVATED!!------------------------------");
                ROS_ERROR("----------------------------------------------------"
                          "--------------------------");
            }
            if (frame_type == FRAME_TYPE_TRANSPORT_DATA ||
                frame_type == FRAME_TYPE_TRANSPORT_ACK)
            {
                incoming_frame_count++;

                if ((float)incoming_frames_lost / (float)incoming_frame_count <=
                    loss_ratio)
                {
        ...
        uint8_t buffer_index = current_buffer_index - 1 >= 0
                                   ? current_buffer_index - 1
                                   : max_safed_frames - 1;
        // process_frame = !isBufferInList(buffer_incoming, &frames_as_strings,
        // buffer_index); todo
        ...
        buffer_size = 250; // todo create buffer_str_len member for all other
                           // frame classes and set it after frame is created
                           // (like mc maintenace frame)
        /*frame*/
        ...
        else if (frame_type == FRAME_TYPE_ACK) // ACK PACKET
        {
            ...
            bool frame_is_4_me =
                compareMac(src_mac, inc_ack_lframe.header_.mac_destination_);

            buffer_size = inc_ack_lframe.buffer_str_len_;

            /* continue if cr is disabled and frame is an neg ack*/
            if (enable_cooperative_relaying == false && !frame_is_4_me &&
                inc_ack_lframe.pos_ack_flag_)
            {
                continue;
            }
        ...
        else if (frame_type == FRAME_TYPE_TRANSPORT_DATA) // && (packet_4_me ||
                                                          // packet_is_bcast))
        {
        ...
        else if (frame_type == FRAME_TYPE_BROADCAST)
        {
            ...
            if (std::find(bcasts_l.begin(), bcasts_l.end(), bc_str) ==
                    bcasts_l.end() &&
                bcast.hostname_source_.compare(hostname) != 0)
            {
                ...
                if (bcast.header_.payload_type == FRAME_DATA_TYPE_ROBOT_UPDATE)
                {
                    ...
                    desializeObject((unsigned char *)bcast.payload_.data(),
                                    bcast.payload_.length(), &r_up);
            ...
            ROS_ERROR("UNKNOWN FRAMETYPE: [%u] source %s dest %s", frame_type,
                      getMacAsStr(ethernet_header->eh_source).c_str(),
                      getMacAsStr(dest_mac_p).c_str());

        if (frame_type != FRAME_TYPE_BEACON)
        {
            current_buffer_index =
                current_buffer_index + 1 < frames_as_strings.size()
                    ? current_buffer_index + 1
                    : 0;
            frames_as_strings[current_buffer_index] =
                std::string((const char *)buffer_incoming, buffer_size);
        }
...
template <class message> void publishMessage(message m, string topic)
{
    /* Description:
     * Publish any ROS message on a given topic.
     * Maximal different topics to publish are limited by MAX_DIFFERENT_TOPICS
     * -> defines.h
     */
    ...
    try
    {
        ...
        for (std::list<ros::Publisher>::iterator i = publishers_l.begin();
             i != publishers_l.end(); i++)
        {
        ...
        if (!pubExsists)
        {
            publishers_l.push_front(
                n_pub->advertise<message>(topic, 1000, true));
...
void sendBeacons()
{
     ...
     *
     * NOTE: The neighbor table is only used for debugging reasons. The protocol
     * will also work without neighbor table.
    ...
    while (ros::ok())
    {
        ...
        socketSend(beacon);
        {
            ...
            while (i != neighbors_l.end())
            {
                ...
                if (getMillisecondsTime() - neighbor.ts >=
                        TIME_BEFORE_REMOVE_NEIGHBOR &&
                    neighbor.reachable)
                {
                    ROS_ERROR("REMOVE NEIGHBOR: HOSTNAME[%s]",
                              neighbor.hostname.c_str());
                    std_msgs::String msg;
                ...
                {
                    ROS_DEBUG("NEIGHBOR: HOSTNAME[%s] LAST BEACON GOT[%lu]",
                              neighbor.hostname.c_str(), neighbor.ts);
                }
                i++;
            }
        }
...
{
    boost::unique_lock<boost::mutex> lock(mtx_mc_ack_l);
    for (list<McPosAckObj *>::iterator i = mc_ack_l.begin();
         i != mc_ack_l.end(); i++)
    {
        McPosAckObj *ack_obj = *i;
        if (ack_obj->group_name.compare(*group) == 0 &&
            p_id == ack_obj->packet_id && ack_obj->sequence_num == seq)
        {
            return ack_obj;
        }
    }
...
void sendLinkAck(unsigned char *dest, unsigned char *confirmer_mac, uint32_t id,
                 string source, bool cr, uint8_t type)
{
    AckLinkFrame ack =
        AckLinkFrame(src_mac, confirmer_mac, dest, id, source, type);
    ack.cr_flag_ = cr;
...
void processMcAckFrame(McAckFrame *f)
{
    ...
    McPosAckObj *ack_obj = getMcAckObj(&f->mc_group_, f->header_.packet_id,
                                       f->header_.frame_seq_num);
    boost::unique_lock<boost::mutex> lock(mtx_mc_ack_l);
    if (ack_obj != NULL)
    {
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
        Logging::increaseProperty("num_mc_acknowledgments_received_directly");
        Logging::increaseProperty("num_mc_total_bytes_received ",
                                  f->buffer_str_len_);
#endif
        // ROS_ERROR("GOT MULTICAST ACK: GROUP[%s] HOST[%s] PACKET[%u]
        // SEQUENCE[%u] FROM [%s]", f->mc_group_.c_str(),
        // f->hostname_source_.c_str(), f->header_.packet_id,
        // f->header_.frame_seq_num, getMacAsStr(f->eh_h_.eh_source).c_str());
        if (ack_obj->GotAck(f))
        {
            boost::unique_lock<boost::mutex> lockd(mtx_notify);
            got_rframe_ack.notify_all();

            ROS_DEBUG("GOT ALL MULTICAST ACK: GROUP[%s] HOST[%s] PACKET[%u] "
                      "SEQUENCE[%u]",
                      f->mc_group_.c_str(), f->hostname_source_.c_str(),
                      f->header_.packet_id, f->header_.frame_seq_num);

            if (f->hostname_source_.compare(hostname) != 0)
            {
                /*send ack*/
                McAckFrame arf =
                    McAckFrame(src_mac, f->eh_h_.eh_source, f->hostname_source_,
                               f->mc_group_, f->header_.packet_id,
                               f->header_.frame_seq_num);
...
void processAckRoutedFrame(AckRoutedFrame *in_a_rf)
{
    ...
    /*create struct of a frame, to safe it and make sure that every frame will
     * only received once*/
    ...
    bool frame_is_for_me =
        compareMac(in_a_rf->header_.mac_destination_, src_mac);

    if (frame_is_for_me)
    {
        /*Send acknowledgment of link frame*/
        sendLinkAck(in_a_rf->eh_h_.eh_source, src_mac, frame_to_safe.frame_id,
                    frame_to_safe.hostname_source, in_a_rf->cr_flag,
                    in_a_rf->header_.frame_type);
    }

    if (!frame_is_for_me && !enable_cooperative_relaying)
    {
        ROS_DEBUG("DONT PROCESS FRAME BECAUSE IT IS NOT FOR ME AND "
                  "COOP_RELAYING IS TURNED OFF!");
        return;
    }
    ...
    routing_entry route;
    {
        ...
        boost::unique_lock<boost::mutex> lock(mtx_routing_table);
        std::list<routing_entry>::iterator find_route =
            std::find(routing_table_l.begin(), routing_table_l.end(), route);
        /*Search the route in the routing table*/
        if (find_route == routing_table_l.end())
        {
            ROS_DEBUG("ROUTE FOR THE ACK FRAME NOT FOUND: ID[%u] HOSTNAME "
                      "SOURCE[%s] ROUTE ID[%u]",
                      in_a_rf->header_.frame_id,
                      in_a_rf->hostname_source_.c_str(),
                      in_a_rf->header_.route_id);
            return;
        }
    ...
    if (frame_is_for_me)
    {
        ...
        if (route.hostname_source.compare(hostname) == 0)
        {
            ...
            ROS_DEBUG("GOT ACKNOWLEGEMENT OF ROUTED FRAME: HOSTNAME[%s] ID[%u]",
                      route.hostname_destination.c_str(),
                      in_a_rf->header_.frame_id);
#ifdef PERFORMANCE_LOGGING_UC_LINK_SUMMARY
...

            {
                ...
                if (in_a_rf->header_.frame_id ==
                        unack_routed_frame->frame.header_.frame_id &&
                    route.hostname_destination.compare(
                        unack_routed_frame->hostname_destination) == 0)
                {
        ...
        {
            // ROS_ERROR("FORWARD ACK OF ROUTED FRAME: FROM[%s]->TO[%s]",
            // getMacAsStr(in_a_rf->eh_h_.eh_source).c_str(),
            // getMacAsStr(route.previous_hop).c_str());

            string network_string =
                in_a_rf->getFrameAsNetworkString(route, src_mac);

            /*SAFE the packet as unacknowledged*/
            resendUnackLinkFrame(in_a_rf->hostname_source_,
                                 in_a_rf->header_.frame_id, route.previous_hop,
                                 network_string, FRAME_TYPE_TRANSPORT_ACK);
...
void processRoutedFrame(RoutedFrame *f)
{
    ...
    /*Check if I am the destination of the frame*/
    bool frame_is_for_me =
        compareMac(inc_frame.header_.mac_destination_, src_mac);

    if (frame_is_for_me)
    {
        /*Send acknowledgment */
        sendLinkAck(inc_frame.eh_h_.eh_source, src_mac,
                    inc_frame.header_.frame_id, inc_frame.hostname_source_,
                    f->cr_flag, f->header_.frame_type);
    }
    ...
    {
        // ROS_ERROR("GOT R FRAME ALREADY %u",
        // inc_frame.header_.packet_sequence_num);
        return;
    }
    ...
    route_f.hostname_source = inc_frame.hostname_source_;
    {
        boost::unique_lock<boost::mutex> lock(mtx_routing_table);
        std::list<routing_entry>::iterator find_route =
            std::find(routing_table_l.begin(), routing_table_l.end(), route_f);
        if (find_route == routing_table_l.end())
        {
            ROS_DEBUG("ROUTE FOR THE ROUTED FRAME NOT FOUND: ID[%u] HOSTNAME "
                      "SOURCE[%s] ROUTE ID[%u]",
                      inc_frame.header_.frame_id,
                      inc_frame.hostname_source_.c_str(),
                      inc_frame.header_.route_id);
            return;
        }
    ...
    {
        ROS_DEBUG("GOT ROUTED FRAME: HOSTNAME[%s] ID[%u]",
                  route.hostname_destination.c_str(),
                  inc_frame.header_.frame_id);
        ...
        std::string network_string =
            arf.getFrameAsNetworkString(route, src_mac);
#ifdef DELAY
        if (simulation_mode)
            boost::this_thread::sleep(boost::posix_time::milliseconds(delay));
#endif

        resendUnackLinkFrame(arf.hostname_source_, arf.header_.frame_id,
                             arf.header_.mac_destination_, network_string,
                             FRAME_TYPE_TRANSPORT_ACK);

        socketSend(network_string);
        ROS_DEBUG("SEND ACKNOWLEGEMENT OF ROUTED FRAME: ID[%u] DESTINATION[%s] "
                  "NEXT HOP[%s]",
                  inc_frame.header_.frame_id,
                  route.hostname_destination.c_str(),
                  getMacAsStr(route.previous_hop).c_str());
    ...
    {
        string network_string =
            inc_frame.getFrameAsNetworkString(route, src_mac);
        ...
        resendUnackLinkFrame(f->hostname_source_, f->header_.frame_id,
                             inc_frame.header_.mac_destination_, network_string,
                             FRAME_TYPE_TRANSPORT_DATA);

        ROS_DEBUG("FORWARD ROUTED FRAME: FROM[%s] TO[%s]",
                  getMacAsStr(route.previous_hop).c_str(),
                  getMacAsStr(route.next_hop).c_str());
#ifdef PERFORMANCE_LOGGING_UC_LINK_SUMMARY
        Logging::increaseProperty("num_unique_data_frames_forwarded");
        Logging::increaseProperty("num_total_bytes_sent ",
                                  network_string.length());
...
void sendMcAck(unsigned char *dst, string h_source, string g_name,
               uint32_t packet_id, uint32_t seq_num)
{
    ROS_DEBUG(
        "SEND POS MC ACKNOWLEGEMENT: ID[%u] GROUP[%s] SOURCE[%s] FROM[%s]",
        packet_id, g_name.c_str(), h_source.c_str(),
        getHostnameFromMac(dst).c_str());
    /*SEND ack to the root source */

    McAckFrame arf =
        McAckFrame(src_mac, dst, h_source, g_name, packet_id, seq_num);
    string network_string = arf.getFrameAsNetworkString();
    socketSend(network_string);
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
    Logging::increaseProperty("num_mc_acknowledgments_sent");
...
void deliverMcAckFrame(stc_RoutedFrame &stc_rf, routing_entry &r, bool send_ack)
{
    ...
    while (stc_rf.frame_is_ack == false &&
           stc_rf.retransmitted <= num_e2e_retrans)
    {
        {
            boost::unique_lock<boost::mutex> lockAckRF(mtx_notify);
            boost::thread *resendMcFrameT = new boost::thread(
                resendMcFrame, boost::ref(got_rframe_ack), &stc_rf, r);
            got_rframe_ack.wait(lockAckRF); // wait for response
            resendMcFrameT->interrupt();
            delete resendMcFrameT;
        }

        McPosAckObj *mc_ack = getMcAckObj(
            &stc_rf.frame.mc_g_name_, stc_rf.frame.header_.packet_id,
            stc_rf.frame.header_.packet_sequence_num);
    ...
    if (send_ack && stc_rf.frame_is_ack)
    {
        ...
        sendMcAck(stc_rf.frame.eh_h_.eh_source, stc_rf.frame.hostname_source_,
                  stc_rf.frame.mc_g_name_, stc_rf.frame.header_.packet_id,
                  stc_rf.frame.header_.packet_sequence_num);
    }
    ...
    {
        ROS_WARN("COULD NOT SENT MC ACK FRAME: SOURCE[%s] GROUP[%s] "
                 "PACK_ID[%u] SEQ_N[%u]",
                 stc_rf.frame.hostname_source_.c_str(),
                 stc_rf.frame.mc_g_name_.c_str(),
                 stc_rf.frame.header_.packet_id,
                 stc_rf.frame.header_.packet_sequence_num);
    }
...
void processBroadcastFrame(RoutedFrame *f)
{
    ...
    if (inc_frame.mc_flag)
    {
        ...
        if (g != NULL && (!g->activated || !g->connected ||
                          !g->processFrame(inc_frame.eh_h_.eh_source)))
        {
            //     ROS_ERROR("break %u %u %u",!g->activated, !g->connected ,
            //     !g->processFrame(inc_frame.eh_h_.eh_source));
            return;
        }
    }
    ...
    if (gotFrameRecently(inc_frame))
    {
        // ROS_ERROR("got bcast frame recently %u",
        // inc_frame.header_.packet_sequence_num);
        if (f->negative_ack_type == false || (f->cr_flag && f->mc_flag))
            sendMcAck(inc_frame.eh_h_.eh_source, inc_frame.hostname_source_,
                      inc_frame.mc_g_name_, inc_frame.header_.packet_id,
                      inc_frame.header_.packet_sequence_num);
    ...
    if (inc_frame.mc_flag)
    {
        bool propagated = false;
        {
            ...
            if (!mc_t->activated)
            {
                ROS_ERROR("MC ROUTE IS NOT ACTIVE [%s]",
                          inc_frame.mc_g_name_.c_str());
                return;
            }
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
            Logging::increaseProperty(
        ...
        ROS_INFO("GOT MC FRAME: ID[%u] GROUP[%s] SOURCE[%s] FROM[%s] P_ID[%u] "
                 "SEQ_N[%u]",
                 inc_frame.header_.frame_id, inc_frame.mc_g_name_.c_str(),
                 inc_frame.hostname_source_.c_str(),
                 getHostnameFromMac(inc_frame.eh_h_.eh_source).c_str(),
                 inc_frame.header_.packet_id,
                 inc_frame.header_.packet_sequence_num);
        ...
        {
            /* If the frame was propagated, the node is a end node and can send
             * the acknowledgement*/
            ...
            if (propagated)
            {
                ...
                boost::thread resendMcAckFrameT(deliverMcAckFrame, stc_rf,
                                                route, recursive_mc_ack);
            }
        }

        if (propagated)
        {
            memcpy((unsigned char *)inc_frame.header_.mac_destination_,
                   (unsigned char *)bcast_mac, 6);
            inc_frame.resend_flag = false;
            string network_string = inc_frame.getFrameAsNetworkString(
                inc_frame.header_.route_id, inc_frame.header_.mac_destination_,
                inc_frame.hostname_source_, src_mac);
            socketSend(network_string);
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
            ...
            Logging::increaseProperty("num_mc_total_bytes_sent ",
                                      network_string.length());
#endif
            ROS_DEBUG("FORWARD MC FRAME: ID[%u] GROUP[%s] SOURCE[%s]",
                      inc_frame.header_.frame_id, inc_frame.mc_g_name_.c_str(),
                      inc_frame.hostname_source_.c_str());
        }
        else
            ROS_DEBUG("Don't propergate mc frame: ID[%u] group[%s] source[%s]",
                      inc_frame.header_.frame_id, inc_frame.mc_g_name_.c_str(),
                      inc_frame.hostname_source_.c_str());
    ...
    {
// ROS_DEBUG("GOT BROADCAST: SOURCE HOST[%s]
// ID[%u]",getMacAsCStr(inc_frame.mac_source_),inc_frame.header_.frame_id);
...
void processRouteRequest(RouteRequest req)
{
    ...
    bool route_req_exsits = true;
    {
        std::list<route_request>::iterator findReq;
        {
            ...
            findReq = std::find(route_requests_l.begin(),
                                route_requests_l.end(), inc_req);
        ...
        if ((req.hostname_destination_.compare(hostname) == 0 ||
             (mc_g != NULL && mc_g->connected && mc_g->activated)))
        {
            lock_mc_groups.unlock();
            uint16_t counter = 1;
            if (route_req_exsits)
            ...
            lock_mc_groups.lock();
            {
                ...
                boost::unique_lock<boost::mutex> lock_rreq(mtx_route_requests);
                Logging::logRRequestReceiver(
                    req.hostname_destination_, req.header_.id, counter,
                    req.header_.hop_count - 1, counter == 1, req.path_l_);
            }
    ...
    if (req.hostname_source_.compare(hostname) != 0 &&
        (req.header_.hop_count + 1 <= req.header_.hop_limit ||
         req.header_.hop_limit == 0) &&
        !route_req_exsits) //(!routeReqestExsists || req.mc_flag_))
    {
        if (req.mc_flag_ == false)
        {
            ...
            if (req.hostname_destination_.compare(hostname) == 0)
            {
                ...
                ROS_INFO("GOT ROUTE REQUEST FOR ME: SOURCE[%s] ID[%u]",
                         req.hostname_source_.c_str(), req.header_.id);
                ...
                ROS_INFO("ADD ROUTE: ID[%u] SOURCE[%s] DEST[%s] NEXT HOP[%s] "
                         "PREVIUS HOP[%s] PATH:[%s]",
                         new_route.id, new_route.hostname_source.c_str(),
                         new_route.hostname_destination.c_str(),
                         getMacAsStr(new_route.next_hop).c_str(),
                         getMacAsStr(new_route.previous_hop).c_str(),
                         getPathAsStr(rr.path_l_).c_str());
                 ...
                 * the request process duration is too fast for the "request
                 * resend thread".
                 * The source gets the response before that thread can be
                 * started and so the thread will resend the request for no
                 * reason
                 * To avoid this problem, a little delay must be added for this
                 * case
                 */
            ...
            {
                ROS_INFO("FORWARD REQUEST: SOURCE HOST[%s] DESTINATION "
                         "HOST[%s] MAX HOPS[%u] CURRENT HOP[%u]",
                         req.hostname_source_.c_str(),
                         req.hostname_destination_.c_str(),
                         req.header_.hop_limit, req.header_.hop_count + 1);
                // DEBUG
...
/*Logging */
        ...
        else if (req.mc_flag_)
        {
            ...
            if (mc_t != NULL && mc_t->connected &&
                mc_t->activated) // || mc_t.connected == false)
            {
                /* return if request is from the uplink */
                if (!mc_t->root && compareMac(mc_t->route_uplink_->next_hop,
                                              req.eh_h_.eh_source))
                    return;
                inc_req.counter = 1;
                inc_req.response_sent = true;
                ...
                memcpy((void *)route->previous_hop, (void *)req.eh_h_.eh_source,
                       6);
                ...
                RouteResponse rr = RouteResponse(
                    req, src_mac,
                    mc_t->route_uplink_->root_distance + req.header_.hop_count);

                // ROS_DEBUG("Path[%s]", getPathAsCStr(req.path_l_));
                ROS_INFO("GOT MC REQUEST FOR ME AS DEST: MC GROUP[%s] "
                         "SOURCE[%s] ID[%u] HOPS[%u] RD[%u]",
                         mc_t->group_name_.c_str(),
                         req.hostname_source_.c_str(), req.header_.id,
                         req.header_.hop_count,
                         mc_t->route_uplink_->root_distance);
                string network_string = rr.getResponseAsNetworkString(src_mac);
                socketSend(network_string);
            ...
            else if (mc_t != NULL && mc_t->outgoing_request_ != NULL &&
                     getMillisecondsTime() - mc_t->time_stamp_ <
                         INTERVAL_WAIT_FOR_MCROUTES)
            {
                ...
                {
                    // ROS_ERROR("ADD REQUEST IN WAITING LIST: DEST[%s]
                    // PATH:[%s]", request->hostname_destination_.c_str(),
                    // getPathAsStr(request->path_l_).c_str());
            ...
            else if (!route_req_exsits) // only forward request if I have not
                                        // already forwarded the req
            {
                ROS_INFO("FORWARD MC REQUEST: SOURCE HOST[%s] DESTINATION "
                         "GROUP[%s] MAX HOPS[%u] CURRENT HOP[%u]",
                         req.hostname_source_.c_str(),
                         req.hostname_destination_.c_str(),
                         req.header_.hop_limit, req.header_.hop_count + 1);
                inc_req.response_sent = false;
                ...
                mc_handler.getMcGroup(&req.hostname_destination_)
                    ->safeOutgoingRequest(new RouteRequest(req));
                {
                    boost::unique_lock<boost::mutex> lock(mtx_routing_table);
                    Logging::logRoutingTable(&routing_table_l, &mc_groups_l);
                }
            ...
            else
            {
                ...
                /* return the function to prevent that the request will be
                 * stored if routeRequest already exists */
                return;
            }
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
...
void processRouteResponse(RouteResponse rr)
{
    ...
    std::list<route_request>::iterator find_rreq =
        std::find(route_requests_l.begin(), route_requests_l.end(),
                  incoming_req); // find the response

    /*Check if was involved in the route request process, so that i can forward
     * the response*/
    if (find_rreq != route_requests_l.end())
    {
        route_request &request(*find_rreq); // safe a reference of the request
        routing_entry new_route;
        /*Check if i am the sender to forward and if I already forward the
         * response*/
        //  std::string currenthop = getHostnameFromMac( rr.mac_current_hop_);
        //  ROS_DEBUG("current host: %s
        //  %u",currenthop.c_str(),request.response_sent);
        if (compareMac(rr.mac_current_hop_, src_mac) &&
            request.response_sent == false)
        {
            ...
            RouteResponse resp_2_get_next_hop = RouteResponse(
            ...
            if (rr.mc_flag_ == false)
            {
                ...
                if (rr.current_hop_ == rr.hop_count_ &&
                    rr.hostname_source_ == hostname)
                {
                    ROS_INFO("GOT ROUTE RESPONSE FROM[%s]",
                             request.hostname_destination.c_str());
                    ROS_INFO("PATH[%s]", getPathAsStr(rr.path_l_).c_str());

                    new_route.hostname_destination =
                        request.hostname_destination;
...

                    {
                        ...
                        routing_table_l.push_front(new_route);
                        boost::unique_lock<boost::mutex> lock_g(mtx_mc_groups);
                        Logging::logRoutingTable(&routing_table_l,
                                                 &mc_groups_l);
                    }
                    ...
                    Logging::increaseProperty(
                        ...
                        rr.hostname_source_.length() +
                            RouteResponse::HEADER_FIXED_LEN);
#endif
                } /*Forward the response*/
                else
                {

                    ROS_INFO("FORWARD RESPONSE FROM[%s]: SOURCE HOST[%s] "
                             "DESTINATION HOST[%s] NEXT HOP[%s]",
                             getMacAsStr(new_route.next_hop).c_str(),
                             new_route.hostname_source.c_str(),
                             new_route.hostname_destination.c_str(),
                             getMacAsStr(new_route.previous_hop).c_str());
                    ...
                    string network_string =
                        rr.getResponseAsNetworkString(src_mac);
                    socketSend(network_string);
#ifdef PERFORMANCE_LOGGING_UC_LINK_SUMMARY
                    Logging::increaseProperty("num_rrep_sent");
                    Logging::increaseProperty("num_total_bytes_received",
            ...
            else
            {
                /* Check if the mc tree has not been activated*/
                {
                    boost::unique_lock<boost::mutex> lock(mtx_mc_groups);
                    McTree *tree =
                        mc_handler.getMcGroup(&request.hostname_destination);
                ...
                route->root_distance =
                    rr.root_distance - (rr.hop_count_ - rr.current_hop_);
                ...
                if (rr.hostname_source_.compare(hostname) == 0)
                {
                    ...
                    Logging::increaseProperty(
                        "num_mc_total_bytes_received",
                        rr.hostname_source_.length() +
                            RouteResponse::HEADER_FIXED_LEN);
#endif
                    // ROS_ERROR("%s", getPathAsCStr(rr.path_l_));

                    ROS_INFO("GOT MC RESPONSE: ROOT DISTANCE[%u] FROM[%s] MC "
                             "GROUP[%s]",
                             route->root_distance,
                             getMacAsStr(rr.eh_.h_source).c_str(),
                             request.hostname_destination.c_str());
                ...
                {
                    /*forward*/
                    ROS_INFO("FORWARD MC RESPONSE FROM[%s]: SOURCE HOST[%s] "
                             "DESTINATION MC[%s] NEXT HOP[%s]",
                             getMacAsStr(rr.eh_.h_source).c_str(),
                             rr.hostname_source_.c_str(),
                             request.hostname_destination.c_str(),
                             getMacAsStr(new_route.previous_hop).c_str());
            ...
            ROS_INFO("I SENT ALREADY A RESPONSE: ID[%u] SOURCE HOST[%s]",
                     rr.request_id_, rr.hostname_source_.c_str());
        else if (compareMac(rr.mac_current_hop_, src_mac) == false)
            ROS_ERROR("I AM NOT THE CURRENT HOP[%s] TO FORWARD THE RESPONSE!",
                      getMacAsStr(rr.mac_current_hop_).c_str());
    }
    else
        ROS_DEBUG("WAS NOT INVOLVED IN ROUTING PROCESS: ID[%u] SOURCE HOST[%s]",
                  rr.request_id_, rr.hostname_source_.c_str());
}
 ...
 */
    ...
    Logging::increaseProperty("num_total_bytes_received",
                              beacon->HEADER_FIXED_LEN +
                                  beacon->hostname_.length());
#endif
    ...
    std::list<hostname_mac>::iterator current_neighbor =
        std::find(neighbors_l.begin(), neighbors_l.end(),
                  hm); // checks if the frame already exsists. return pointer to
                       // end if not
    if (current_neighbor != neighbors_l.end())
    {
        new_neighbor = false;

        (*current_neighbor).stamp(); // set the hello count of all neighbors
                                     // that already exsit to 0
...
void processMcActivationFrame(McRouteActivationFrame *f)
{
    ...
    sendLinkAck(f->eh_h_.eh_dest, src_mac, f->header_.id, "", false,
                FRAME_TYPE_MC_ACTIVATION);
    ...
    if (mc_t != NULL && ((mc_t->connected && mc_t->activated) || mc_t->root))
    {
        ...
        if (mc_t->addDownlinkAsMember(downlink))
        {
            ...
            ROS_INFO("ADD NEW LEAFE TO MC TREE: GROUP[%s] LEAFE MAC[%s]",
                     f->mc_group_.c_str(),
                     getMacAsStr(f->eh_h_.eh_source).c_str());
            // mc_handler.printMcGroups();
        }
        else
        {
            // ROS_ERROR("unexpected failure: could not add member[%s] as
            // downlink", getMacAsCStr(downlink->mac_adr));

            //  mc_t->printTree();
            // delete downlink;
        }
    } /* connector*/
    else if ((mc_t = mc_handler.getMcGroup(&f->hostname_source_,
                                           &f->header_.route_id)) != NULL)
    {
        ...
        {
            ROS_INFO("ACT AS CONNECTOR FOR MC TREE: GROUP[%s]",
                     mc_t->group_name_.c_str());
            if (!mc_t->activateRoute(&f->hostname_source_, &f->header_.route_id,
                                     f->eh_h_.eh_source))
                ROS_ERROR("unexpected failure: could not activate mc tree as "
                          "connector");

            McRouteActivationFrame r_act_frame = McRouteActivationFrame(
                mc_t->route_uplink_->next_hop, mc_t->group_name_,
                mc_t->route_uplink_->id, mc_t->route_uplink_->hostname_source);
            ...
            ROS_ERROR(
                "unexpected failure: could not add downlink as connector");
    }
...
void processIncomingFrames()
{
    /* Description:
     * Process a Packet.
     * Creates a new packet if the frame belongs to a new packet. If packet
     * already exists, the frame will be added to the packet.
    ...
    while (ros::ok())
    {
        ...
        while (frames.size() >= 1)
        {

            {
                ...
                boost::unique_lock<boost::mutex> lock_packets_inco(
                    mtx_packets_incomplete);
                // if (getMillisecondsTime() - time_lock >= 100)
                //     ROS_ERROR("dont got lock 1");
                for (std::list<Packet>::iterator p_i =
                         packets_incomplete_l.begin();
                     p_i != packets_incomplete_l.end();)
                {

                    bool packet_is_complete = false;
                    Packet &p(*p_i);

                    for (std::list<RoutedFrame>::iterator f_i = frames.begin();
                         f_i != frames.end();)
                    {
                    ...
                    if (packet_is_complete)
                    {
                        ...
                        if (p_comp.isMcFrame())
                        {
                            ...
                            boost::unique_lock<boost::mutex> lock(
                                mtx_cached_mc_packets);

                            for (std::list<Packet>::iterator it =
                                     cached_mc_packets_l.begin();
                                 it != cached_mc_packets_l.end(); it++)
                            {
                                Packet pa = *it;
                                if (pa.id_ == p_comp.id_ &&
                                    pa.hostname_source_.compare(
                                        p_comp.hostname_source_) == 0 &&
                    ...
                    else if (getMillisecondsTime() - p.ts_ >
                             INTERVAL_DELETE_OLD_PACKETS)
                    {
                        ROS_INFO("DROP UNFINISHED PACKET: ID[%u] SOURCE[%s] "
                                 "SIZE[%u/%zu]",
                                 p.id_, p.hostname_source_.c_str(), p.size_,
                                 p.frames_l_.size());
                        p_i = packets_incomplete_l.erase(p_i);
                    }
                    else
                 ...
                 f_i != frames.end();)
            {
                ...
                /* Look in incomplete packets */
                {
                    ...
                    for (std::list<Packet>::iterator it =
                             packets_incomplete_l.begin();
                         it != packets_incomplete_l.end(); it++)
                    {
                        Packet p = *it;
                        if (p.id_ == frame.header_.packet_id &&
                            p.hostname_source_.compare(
                ...
                if (new_packet)
                {
                    ...
                    for (std::list<Packet>::iterator it_pack =
                             cached_mc_packets_l.begin();
                         it_pack != cached_mc_packets_l.end(); it_pack++)
                    {
                        if ((*it_pack).id_ == frame.header_.packet_id &&
                            (*it_pack).hostname_source_.compare(
                                frame.hostname_source_) == 0 &&
                            (*it_pack).mc_group_.compare(frame.mc_g_name_) ==
                                0 &&
                            (*it_pack).size_ <= (*it_pack).frames_l_.size() &&
                            (*it_pack).size_ > 1)
                        {
                            new_packet = false;
                            // f_i = frames.erase(f_i);
                ...
                if (new_packet)
                {
                    ...
                    ROS_DEBUG(
                        "ADD NEW PACKET: ID[%u] GROUP[%s] SOURCE[%s] SIZE[%u]",
                        p.id_, p.mc_group_.c_str(), p.hostname_source_.c_str(),
                        p.size_);
...
void publishPacket(Packet *p)
{
    ...
    for (list<stc_packet>::iterator it = published_packets_l.begin();
         it != published_packets_l.end();)
    {

        if (pack == *it)
        {
            //        ROS_ERROR("DONT PUBLISH PACKET: ID[%u] SRC[%s/%s]",
            //        pack.id, pack.src.c_str(), pack.mc_group.c_str());
            return;
        }
    ...
    ROS_INFO("PUBLISH PACKET: ID[%u] SRC[%s/%s]", pack.id, pack.src.c_str(),
             pack.mc_group.c_str());

    if (p->mc_group_.compare("") == 0 || mc_member)
    {
        //  ROS_ERROR("GOT PACKET: ID[%u] SOURCE[%s] SIZE[%u]", p->id_,
        //  p->hostname_source_.c_str(), p->size_);
        try
        {
        ...
        getQuaternionFromNetworkString((unsigned
        char*)payload.data(),payload.length());
...
{
    /* Description:
     * Re-Send a route_request and notify if the request has its max
     * retransmissions reached.
     * Also the hop_limit_max will be increased if i get no response. The thread
     * stops also, if he will be interrupted.
     * The thread is supposed to be interrupted if the node gets an response of
     * the route request
     *
    ...
    while (boost::this_thread::interruption_requested() == false)
    {
        ...
        if (req.retransmitted >= (unsigned)num_rreq)
        {
            if (req.hop_limit < (unsigned)hop_limit_max)
            {
                ...
                req.retransmitted = 0;
                ROS_INFO("INCREASE HOP COUNT OF ROUTE REQUEST: ID[%u] "
                         "DESTINAION HOST[%s] MAX HOPS[%u]",
                         req.id, req.hostname_destination.c_str(),
                         req.hop_limit);
            }
            else
            {
                ROS_WARN("DROP ROUTE REQUEST: ID[%u] DESTINAION HOST[%s]",
                         req.id, req.hostname_destination.c_str());
        ...
        else
        {
            req.id++;
            ROS_INFO(
                "RESEND ROUTE REQUEST: ID[%u] DESTINAION HOST[%s] MAX HOPS[%u]",
                req.id, req.hostname_destination.c_str(), req.hop_limit);
...
void resendRoutedFrame(boost::condition_variable &condi, routing_entry route)
{
    ...
    /* Description:
     * Re-Send the unacknowledged routed frame and notify if the routed frame
     * has its max retransmissions reached.
    ...
    {
        {
            ...
            else
            {
                ...
                string network_string =
                    unack_routed_frame->frame.getFrameAsNetworkString(route,
                                                                      src_mac);
                // ROS_DEBBUG("RESEND ROUTED FRAME: ID[%u] DESTINAIONT HOST[%s]
                // NEXT HOP[%s]", unack_routed_frame->frame.header_.frame_id,
                // unack_routed_frame->hostname_destination.c_str(),
                // getMacAsStr(unack_routed_frame->frame.header_.mac_destination_).c_str());
                socketSend(network_string);
...
void resendMcFrame(boost::condition_variable &condi, stc_RoutedFrame *rf,
                   routing_entry route)
{
    Logging::increaseProperty("running_multicast_threads");
    ...
    if (!boost::this_thread::interruption_requested())
    {
        ...
        // ROS_DEBUG("RESEND MC FRAME: ID[%u] SRC HOST[%s] NEXT HOP[%s]",
        // rf->frame.header_.frame_id, rf->frame.hostname_source_.c_str(),
        // getMacAsStr(rf->frame.header_.mac_destination_).c_str());
        string network_string =
            rf->frame.getFrameAsNetworkString(route, src_mac);
        socketSend(network_string);
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
    ...
    {
        {
            ...
            while (
                r !=
                route_requests_l.end()) // check if the route request is from me
            {
                route_request &currentRequest(*r);
                if (getMillisecondsTime() - currentRequest.ts >
                    MAX_TIME_CACHE_ROUTE_REQUEST)
                {
                    ROS_DEBUG("DROP CACHED ROUTE REQUEST: ID[%u] DESTuINAION "
                              "HOST[%s]",
                              currentRequest.id,
                              currentRequest.hostname_destination.c_str());
    ...
    {
        {
            ...
            {
                Packet &p(*r);
                if ((p.size_ <= p.frames_l_.size() &&
                     getMillisecondsTime() > p.ts_ &&
                     getMillisecondsTime() - p.ts_ >=
                         INTERVAL_DELETE_OLD_PACKETS) ||
                    !p.isMcFrame())
                {
                    ROS_INFO("DROP OLD PACKET: ID[%u] GROUP[%s] SRC HOST[%s]",
                             p.id_, p.mc_group_.c_str(),
                             p.hostname_source_.c_str());
...

        {
            ...
            {
                Packet &p(*r);
                if ((getMillisecondsTime() > p.ts_ &&
                     getMillisecondsTime() - p.ts_ >=
                         INTERVAL_DELETE_OLD_PACKETS) &&
                    p.isMcFrame())
                {
                    ROS_INFO("DROP OLD UNFINISHED PACKET: ID[%u] GROUP[%s] SRC "
                             "HOST[%s]",
                             p.id_, p.mc_group_.c_str(),
                             p.hostname_source_.c_str());
...
void resendLinkFrame(stc_frame f)
{
    ...
    do
    {
        ...
        {
            boost::unique_lock<boost::mutex> lock(mtx_unack_link_frames);
            std::list<stc_frame>::iterator cr_frame_it = std::find(
                unack_link_frames_l.begin(), unack_link_frames_l.end(), f);
            frame_successfully_acknowleged =
                cr_frame_it == unack_link_frames_l.end();
        }
        ...
        if (frame_successfully_acknowleged == false)
        {
            ...
            if (f.type == FRAME_TYPE_TRANSPORT_ACK ||
                f.type == FRAME_TYPE_TRANSPORT_DATA)
            {
            ...
            //    ROS_ERROR("RESEND LINK FRAME: ID[%u] CONFIRMER MAC[%s] SOURCE
            //    HOST[%s] TYPE[%s] RELAY[%u]", f.frame_id,
            //    getMacAsStr(f.mac).c_str(), f.hostname_source.c_str(),
            //    frame_types[f.type].c_str(), f.cr);
    ...
    {
        {
            ...
            if (cr_frame_it != unack_link_frames_l.end())
            {
                ...
                ROS_WARN("DROP FRAME: ID[%u] CONFIRMER MAC[%s] SOURCE HOST[%s] "
                         "TYPE[%s]",
                         f.frame_id, getMacAsStr(f.mac).c_str(),
                         f.hostname_source.c_str(),
                         frame_types[f.type].c_str());
            }
        }
