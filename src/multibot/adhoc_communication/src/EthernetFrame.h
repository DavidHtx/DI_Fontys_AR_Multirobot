...
struct stc_frame
{
    ...
    void print_frame()
    {
        ROS_ERROR("ID:[%u] SOURCE[%s] MAC[%s] TYPE[%u] GROUP[%s]", frame_id,
                  hostname_source.c_str(), getMacAsCStr(mac), type,
                  mc_group.c_str());
    }

    stc_frame() { ts = getMillisecondsTime(); }
    ...
    bool operator==(const stc_frame &st)
    {
        ...
        if (mc_group.compare("") == 0)
            return (compareMac(st.mac, mac) &&
                    hostname_source.compare(st.hostname_source) == 0 &&
                    frame_id == st.frame_id) &&
                   type == st.type;
        else
        {

            return (hostname_source.compare(st.hostname_source) == 0 &&
                    frame_id == st.frame_id &&
                    mc_group.compare(st.mc_group) == 0);
        }
    }
};
...
class EthernetFrame
{
  public:
    ...
    struct eh_header eh_h_; ///< Struct of the layer 2 Ethernet header.
                            ///Includes: source mac, destination mac an protocol
                            ///type
    bool correct_crc_;
