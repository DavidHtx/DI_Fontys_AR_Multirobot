...
McAckFrame::McAckFrame(unsigned char *source, unsigned char *dest,
                       std::string hostname_src, std::string group_n,
                       uint32_t p_id, uint32_t seq_n)
{
...
std::string McAckFrame::getFrameAsNetworkString()
{
    ...
    /*CRC*/
    int dynamic_field_len =
        this->hostname_source_.length() + this->mc_group_.length();
    ...
    return string((const char *)buffer_start,
                  this->HEADER_FIXED_LEN + dynamic_field_len + sizeof(crc));
}

void McAckFrame::print_frame()
{
    ROS_ERROR("FRAME SEQ NUM [%u] PACKET ID[%u] SRC[%s]  MC GROUP[%s]",
              header_.frame_seq_num, header_.packet_id,
