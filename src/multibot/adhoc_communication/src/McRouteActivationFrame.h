...
class McRouteActivationFrame : public EthernetFrame
{
  public:
    /** Constructs the McMaintenanceFrame on the sender side.
            ...
            \param disconnect \todo Eventually not final konnst
     */
    McRouteActivationFrame(unsigned char *next_hop, std::string mc_g,
                           uint32_t route_id, std::string source);
    ...
    struct mc_act_header header_; ///< Struct which stores all fixed length
                                  ///header information of the
                                  ///McMaintenanceFrame
    ...
    static uint32_t
        HEADER_FIXED_LEN; ///< The length of the beacon header (34 bytes)
    static uint32_t stat_id_count;
};
uint32_t McRouteActivationFrame::HEADER_FIXED_LEN =
    sizeof(eh_header) + sizeof(mc_act_header);
