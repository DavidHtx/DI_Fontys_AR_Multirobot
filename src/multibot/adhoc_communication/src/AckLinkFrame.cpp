...
AckLinkFrame::AckLinkFrame(unsigned char source[],
                           unsigned char confirmer_mac[], unsigned char dest[],
                           uint32_t frame_id, std::string hostname,
                           uint8_t type)
{
...
{
    ROS_ERROR("FRAME ID [%u] SRC[%s] TYPE[%u]", header_.frame_id,
              hostname_source_.c_str(), header_.ack_frame_type);
    ROS_ERROR("MAC_CONF[%s] SRC MAC[%s] DST MAC[%s]",
              getMacAsStr(header_.mac_confirmer).c_str(),
              getMacAsStr(eh_h_.eh_source).c_str(),
