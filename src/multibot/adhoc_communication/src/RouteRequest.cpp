 ...
 * 6Byte
 * SOURCE MAC		// MAC of the using interface
 * 6Byte
 * ETH_TYPE_FIELD	// 0x4148 (AH as string)
 * 2Byte
 * FRAME_TYPE		// 0x52 (R as string)
 * 1Byte
 * REQUEST ID.		// Every node has a consecutive number for his requests
 * 4Byte
 * MC FLAG 1Byte
 ...
 * SOURCE HOST VAR
 * MAX HOP COUNT 4Byte
 * HOB COUNT		// is the amount of node which are involved to process the packet
 * 4Byte
 * PATH				//the list of macs from the hostname_source_ to destination			VAR
 * (HOB COUNT * 6Byte)
 * CRC 4Byte
 *
 * Every node only accept one request with the same primary key. This is so to
 ...
 * 3) N1 and N2 receive the request and insert their mac in the path list then
 * they broadcast the request forward
 * 4) N3 and N3 receive the request from N1 or N2 (depends on which node is
 * faster). They do the same like in step 2.
 * 5) The D gets the request either from N3 or from N4 and send an route
 * response
 *
 */
#include <list>
...
RouteRequest::RouteRequest(unsigned char *buffer)
{
    ...
    hostname_destination_.append((const char *)buffer,
                                 rfh->hostname_destination_len);
    ...
    {
        mac current_mac;
        memcpy((unsigned char *)current_mac.mac_adr, (unsigned char *)buffer,
               6);
    ...
    uint16_t dynamic_field_len = +rfh->hostname_source_len +
                                 rfh->hostname_destination_len +
                                 path_l_.size() * 6;
...
/*
RouteRequest::RouteRequest(std::string sourHost,std::string destHost,uint32_t
maximalHops) {
 ...
 */
RouteRequest::RouteRequest(std::string my_hostname, std::string destination,
                           uint16_t max_hops, bool is_multicast)
{
...
std::string RouteRequest::getRequestAsNetworkString(unsigned char source_mac[6])
{
    ...
    memcpy((unsigned char *)buffer,
           (unsigned char *)this->hostname_destination_.data(),
           this->hostname_destination_.length());
    ...
    int dynamic_field_len = this->hostname_source_.length() +
                            this->hostname_destination_.length() +
                            6 * path_l_.size();
    std::string crc_string = std::string(
        (const char *)buffer_start, this->HEADER_FIXED_LEN + dynamic_field_len);
    uint32_t crc = GetCrc32(crc_string);
    memcpy(buffer, &crc, sizeof(uint32_t));
    buffer += sizeof(uint32_t);

    return std::string((const char *)buffer_start, this->HEADER_FIXED_LEN +
                                                       dynamic_field_len +
                                                       sizeof(crc));
}
