...
unsigned char bcast_mac[6] = {0xFF, 0xFF, 0xFF,
                              0xFF, 0xFF, 0xFF}; /*Broadcast address - all 1's*/
...
/*Tutorial*/
#define FRAME_DATA_TYPE_QUATERNION                                             \
    0x51 // 0x51 in hex means Q in ASCII -> stands for QUATERNION

#define MAX_INTERN_JOIN_ATTEMPS                                                \
    5 // defines how often a robot will try to join a mc group
#define MAX_JOIN_ATTEMPS                                                       \
    5 // defines how often a robot will try to join a mc group
#define INTERVAL_WAIT_MC_RECONN                                                \
    200 //[ms]if a node gets a prune message, it will will a little to give the
...
#define MAX_TIME_CACHE_ROUTE_REQUEST                                           \
    15 * 1000 // [ms] Max time a route request will be cached
//#define MAX_NO_HELLO_FROM_NEIGHBOR 15       //Max times of beacon_interval
//after a node will remove neighbor
#define MAX_FRAMES_CACHED                                                      \
    200 // Max frames that are cached (to prevent multiple incoming of the same
        // frame)
#define INTERVAL_TO_RETRANSMIT_LINK_FRAMES                                     \
    30 // [ms] Interval to retransmit unacknowledged link frame
#define MAX_RELAYS 3
...
#define INTERVAL_UPTDATE_THREAD_TO_DELETE_OBSOLETE_REQUESTS                    \
    1200 // [ms] Time that the thread to delete obsolete requests will sleep,
         // after spin
#define INTERVAL_RESEND_ROUTED_FRAME                                           \
    100 //[ms] Time to re-send a unacknowledged routed frame
#define INTERVAL_RESEND_ROUTE_REQUEST                                          \
    1200 // [ms] Time to re-send a route request, if i get no response
