...
PositionSubscriber::PositionSubscriber()
{
    ...
    callback_count = 0;
    callback_refresh =
        4; // every 10 call of the subscribe f, the position will be updated.
}
...
double PositionSubscriber::calcDistance(PositionSubscriber *other)
{
    ...
    // ROS_ERROR("me [%s] %u other [%s] %u", this->robot_name_.c_str(),
    // initialized ,other->robot_name_.c_str() ,other->initialized);

    if (initialized && other->initialized)
        return sqrt(pow(x_pos_ - other->x_pos_, 2) +
                    pow(y_pos_ - other->y_pos_, 2));
    else
        return -1;
}
