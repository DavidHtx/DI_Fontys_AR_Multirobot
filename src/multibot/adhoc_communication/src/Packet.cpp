...
void Packet::refreshLists()
{
    ...
    list<RoutedFrame> ordered_frames;
    /* Generates a new ordered frame list, in which every seq num is only
     * included once*/
    for (uint32_t i = 0; i < this->size_; i++)
    {
    ...
    // ROS_ERROR("list refreshed %u %u %u", this->size_,
    // missed_sequences.size(),
    //	frames_l_.size());
}

inline bool Packet::isMcFrame() { return mc_group_.compare("") != 0; }
...
std::string Packet::getPayload()
{
    ...
    if (frames_l_.size() >= size_)
    {
             ...
             it != frames_l_.end(); ++it)
        {
            ...
            if (frame.header_.packet_sequence_num ==
                expected_sequence++) // doublecheck if frame is in the right
                                     // order
                payload.append(frame.payload_);
            else
            {
                ROS_ERROR("PACKET SEG FAULT! %u %u",
                          frame.header_.packet_sequence_num, expected_sequence);
                return std::string("");
            }
        }
    }
