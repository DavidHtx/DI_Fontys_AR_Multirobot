...
void deliverMcAckFrame(stc_RoutedFrame &stc_rf, routing_entry &r,
                       bool send_ack);
geometry_msgs::PoseStamped
getPoseStampFromNetworkString(unsigned char *serializedPoseStamp,
                              uint32_t length);
...
stc_RoutedFrame *unack_routed_frame; // there can be just one unacknowledged
                                     // routed frame at a time, because the
                                     // protocol waits for the ack until the
                                     // next routed frame will be send
list<route_request> route_requests_l;
list<routing_entry> routing_table_l;

std::map<unsigned char *, string> mac_hname_dict;
list<hostname_mac> neighbors_l;    // list with all reachable direct neighbors.
                                   // This list is not used for the routing
                                   // process. Nodes detect each other by "hello
                                   // frames"
...
RoutedFrame frames_received_recently_a[MAX_FRAMES_CACHED]; // static array to
                                                           // cache frames
                                                           // incoming frames.
                                                           // This is needed,
                                                           // because frames are
...
uint16_t frr_count = 0; // count to know how many frames are cached. This is
                        // only important at the beginning, later frr_count will
                        // always be MAX_FRAMES_CACHED
...
void resendRequestedFrame(uint32_t *seq_num, uint32_t *packet_id,
                          string *source_h, string *mc_group);
...
bool getRoute(routing_entry &r, std::string hostname)
{
     ...
     * The parameter of the type routing_entry will be initialized, if a route
     * can be found.
     *
     * Returns:
    ...
    for (std::list<routing_entry>::iterator it = routing_table_l.begin();
         it != routing_table_l.end(); ++it)
    {
...
void socketSend(string network_string)
{
    sendto(raw_socket, network_string.data(), network_string.length(), 0,
...
void resendUnackLinkFrame(string hostname_src, uint32_t id, unsigned char *mac,
                          string network_string, uint8_t type)
{
...
void cleanRoutingTable()
{
    ...
    for (list<routing_entry>::iterator i = routing_table_l.begin();
         i != routing_table_l.end();)
    {
...
void processMcDisconnectUplink(McDisconnectFrame *f)
{
    ...
    {
        ROS_ERROR("Unexpected Error: %s is not a downlink for group %s",
    ...
    {
        groups_lock.unlock();
        // ROS_ERROR("GOT DISCONNECT MSG FROM DOWNLINK: GROUP[%s] DOWNLINK[%s]",
        // f->mc_group_.c_str(),
        // getHostnameFromMac(f->eh_h_.eh_source).c_str());
        groups_lock.lock();
    }
    else
    {
...
void processMcDisconnectDownlink(McDisconnectFrame *f)
{
    ...
    // ROS_ERROR("GOT MC PRUNE MESSAGE GROUP[%s] HOST[%s]",
    // f->mc_group_.c_str(), getHostnameFromMac(f->eh_h_.eh_source).c_str());

    if (!group->downlinks_l_.empty())
    {
        //  ROS_ERROR("FORWARD MC PRUNE GROUP[%s] HOST[%s]",
        //  group->group_name_.c_str(),
        //  getHostnameFromMac(f->eh_h_.eh_source).c_str());
...
void testPacket()
{
    ...
    {
        RoutedFrame f("topic", "random_data", 2, packet_id, packet_seq,
                      packet_size);
        l.push_back(f);
    ...
    ROS_ERROR("missings seq: %s %u %zu %zu",
              getListAsString(p.missed_sequences_l_).c_str(), p.size_,
              p.frames_l_.size(), p.missed_sequences_l_.size());
...
bool gotFrameRecently(RoutedFrame rf)
{
    ...
    int interations = 0;
    //	ROS_DEBUG("COMPARE FRAME SEQ NUM[%u] PACKET ID[%u] SOURCE[%s]
    //",rf.header_.packet_sequence_num,rf.header_.packet_id,rf.hostname_source_.c_str());
    /*The first index to search will be the last of insert this makes search
     * faster, because it happens often that a frame will be received twice from
     * the network interface*/
    for (int i = frr_index > 0 ? frr_index - 1 : frr_count - 1;
         interations < frr_count;)
    {

        //	ROS_DEBUG("IT SEQ[%u] PACKET ID[%u] SOURCE[%s]
        //",frames_received_recently_a[i].header_.packet_sequence_num,frames_received_recently_a[i].header_.packet_id,frames_received_recently_a[i].hostname_source_.c_str());
        /* UNICAST */
        if (!rf.mc_flag && !rf.resend_flag &&
            frames_received_recently_a[i].header_.packet_sequence_num ==
                rf.header_.packet_sequence_num &&
            frames_received_recently_a[i].header_.packet_id ==
                rf.header_.packet_id &&
            frames_received_recently_a[i].hostname_source_.compare(
                rf.hostname_source_) == 0) // &&
        {
            //	ROS_DEBUG("GOT FRAME RECENTLY: SEQ NUM[%u] PACKET ID[%u]
            //SOURCE[%s]
            //",rf.header_.packet_sequence_num,rf.header_.packet_id,rf.hostname_source_.c_str());
            return true;
        }
        ...
        if (rf.mc_flag &&
            ...
            frames_received_recently_a[i].hostname_source_.compare(
                rf.hostname_source_) == 0 &&
            frames_received_recently_a[i].mc_g_name_.compare(rf.mc_g_name_) ==
                0)
        {
            // ROS_DEBUG("GOT MC FRAME RECENTLY: SEQ NUM[%u] PACKET ID[%u]
            // SOURCE[%s]
            // ",rf.header_.packet_sequence_num,rf.header_.packet_id,rf.hostname_source_.c_str());
            return true;
        }
...
void safeFrame(RoutedFrame incomingFrame)
{
    ...
    boost::unique_lock<boost::mutex> lock(mtx_frames_received_recently);
    //	ROS_DEBUG("safe frame %u
    //%s",incomingFrame.frame_id,incomingFrame.hostname_source.c_str());

    frames_received_recently_a[frr_index].header_.packet_sequence_num =
        incomingFrame.header_.packet_sequence_num;
    frames_received_recently_a[frr_index].header_.packet_id =
        incomingFrame.header_.packet_id;
    frames_received_recently_a[frr_index].header_.frame_id =
        incomingFrame.header_.frame_id;
    frames_received_recently_a[frr_index].hostname_source_ =
        incomingFrame.hostname_source_;
    frames_received_recently_a[frr_index].mc_g_name_ = incomingFrame.mc_g_name_;
    memcpy(frames_received_recently_a[frr_index++].header_.mac_destination_,
           incomingFrame.header_.mac_destination_, 6);
...
std::string getHostnameFromMac(unsigned char mac[6])
{
     ...
     *
     * NOTE: This function is not used in this version.. but in earlier
     * implementations.
     */
...
void initParams(ros::NodeHandle *n)
{
    ...
    n->param("topic_remove_robot", topic_remove_robot,
             std::string("remove_robot"));
    ...
    n->param(
        "sim_robot_macs", sim_robot_macs,
        std::string(
            "")); // exp: "robot_0,00:11:00:00:00:00!robot_0,00:11:00:00:00:00
    ...
    //  ROS_ERROR("parameters int: %s mac: %s real mac
    //  %s",interface_as_string.c_str(),mac_as_string.c_str(),
    //  getMacAsCStr(src_mac));
    ...
    {
        std::string command =
            "iwconfig " + std::string((const char *)interface) + " txpower ";
...
bool isReachable(unsigned char mac[6])
{
    if (simulation_mode)
    {
        ...
        for (auto it = robot_positions_l.begin(); it != robot_positions_l.end();
             ++it)
        {
            if (other_robot.robot_name_.compare((*it)->robot_name_) == 0)
            {
                ...
                if (p_thres <= p_rx)
                {
                    // ROS_DEBUG("connected with %s %f
                    // %f",other_robot->robot_name_.c_str(),d,p_rx);
                    return true;
                }
                else
                {
    ...
    else if (sim_robot_macs.compare("") != 0)
    {
        ...
        hostname_mac n(mac);
        auto searchNeighbor =
            std::find(neighbors_l.begin(), neighbors_l.end(), n);
...
bool connectedWith(unsigned char mac[6])
{
    ...
    boost::unique_lock<boost::mutex> lock(mtx_neighbors);
    std::list<hostname_mac>::iterator searchNeighbor =
        std::find(neighbors_l.begin(), neighbors_l.end(), n);

    // ROS_DEBUG("reachable %u %s", ((hostname_mac) * searchNeighbor).reachable,
    // getMacAsCStr((unsigned char*) mac));

    return searchNeighbor != neighbors_l.end() &&
           ((hostname_mac)*searchNeighbor).reachable;
}
...
void disconnectDownlinks(McTree *t)
{
    ...
    for (list<mac *>::iterator i = t->downlinks_l_.begin();
         i != t->downlinks_l_.end(); i++)
    {
        resendUnackLinkFrame("", disc_f.header_.id, (*i)->mac_adr,
...
void mcLostConnection(hostname_mac host)
{
    ...
    while (!trees.empty())
    {
        ...
        ROS_ERROR("REMOVE DOWNLINK: GROUP[%s]  HOST[%s]",
                  t->group_name_.c_str(), host.hostname.c_str());
        trees.pop_back();
    ...
    while (!trees.empty())
    {
        ...
        // ROS_ERROR("LOST CONNECTION TO UPLINK GROUP[%s] HOST[%s]",
        // t->group_name_.c_str(), host.hostname.c_str());
        ...
        if (rebuild_mc_tree)
        {
            ...
            {
                ROS_INFO("SEND PRUNE MESSAGE TO TREE LEAFES GROUP[%s]",
                         t->group_name_.c_str());
                disconnectDownlinks(t);
            }
            else
            {
                ROS_INFO("NO OTHER DOWNLINKS: GROUP[%s]",
                         t->group_name_.c_str());
            }
            ...
            if (t->member)
            {
                ...
                t->connected = false;
                boost::thread tryToJoin(&reconnectToMcGroup,
                                        std::string(t->group_name_));
            }
            else
            {
...
void cacheNackMcFrame(RoutedFrame rf)
{
    ...
    for (std::list<Packet>::iterator it_p = cached_mc_packets_l.begin();
         it_p != cached_mc_packets_l.end(); ++it_p)
    {
        Packet &p(*it_p);
        ...
        {
            packet_exsists = true;
            if (rf.hostname_source_.compare(p.hostname_source_) == 0 &&
                rf.mc_g_name_.compare(p.mc_group_) == 0)
            {
                bool frame_exsists = false;
                for (std::list<RoutedFrame>::iterator it_f =
                         p.frames_l_.begin();
                     it_f != p.frames_l_.end(); ++it_f)
                {
                    RoutedFrame cur_f = *it_f;
                    if (cur_f.header_.packet_sequence_num ==
                        rf.header_.packet_sequence_num)
                    {
                        frame_exsists = true;
                        break;
...
void resendRequestedFrameFromPacket(McNackFrame nack)
{
    ...
    {
        /* packet found*/
        if ((*it_p).id_ == nack.header_.packet_id &&
            nack.hostname_source_.compare((*it_p).hostname_source_) == 0 &&
            nack.mc_group_.compare((*it_p).mc_group_) == 0)
        {
            bool one_frame_resent = false;

            std::vector<RoutedFrame> frames_in_packet((*it_p).frames_l_.begin(),
                                                      (*it_p).frames_l_.end());
            lock.unlock();

            // ROS_DEBUG("pid %u size %u",p.id_,p.frames_l_.size());
            for (std::vector<RoutedFrame>::iterator it_f =
                     frames_in_packet.begin();
                 it_f != frames_in_packet.end(); ++it_f)
            {
                RoutedFrame f = *it_f;

                list<uint32_t> l(nack.req_seq_nums_.begin(),
                                 nack.req_seq_nums_.end());

                //    ROS_ERROR("%s", getListAsString(l).c_str());

                if (std::find(nack.req_seq_nums_.begin(),
                              nack.req_seq_nums_.end(),
                    ...
                    nack.req_seq_nums_.end())
                {
                    ...
                    string network_string =
                        f.getFrameAsNetworkString(f.header_.route_id, bcast_mac,
                                                  f.hostname_source_, src_mac);
                    //    ROS_ERROR("RESEND REQUESTED FRAME: SEQ NUM[%u] PACKET
                    //    ID[%u] GROUP[%s] SOURCE[%s]",
                    //    f.header_.packet_sequence_num, f.header_.packet_id,
                    //    f.mc_g_name_.c_str(), f.hostname_source_.c_str());
                    socketSend(network_string);
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
    ...
    // ROS_ERROR("COULD NOT FIND REQ PACKET: PACKET ID[%u] GROUP[%s]
    // SOURCE[%s]", nack.header_.packet_id, nack.mc_group_.c_str(),
    // nack.hostname_source_.c_str());
}
    ...
    while (ros::ok())
    {
        ...
        if (frames.size() > 0)
        {
            ...
            while (!frames.empty())
            {
                ...
                // resendRequestedFrame(&f.header_.frame_seq_num,
                // &f.header_.packet_id, &f.hostname_source_, &f.mc_group_);
                it = frames.erase(it);
                // ros::Duration(0, INTERVAL_BETWEEN_RESEND_REQUESTED_FRAMES *
                // 1000).sleep();
            }
        }
        else
        {
            ros::Duration(0, INTERVAL_BETWEEN_RESEND_REQUESTED_FRAMES * 1000)
                .sleep();
        }
    }
}
...
void reconnectToMcGroup(std::string group_name)
{
    if (rebuild_mc_tree)
    {
        // if a node gets a prune message, it will will a little to give the
        // network time for synconization
    ...
    {
        {
            ...
            if (t != NULL && t->activated &&
                (!t->downlinks_l_.empty() || t->member))
            {
                if (!t->connected)
                {
                    ROS_INFO("TRY RECONNECTION TO MC GROUP[%s]",
                             group_name.c_str());
                    reconnect_tries++;
    ...
    if (reconnect_tries <= MAX_JOIN_ATTEMPS)
    {
        ...
        ROS_ERROR("STOP TRYING TO CONNECT TO MC GROUP: NAME[%s]",
                  group_name.c_str());
        routing_entry route;
...
struct frame_packet_info
{
    ...
    {
        if (seq_num == st.seq_num && packet_id == st.packet_id &&
            group_name.compare(st.group_name) == 0 && source.compare(st.source))
            return true;
...
std::vector<McNackFrame> getMissingFrames(Packet *p)
{
    ...
    for (std::list<uint32_t>::iterator i = p->missed_sequences_l_.begin();
         i != p->missed_sequences_l_.end(); i++)
    {
        list_4_nack.push_back(*i);

        if (list_4_nack.size() % MAX_REQUESTED_FRAMES_IN_NACK == 0)
        {
            McNackFrame nack(src_mac, bcast_mac, p->hostname_source_,
                             p->mc_group_, p->id_, list_4_nack);
            l.push_back(nack);
            list_4_nack.clear();
        }
    }
    ...
    {
        McNackFrame nack(src_mac, bcast_mac, p->hostname_source_, p->mc_group_,
                         p->id_, list_4_nack);
        l.push_back(nack);
    }
...
{
    for (std::list<Packet>::iterator p = packets->begin(); p != packets->end();
         p++)
    {
...
void updateTsRequestFrames(Packet p)
{
    ...
    for (std::list<Packet>::iterator pi = packets_incomplete_l.begin();
         pi != packets_incomplete_l.end(); pi++)
    {
        if ((*pi).id_ == p.id_ &&
            (*pi).hostname_source_.compare(p.hostname_source_) == 0 &&
            (*pi).mc_group_.compare(p.mc_group_) == 0)
        {
...
void requestPendingFrames()
{
    ...
    while (ros::ok())
    {
        ...
        for (std::list<Packet>::iterator p = pack_inco_copy.begin();
             p != pack_inco_copy.end(); p++)
        {
            if (!(*p).isNack() || (*p).size_ <= (*p).frames_l_.size())
                continue;

            float perc_frames_got =
                (float)(*p).frames_l_.size() / (float)(*p).size_;
            // ROS_DEBUG("%f %u",perc_frames_got,p.id_);
            // if ((perc_frames_got >= START_RQUESTING_PACKET_FILL_LEVEL ||
            // getMillisecondsTime() - (*p).ts_ >=
            // START_RQUESTING_INTERVAL_AFTER_LAST_FRAME_GOT) ||
            // !(*p).missed_sequences_l_.empty())

            if (getMillisecondsTime() - (*p).ts_ >=
                    START_RQUESTING_INTERVAL_AFTER_LAST_FRAME_GOT ||
                (!(*p).missed_sequences_l_.empty() &&
                 getMillisecondsTime() - (*p).ts_last_frame_request >= 200))
            {
                ...
                while (!nacks.empty())
                {
                    ...
                    //   ROS_ERROR("%u %u %u ", (*p).size_,
                    //   (*p).frames_l_.size(),
                    //   (*p).missed_sequences_l_.size());

                    //  ROS_ERROR("REQUEST FRAMES: PACKET ID[%u] SEQ[%s]
                    //  GROUP[%s] SOURCE[%s]", (*p).id_,
                    //  getListAsString(nacks.back().req_seq_nums_).c_str(),
                    //  (*p).mc_group_.c_str(), (*p).hostname_source_.c_str());

                    string network_string =
                        nacks.back().getFrameAsNetworkString();
                    //    ROS_ERROR("%u %u %u %u", network_string.length(),
                    //    nacks.back().req_seq_nums_.size(), nacks.size(),
                    //    (*p).missed_sequences_l_.size());
                    nacks.pop_back();
                    socketSend(network_string);
#ifdef PERFORMANCE_LOGGING_MC_LINK_SUMMARY
...
void initRobotMacList(std::string *robot_mac)
{
    ...
    for (std::vector<std::string>::iterator i = hostnames_macs.begin();
         i != hostnames_macs.end(); i++)
    {
        std::string &h(*i);
        if (h.compare("") == 0)
            continue;
        ...
        try
        {
            ...
            ROS_ERROR("ADD TO WHITELIST: %s [%s]", hm.hostname.c_str(),
                      getMacAsStr(hm.mac).c_str());
        }
        catch (const std::out_of_range &oor)
        {
            ROS_ERROR("SYNTAX ERROR IN PARAMETER 'sim_robot_macs' %s ",
                      robot_mac->c_str());
        }
    }
}
                     ...
                     t *obj)
{
    ...
    catch (ros::serialization::StreamOverrunException e)
    {
        ...
        ROS_ERROR("PARAMETERS: length=[%u] object type=[%s]", length,
                  typeid(*obj).name());
    }
}
