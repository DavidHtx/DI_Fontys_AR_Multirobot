...
McRouteActivationFrame::McRouteActivationFrame(unsigned char *buffer)
{
    ...
    uint32_t dynamic_field_len =
        rfh->mc_group_name_len + rfh->hostname_source_len;
...
std::string
McRouteActivationFrame::getFrameAsNetworkString(unsigned char source_mac[6])
{
    ...
    int dynamic_field_len =
        this->mc_group_.length() + this->hostname_source_.length();
