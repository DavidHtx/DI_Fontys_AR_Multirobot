...
struct routing_entry
{
    ...
    routing_entry() { ts = getMillisecondsTime(); }
...
struct route_request
{
    ...
    uint32_t hop_limit;
    uint16_t counter; // defines how much requests have been received with the
                      // same id
    ...
    {
        if (hostname_source.compare(st.hostname_source) == 0 &&
            id == st.id) // && compareMac((const char*)st.receiver_mac,(const
                         // char*) this->receiver_mac))
            return true;
...
struct stc_RoutedFrame
{
    ...
    {
        return (frame.header_.frame_id == st.frame.header_.frame_id &&
                hostname_destination.compare(st.hostname_destination) == 0);
    }
};
...
struct hostname_mac
{
    ...
    hostname_mac() { ts = getMillisecondsTime(); }
    ...
    void stamp() { ts = getMillisecondsTime(); }
...
struct stc_packet
{
    ...
    {
        return p.src.compare(src) == 0 && p.id == id &&
               mc_group.compare(p.mc_group) == 0;
    }
};
...
struct ack_cr_info
{
    ...
    bool operator==(const ack_cr_info &p)
    {
        ...
        return compareMac(frame_src_mac, p.frame_src_mac) &&
               compareMac(frame_dst_mac, p.frame_dst_mac) && id == p.id &&
               seq == p.seq && mc_group.compare(p.mc_group) == 0 &&
               source_host.compare(p.source_host) == 0 &&
