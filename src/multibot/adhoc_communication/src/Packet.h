...
The packet class implements all functions required to build a network layer
packet.
*/
class Packet
{
  public:
    ...
    */
    Packet(RoutedFrame frame); //(uint32_t packet_id,uint32_t packet_size,
                               //uint8_t payload_data_type,std::string
                               //source,std::string topic, string mc_group);
    Packet();
    virtual ~Packet();
     ...
     * This method also checks if all frames are received and the packet is
     complete.
       \param frame The frame that should be added
       \return True, if packet is complete and ready to publish. False, in every
     other case
    ...
    */
    ...
    std::list<uint32_t>
        missed_sequences_l_; ///< A list with all lost pending frames
    // ecl::TimeStamp time_stamp_;		///< Time stamp when \todo What does the
    // time stamp mean?
    unsigned long ts_;
