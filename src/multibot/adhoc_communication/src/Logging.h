...
class Logging
{
  ...
  public:
    ...
    static void logServiceCalls(string service_name, unsigned long time_call,
    ...
    static void logMemoryConsumptionPackets(
        unsigned long interval_ms, boost::mutex *mtx_pack,
        list<Packet> *packets, boost::mutex *mtx_cached_mc,
        list<Packet> *cached_mc, boost::mutex *mtx_unack_link_frames,
        list<stc_frame> *unack_link_frames, boost::mutex *mtx_unack_cr_frames,
        list<ack_cr_info> *unack_cr_frames);
    static void logRoutingTable(std::list<routing_entry> *u_entries,
    ...
    static void logTransportFrame(stc_RoutedFrame *f, routing_entry *route,
                                  unsigned long start_time,
                                  unsigned long end_time, bool success);
    static void logUcLinkTransmission(stc_frame f);
    static void logRRequestReceiver(std::string h_src, uint32_t id,
    ...
    static void createLogFile(std::list<std::string> *table,
                              std::string *filename);
    ...
    //  static uint32_t running_unicast_cr_threads, running_multicast_threads,
    //  running_unicast_transport_threads, running_unicast_link_threads; //
    //  running threads
    static boost::mutex mtx_logging;

    static std::map<std::string, uint32_t> uint32_properties;

    static std::list<std::string> entries_r_table, entries_rreq_initi,
        entries_rreq_recv, entries_rreq_interm, entries_mem_consumption,
        entries_uc_frames, entries_mc_frames, entries_link_frames,
        entries_service_calls, entries_transport_frames;
    static std::string log_path, routing_table_file, route_req_src_file,
        route_req_dst_file, route_req_iterm_file, mem_consumption_file,
        uc_frames_file, mc_frames_file, link_frames_file, sercice_calls_file,
        transport_frames_file;
...
std::string Logging::uc_frames_file =
    "unicast_datalink_transmission_summary.csv";
