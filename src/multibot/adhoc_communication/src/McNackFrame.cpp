...
McNackFrame::McNackFrame(unsigned char *source, unsigned char *dest,
                         std::string hostname_src, std::string group_n,
                         uint32_t p_id, std::vector<uint32_t> seq_n)
{
...
McNackFrame::McNackFrame(unsigned char *buffer)
{
    ...
    crc_data_string.append((const char *)buffer_start,
                           this->HEADER_FIXED_LEN + rfh->hostname_source_len +
                               rfh->mc_group_len +
                               rfh->frame_seq_num_size * sizeof(uint32_t));
...
std::string McNackFrame::getFrameAsNetworkString()
{
    ...
    /*req_seq_nums_*/
    for (std::vector<uint32_t>::iterator i = req_seq_nums_.begin();
         i != req_seq_nums_.end(); i++)
    {
    ...
    int dynamic_field_len = this->hostname_source_.length() +
                            ...
                            req_seq_nums_.size() * sizeof(uint32_t);
