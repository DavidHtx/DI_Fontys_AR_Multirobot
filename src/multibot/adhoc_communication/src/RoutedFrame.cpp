 ...
 *
 * Destination MAC 	// ff:ff:ff:ff:ff:ff or "real" destination. Depends on CR
 *6Byte
 * SOURCE MAC		// MAC of the using interface
 *6Byte
 * ETH_TYPE_FIELD	// 0x4148 (AH as string)
 *2Byte
 * FRAME_TYPE		// 0x64 (d as string)
 *1Byte
 ...
 * MC GROUP NAME VAR
 * TOPIC_LENGTH 4Byte
 * TOPIC			// topic that the payload will be published later
 *VAR
 * PAYLOAD TYPE     // this field is used to distinguish different payload types
 *1Byte
 ...
 *	t(7)		 ack->
 *
 *	To send a packet, of course its necessary that there is a route for the
 *destination in the routing table.
 *
 *	1) Source send the frame to N1
 *	2) N1 receive the frame and sends a ack to the S. N1 looks in his routing
 *table wich is the next host mac and send it to the D
...
RoutedFrame::RoutedFrame(unsigned char *buffer)
{
    ...
    crc_data_string.append((const char *)buffer_start,
                           this->HEADER_FIXED_LEN + rfh->hostname_source_len +
                               rfh->mc_group_len + rfh->topic_len +
                               rfh->payload_len);
                                                 ...
                                                 unsigned char source[6])
{
    ...
    /*CRC*/
    uint32_t dynamic_field_len = this->hostname_source_.length() +
                                 this->mc_g_name_.length() + topic_.length() +
                                 payload_.length();
    ...
    return string((const char *)buffer_start, this->HEADER_FIXED_LEN +
                                                  dynamic_field_len +
                                                  sizeof(uint32_t));
}

std::string RoutedFrame::getFrameAsNetworkString(routing_entry r,
                                                 unsigned char src[6])
{
    return this->getFrameAsNetworkString(r.id, r.next_hop, r.hostname_source,
                                         src);
}
