 ...
 * DESTINATION MAC	// the source mac of the frame to acknowledge	| or bcast mac
 * 6Byte
 * FRAME ID			// ID of the frame to acknowledge | or sequence num 4Byte
 * PACKET ID 		//only for neg ack (otherwise highest possible value of the type
 * 4Byte
 * FLAG FIELD           / 1Byte
 ...
 *
 * This frame-type is to acknowledge incoming frames. Every time a i get a frame
 * i should immediately send an acknowledgment, except route requests, route
...
{
  public:
    AckLinkFrame(unsigned char *source, unsigned char *confirmer_mac,
                 unsigned char *dest, uint32_t frame_id, std::string hostname,
                 uint8_t type);
...
uint32_t AckLinkFrame::HEADER_FIXED_LEN =
    sizeof(eh_header) + sizeof(ack_lf_header); // 41;
