...
AckRoutedFrame::AckRoutedFrame(RoutedFrame rf)
{
    ...
    if (mc_flag)
    {
        ...
        /* If its a multicast frame the fields packet_id and route_id will be
         * used to store the packet_id and the packet_squence_num*/
...
std::string AckRoutedFrame::getFrameAsNetworkString(routing_entry r,
                                                    unsigned char src[6])
{
    return this->getFrameAsNetworkString(r.id, r.previous_hop,
                                         r.hostname_source, src);
}
