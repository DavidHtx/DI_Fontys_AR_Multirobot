...
#define OPERATE_WITH_GOAL_BACKOFF true  // navigate to a goal point which is close to (but not exactly at)
                                        // selected goal (in case selected goal is too close to a wall)
...
class Explorer
{
  ...
  public:
    Explorer(tf::TransformListener &tf)
        : counter(0), rotation_counter(0), nh("~"), exploration_finished(false),
          number_of_robots(1), accessing_cluster(0), cluster_element_size(0),
          cluster_flag(false), cluster_element(-1),
          cluster_initialize_flag(false), global_iterattions(0),
          global_iterations_counter(0), counter_waiting_for_clusters(0),
          global_costmap_iteration(0), robot_prefix_empty(false), robot_id(0)
    {
        frontier_selection = nh.param("frontier_selection", 1);
        costmap_width = nh.param("local_costmap/width", 0);
        costmap_resolution = nh.param<double>("local_costmap/resolution", 0);
        number_unreachable_frontiers_for_cluster = nh.param("number_unreachable_for_cluster", 3);
        mc_group = nh.param<std::string>("mc_group", "mc_default");
        ...
        move_base_frame = nh.param<std::string>("move_base_frame", "map");
        waitForResult = nh.param<int>("wait_for_planner_result", 3);
        // determine host name
        robot_prefix = nh.param<std::string>("robot_prefix", "");
        ...
        if (robot_prefix.empty())
        {
            ...
            // gethostname(hostname_c, 1023);
            // robot_name = std::string(hostname_c);
            robot_name = nh.param<std::string>("robot_name", "default");
            ...
            else if (robot_name.compare("joy") == 0)
            {
                robot_id = 1;
            }
            else if (robot_name.compare("marley") == 0)
            {
                robot_id = 2;
            }
            else if (robot_name.compare("bob") == 0)
            {
                robot_id = 3;
            }
            else if (robot_name.compare("hans") == 0)
            {
                robot_id = 4;
            }
        ...
        else
        {
            ...
            ROS_INFO("Starting Global costmap ...");
            costmap2d_global->start();
            ROS_INFO("Global costmap sterted. Starting Local costmap ... ");
         ...
         * RVIZ requires a minimal history length of 1, which means that at
         * least
        ...
        exploration = new explorationPlanner::ExplorationPlanner(
            robot_id, robot_prefix_empty, robot_name, mc_group);
        ...
        exploration->next_auction_position_x = (int) robotPose.getOrigin().getX();
        exploration->next_auction_position_y = (int) robotPose.getOrigin().getY();

        exploration->storeVisitedFrontier(robot_home_position_x,
                                          robot_home_position_y,
                                          robot_id,
                                          robot_name,
                                          -1);

        exploration->storeFrontier(robot_home_position_x,
                                   robot_home_position_y,
                                   robot_id,
                                   robot_name,
                                   -1);

        exploration->setRobotConfig(robot_id,
    ...
    void explore()
    {
        ...
        // TODO: investigate(old :'should be a parameter!! only for testing on Wed/17/9/14')
        ...
        while (!exploration_finished)
        {
            ...
            if (!Simulation)
            {
                ...
                /*
                 * Use mutex to lock the critical section (access to the
                 * costmap) since rosspin tries to update the costmap continuously
                 */
                costmap_mutex.lock();
                 ...
                 * 2 ... Sort the buffer from nearest to furthest frontier,
                 * normalized to the
                 *       robots actual position (EUCLIDEAN DISTANCE)
                 * 3 ... Sort the last 10 entries to shortest TRAVEL PATH
                 * 4 ... Sort all cluster elements from nearest to furthest
                 * (EUCLIDEAN DISTANCE)
                 */
                if (frontier_selection < 0 || frontier_selection > 6)
                {
                    ROS_FATAL("You selected an invalid exploration strategy. "
                              "Please make sure to set it in the interval "
                              "[0,6]. The current value is %d.",
                              frontier_selection);
                }
                if (frontier_selection == 2)
                {
                     ...
                     * the frontiers list(which contain every found frontier
                     * with sufficient
                     * spacing in between) and returns the most attractive one,
                     * based on the
                    ...
                    while (true)
                    {
                        goal_determined = exploration->determine_goal(
                            1, &final_goal, count, 0, &robot_str);
                        if (!goal_determined)
                        {
                            break;
                        }
                ...
                else if (frontier_selection == 3)
                {
                    ...
                    {
                        goal_determined = exploration->determine_goal(
                            2, &final_goal, count, 0, &robot_str);
                        if (!goal_determined)
                        {
                            break;
                        }
                ...
                else if (frontier_selection == 4)
                {
                    ...
                    {
                        goal_determined = exploration->determine_goal(
                            3, &final_goal, count, 0, &robot_str);
                        ROS_DEBUG("Goal_determined: %d   counter: %d", goal_determined, count);
                        if (!goal_determined)
                        {
                            break;
                ...
                else if (frontier_selection == 5)
                {
                    ...
                    while (true)
                    {
                        final_goal.clear();
                        goal_determined = exploration->determine_goal(
                            4, &final_goal, count, cluster_element, &robot_str);
                        ...
                        else
                        {
                            if (!cluster_initialize_flag)
                            {
                                /*
                                 * TODO investigate:
                                 * '... just to reduce the selection of the same
                             ...
                             */
                            negotiation = exploration->negotiate_Frontier(
                                final_goal.at(0),
                                final_goal.at(1),
                                final_goal.at(2),
                                final_goal.at(3),
                            ...
                            else
                            {
                                ...
                                ROS_WARN("Negotiation was not successful, try "
                                         "next cluster");
                            }
                            count++;
                            /*
                             * In order to make one robot wait until the other finds
                             * another cluster to operate in, one have to fill
                             * the clusters_available_in_pool vector if count is
                             * increased at least once which determines that at
                             * least one cluster
                             */
                    ...
                    while (true)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                ROS_INFO("Random cluster_element: %d from available %zu clusters",
                                         cluster_element,
                ...
                else if (frontier_selection == 1)
                {
                    ...
                    while (true)
                    {
                        ...
                        robot_str.clear();
                        goal_determined = exploration->determine_goal(
                            5, &final_goal, 0, cluster_element, &robot_str);
                        ...
                        ROS_DEBUG("Cluster vector position: %d",
                                  cluster_vector_position);

                        if (cluster_vector_position >= 0)
                        {
                            if (exploration->clusters
                                    .at(cluster_vector_position)
                                    .unreachable_frontier_count >=
                                number_unreachable_frontiers_for_cluster)
                            {
                                goal_determined = false;
                                ROS_WARN("Cluster inoperable");
                            }
                            else
                            {
                                ROS_WARN("Cluster operable");
                            }
                        }
                        ...
                        if (!goal_determined)
                        {
                            ...
                            ROS_INFO("No goal was determined, cluster is "
                                     "empty. Bid for another one");

                            final_goal.clear();
                            robot_str.clear();
                            clusters_available_in_pool.clear();

                            bool auctioning = exploration->auctioning(
                                &final_goal,
                            ...
                            else
                            {
                                ...
                                {
                                    ROS_INFO("No cluster was selected but "
                                             "other robots are operating ... "
                                             "waiting for new clusters");
                                    counter_waiting_for_clusters++;
                                    break;
                                }
                                else
                                {
                                    /*
                                     * If NO goals are selected at all, iterate
                                     * over the global
                        ...
                        {
                            ROS_INFO("Still some goals in current cluster, "
                ...
                if (goal_determined)
                {
                    if (OPERATE_WITH_GOAL_BACKOFF)
                    {
                        ROS_DEBUG("Doing smartGoalBackoff");
                        backoff_sucessfull = exploration->smartGoalBackoff(
                ...
                else if (!backoff_sucessfull && !goal_determined)
                {
                    navigate_to_goal = navigate(final_goal);
                    goal_determined = false;
                }
                if (navigate_to_goal && goal_determined)
                {
                    exploration->calculate_travel_path(
                        exploration->visited_frontiers
                            .at(exploration->visited_frontiers.size() - 1)
                            .x_coordinate,
                        exploration->visited_frontiers
                            .at(exploration->visited_frontiers.size() - 1)
                            .y_coordinate);
                    ...
                    exploration->storeVisitedFrontier(
                        final_goal.at(0),
                        final_goal.at(1),
                        final_goal.at(2),
                        robot_str.at(0), // TODO: investigate
                        final_goal.at(3));
                    ROS_DEBUG("Stored Visited frontier");
                }
                else if (!navigate_to_goal && goal_determined)
                {
                    ROS_DEBUG("Storing unreachable...");
                    exploration->storeUnreachableFrontier(
                        final_goal.at(0),
                        final_goal.at(1),
                        final_goal.at(2),
                        robot_str.at(0), // TODO: investigate
    ...
    void map_info()
    {
        fs_csv.open(csv_file.c_str(),
                    std::fstream::in | std::fstream::app | std::fstream::out);
        fs_csv << "#time,exploration_travel_path_global,exploration_travel_"
                  "path_average,global_map_progress,local_map_progress,number_"
                  "of_completed_auctions, number_of_uncompleted_auctions, "
                  "frontier_selection_strategy, costmap_size, "
        ...
        while (ros::ok() && !exploration_finished)
        {
            ...
            double exploration_travel_path_global = exploration->exploration_travel_path_global * 0.02;
            ...
            //  ROS_INFO("global map size: %f   at time: %f",
            //  map_progress.global_freespace, map_progress.time);
            //  ROS_INFO("local map size : %f   at time: %f",
            //  map_progress.local_freespace, map_progress.time);
            //  ROS_INFO("travel path glo: %d   at time: %f",
            //  exploration_travel_path_global,
            //  map_progress.time);
            //  ROS_INFO("travel path ave: %d   at time: %f",
            //  exploration_travel_path_average,
            //  map_progress.time);
            ...
            fs_csv << map_progress.time << "," << exploration_travel_path_global
                   ...
                   << frontier_selection << "," << costmap_width << ","
                   << exploration->unreachable_frontiers.size() << std::endl;
            //  fs_csv << "travel_path_global   = " <<
            //  exploration_travel_path_global << std::endl;
            //  fs_csv << "travel_path_average  = " <<
            //  exploration_travel_path_average << std::endl;
            //  fs_csv << "map_progress_global  = " <<
            //  map_progress.global_freespace << std::endl;
            //  fs_csv << "map_progress_average = " <<
            //  map_progress.local_freespace << std::endl;
            //  fs_csv << "time                 = " <<
            //  map_progress.time << std::endl;
            //  fs_csv << "                       " << std::endl;
    ...
    int global_costmap_size()
    {
        ...
        int num_map_cells_ = costmap2d_global->getCostmap()->getSizeInCellsX() *
                             costmap2d_global->getCostmap()->getSizeInCellsY();
        int free = 0;
    ...
    int local_costmap_size()
    {
        ...
        if (!OPERATE_ON_GLOBAL_MAP)
        {
            ...
            num_map_cells_ = costmap2d_local->getCostmap()->getSizeInCellsX() *
                             costmap2d_local->getCostmap()->getSizeInCellsY();
        }
        else
        {
            occupancy_grid_local = costmap2d_local_size->getCostmap()->getCharMap();
            num_map_cells_ = costmap2d_local_size->getCostmap()->getSizeInCellsX() *
    ...
    void initLogPath()
    {
        ...
        log_path = nh.param<std::string>("log_path", "");

        std::string prefix = "/robot_";
        std::string robo_name = prefix.append(std::to_string(robot_id));
    ...
    void save_progress(bool final = false)
    {
        ...
        double exploration_travel_path =
                exploration->exploration_travel_path_global *
                costmap_resolution;
        double size_global_map =
                map_progress_during_exploration
                        .at(map_progress_during_exploration.size() - 1)
                        .global_freespace;
        ...
        fs.open(tmp_log.c_str(),
                std::fstream::in | std::fstream::trunc | std::fstream::out);
        ...
        fs << "[Exploration]" << "\n";
        fs << "time_file_written              = " << asctime(timeinfo) << "\n";
        fs << "start_time                     = " << time_start << "\n";
        fs << "end_time                       = " << ros::Time::now() << "\n";
        fs << "exploration_time               = " << exploration_time << "\n";
        fs << "required_goals                 = " << navigation_goals_required << "\n";
        fs << "unreachable_goals              = " << exploration->unreachable_frontiers.size() << "\n";
        fs << "travel_path_overall            = " << exploration_travel_path << "\n";
        fs << "number_of_completed_auctions   = " << exploration->number_of_completed_auctions << "\n";
        fs << "number_of_uncompleted_auctions = " << exploration->number_of_uncompleted_auctions << "\n";
        fs << "frontier_selection_strategy    = " << frontier_selection << "\n";
        fs << "costmap_size                   = " << costmap_width << "\n";
        fs << "global costmap iterations      = " << global_costmap_iteration << "\n";
    ...
    void exploration_has_finished()
    {
        ...
        double exploration_time = ros_time.toSec();
        int navigation_goals_required = counter;
        double exploration_travel_path = exploration->exploration_travel_path_global * 0.02;
        double size_global_map = map_progress_during_exploration
                                     .at(map_progress_during_exploration.size() - 1)
                                     .global_freespace;
        ...
        ROS_DEBUG("TIME: %f sec  GOALS: %d  PATH: %f  COMPLETED AUCTIONS: %d  "
                  ...
                  exploration_time, navigation_goals_required,
                  exploration_travel_path,
                  exploration->number_of_completed_auctions,
                  exploration->number_of_uncompleted_auctions);
        ROS_DEBUG("******************************************");
        ...
        // Indicate end of simulation for this robot
    ...
    bool iterate_global_costmap(std::vector<double>* global_goal, std::vector<std::string>* robot_str)
    {
        ...
        if (frontier_selection < 5 && frontier_selection != 1)
        {
            ...
            while (true)
            {
                exploration_flag = exploration->determine_goal(
                        2, global_goal, counter, -1, robot_str);
                 ...
                 frontier_selection == 5)
        {
            ...
            while (true)
            {
                ...
                goal_determined = exploration->determine_goal(
                        5, &goal_vector, 0, cluster_element, robot_str);

                if (!goal_determined)
                {
                    ROS_INFO("No goal was determined, cluster is empty. Bid "
                                     "for another one");

                    goal_vector.clear();
                    bool auctioning = exploration->auctioning(
                            &goal_vector, &clusters_used_by_others,
                            &robot_str_name);
                    if (auctioning)
                    {
                        goal_determined = true;
                    ...
                    {
                        /*
                         * If NO goals are selected at all, iterate over the
                         * global
                         * map to find some goals.
                         */
                        ROS_WARN("No goals are available at all");
            ...
            //     std::vector<double> goal_vector;
            //     goal_determined =
            //     exploration->determine_goal(4,
            //     &goal_vector, counter, cluster_element);
            //
            //     if(goal_determined == false)
            //     {
            //         ROS_INFO("Another cluster is not
            //         available, no cluster determined");
            //         return false;
            //     }
            //     else
            //     {
            //          /*
            //          * If negotiation is not needed,
            //          simply uncomment
            //          * and set the negotiation to TRUE.
            //          */
            //         bool negotiation =
            //         exploration->negotiate_Frontier(goal_vector.at(0),goal_vector.at(1),goal_vector.at(2),goal_vector.at(3),goal_vector.at(4));
            //         if(negotiation == true)
            //         {
            //             ROS_INFO("Negotiation was
            //             successful");
            //             cluster_element =
            //             goal_vector.at(4);
            //
            ...
            //             cluster_element =
            //             goal_vector.at(4);
            //             ROS_ERROR("Negotiation was not
            //             successful, try next cluster");
            //         }
            //         counter++;
            //     }
            // }
    ...
    bool navigate(std::vector<double> goal)
    {
        ...
        /*
         * If received goal is not empty (x=0 y=0), drive the robot to this
         * point
         * and mark that goal as seen in the last_goal_position vector!!!
         * Otherwise turn the robot by 90° to the right and search again for a
         * better
         * frontier.
         */
        ...
        {
            rotation_counter++;
            // ROS_INFO("In navigation ....
            // cluster_available: %lu     counter: %d",
            // clusters_available_in_pool.size(),
            // counter_waiting_for_clusters);
            ROS_INFO("In navigation .... cluster_available: %zu     counter: %d",
                     exploration->clusters.size(), counter_waiting_for_clusters);
            if (exploration->clusters.size() == 0 ||
                counter_waiting_for_clusters > 10) //(rotation_counter >= 2)
            {
                ...
                if (!global_costmap_goal)
                {
                    // global_iterations_counter++;
                    // if (global_iterations_counter >= 5)
                    // {
                    counter++;
                    ROS_INFO("GOAL %d: BACK TO HOME   x: %f    y: %f",
                             counter,
                             home_point_x,
                             home_point_y);
                ...
                else
                {
                    ...
                    std::vector<double> backoffGoal;
                    bool backoff_sucessfull = exploration->smartGoalBackoff(
                            global_goal.at(0),
                            global_goal.at(1),
                            costmap2d_global,
                            &backoffGoal);
                    ...
                    if (backoff_sucessfull)
                    {
                        ...
                        if (completed_navigation)
                        {
                            exploration->calculate_travel_path(
                                    exploration->visited_frontiers
                                            .at(exploration->visited_frontiers.size() -
                                                1)
                                            .x_coordinate,
                                    exploration->visited_frontiers
                                            .at(exploration->visited_frontiers.size() -
    ...
    bool move_robot(int seq, double position_x, double position_y)
    {
        ...
        goal_msgs.target_pose.pose.position.x = position_x; // - robotPose.getOrigin().getX();
        ...
        // ac.waitForResult(ros::Duration(20)); // TODO: Test if it also works with smaller value!
        ac.waitForResult(ros::Duration(waitForResult)); // TODO: investigate('here Parameter!')
...
int main(int argc, char **argv)
{
     ...
     * example be a subscription on another topic. Do this to be able to receive
     * a
     * message.
     */
