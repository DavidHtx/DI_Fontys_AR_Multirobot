...
#define INNER_DISTANCE                                                         \
    5 // radius (in cells) around goal point without obstacles (backoff goal
      // point)
#define MAX_NEIGHBOR_DIST                                                      \
    1 // radius (in cells) around selected goal without obstacles
#define CLUSTER_MERGING_DIST                                                   \
    0.8 // max (euclidean) distance between clusters that are merged
...
ExplorationPlanner::ExplorationPlanner(int robot_id, bool robot_prefix_empty,
                                       std::string robot_name_parameter,
                                       std::string mc_group)
    : costmap_ros_(0), occupancy_grid_array_(0), exploration_trans_array_(0),
      obstacle_trans_array_(0), frontier_map_array_(0), is_goal_array_(0),
      map_width_(0), map_height_(0), num_map_cells_(0), initialized_(false),
      last_mode_(FRONTIER_EXPLORE), p_alpha_(0), p_dist_for_goal_reached_(1),
      p_goal_angle_penalty_(0), p_min_frontier_size_(0),
      p_min_obstacle_dist_(0), p_plan_in_unknown_(true),
      p_same_frontier_dist_(0), p_use_inflated_obs_(false), previous_goal_(0),
      inflated(0), lethal(0), free(0), threshold_free(127),
      threshold_inflated(252), threshold_lethal(253), frontier_id_count(0),
      exploration_travel_path_global(0), cluster_id(0),
      initialized_planner(false), auction_is_running(false),
      auction_start(false), auction_finished(true), start_thr_auction(false),
      auction_id_number(1), next_auction_position_x(0),
      next_auction_position_y(0), other_robots_position_x(0),
      other_robots_position_y(0), number_of_completed_auctions(0),
      number_of_uncompleted_auctions(0), first_run(true),
      first_negotiation_run(true), robot_prefix_empty_param(false)
{
    ...
    std::string sendFrontier_msgs =
        robo_name + "/adhoc_communication/send_frontier";
    std::string sendAuction_msgs =
        robo_name + "/adhoc_communication/send_auction";

    ros::NodeHandle tmp;
    nh_service = &tmp;

    ROS_DEBUG("Sending frontier: '%s'     SendAuction: '%s'",
              sendFrontier_msgs.c_str(), sendAuction_msgs.c_str());

    ssendFrontier =
        nh_service->serviceClient<adhoc_communication::SendExpFrontier>(
            sendFrontier_msgs);
    ssendAuction =
        nh_service->serviceClient<adhoc_communication::SendExpAuction>(
            sendAuction_msgs);

    //    pub_negotion_first = nh_negotiation_first.advertise
    //    <adhoc_communication::Frontier> ("negotiation_list_first", 10000);

    //    pub_frontiers = nh_frontier.advertise <adhoc_communication::Frontier>
    //    ("frontiers", 10000);
    //    pub_visited_frontiers = nh_visited_frontier.advertise
    //    <adhoc_communication::Frontier> ("visited_frontiers", 10000);

    pub_visited_frontiers_points =
        nh_visited_Point.advertise<visualization_msgs::MarkerArray>(
            "visitedfrontierPoints", 2000, true);

    pub_Point =
        nh_Point.advertise<geometry_msgs::PointStamped>("goalPoint", 100, true);
    pub_frontiers_points =
        nh_frontiers_points.advertise<visualization_msgs::MarkerArray>(
            "frontierPoints", 2000, true);

    //    pub_auctioning_status =
    //    nh_auction_status.advertise<adhoc_communication::AuctionStatus>
    //    ("auctionStatus", 1000);
    //    pub_auctioning_first =
    //    nh_auction_first.advertise<adhoc_communication::Auction>
    //    ("auction_first", 1000);

    // pub_clusters =
    // nh_cluster.advertise<geometry_msgs::PolygonStamped>("clusters", 2000,
    // true);

    pub_cluster_grid_0 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_0", 2000, true);
    pub_cluster_grid_1 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_1", 2000, true);
    pub_cluster_grid_2 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_2", 2000, true);
    pub_cluster_grid_3 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_3", 2000, true);
    pub_cluster_grid_4 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_4", 2000, true);
    pub_cluster_grid_5 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_5", 2000, true);
    pub_cluster_grid_6 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_6", 2000, true);
    pub_cluster_grid_7 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_7", 2000, true);
    pub_cluster_grid_8 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_8", 2000, true);
    pub_cluster_grid_9 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_9", 2000, true);
    pub_cluster_grid_10 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_10", 2000, true);
    pub_cluster_grid_11 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_11", 2000, true);
    pub_cluster_grid_12 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_12", 2000, true);
    pub_cluster_grid_13 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_13", 2000, true);
    pub_cluster_grid_14 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_14", 2000, true);
    pub_cluster_grid_15 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_15", 2000, true);
    pub_cluster_grid_16 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_16", 2000, true);
    pub_cluster_grid_17 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_17", 2000, true);
    pub_cluster_grid_18 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_18", 2000, true);
    pub_cluster_grid_19 = nh_cluster_grid.advertise<nav_msgs::GridCells>(
        "cluster_grid_19", 2000, true);
    ...
    //    ROS_INFO("%s  %s  %s  %s", frontier_sub.c_str(),
    //    visited_frontiers_sub.c_str(), auction_sub.c_str(),
    //    all_positions_sub.c_str());

    //    sub_control = nh_control.subscribe("/control", 10000,
    //    &ExplorationPlanner::controlCallback, this);
    sub_frontiers =
        nh_frontier.subscribe(robo_name + "/frontiers", 10000,
                              &ExplorationPlanner::frontierCallback, this);
    sub_visited_frontiers = nh_visited_frontier.subscribe(
        robo_name + "/visited_frontiers", 10000,
        &ExplorationPlanner::visited_frontierCallback, this);
    sub_negotioation = nh_negotiation.subscribe(
        robo_name + "/negotiation_list", 10000,
        &ExplorationPlanner::negotiationCallback, this);
    sub_auctioning =
        nh_auction.subscribe(robo_name + "/auction", 1000,
                             &ExplorationPlanner::auctionCallback, this);
    sub_position =
        nh_position.subscribe(robo_name + "/all_positions", 1000,
                              &ExplorationPlanner::positionCallback, this);

    // TODO
    //    sub_robot =
    //    nh_robot.subscribe(robo_name+"/adhoc_communication/new_robot", 1000,
    //    &ExplorationPlanner::new_robot_callback, this);
    ...
    //    if(robot_id == 1)
    //    {
    //        sub_frontiers = nh_frontier.subscribe("/robot_1/frontiers", 10000,
    //        &ExplorationPlanner::frontierCallback, this);
    //        sub_visited_frontiers =
    //        nh_visited_frontier.subscribe("/robot_1/visited_frontiers", 10000,
    //        &ExplorationPlanner::visited_frontierCallback, this);
    //        sub_negotioation_first =
    //        nh_negotiation_first.subscribe("/robot_0/negotiation_list_first",
    //        10000, &ExplorationPlanner::negotiationCallback, this);
    //        sub_auctioning_first =
    //        nh_auction_first.subscribe("/robot_0/auction_first", 1000,
    //        &ExplorationPlanner::auctionCallback, this);
    //        sub_position = nh_position.subscribe("/robot_1/all_positions",
    //        1000, &ExplorationPlanner::positionCallback, this);
    //        sub_auctioning_status =
    //        nh_auction_status.subscribe("/robot_0/auctionStatus", 1000,
    //        &ExplorationPlanner::auctionStatusCallback, this);
    //    }else if(robot_id == 0)
    //    {
    //        sub_frontiers = nh_frontier.subscribe("/robot_0/frontiers", 10000,
    //        &ExplorationPlanner::frontierCallback, this);
    //        sub_visited_frontiers =
    //        nh_visited_frontier.subscribe("/robot_0/visited_frontiers", 10000,
    //        &ExplorationPlanner::visited_frontierCallback, this);
    //        sub_negotioation_first =
    //        nh_negotiation_first.subscribe("/robot_1/negotiation_list_first",
    //        10000, &ExplorationPlanner::negotiationCallback, this);
    //        sub_auctioning_first =
    //        nh_auction_first.subscribe("/robot_1/auction_first", 1000,
    //        &ExplorationPlanner::auctionCallback, this);
    //        sub_position = nh_position.subscribe("/robot_0/all_positions",
    //        1000, &ExplorationPlanner::positionCallback, this);
    //        sub_auctioning_status =
    //        nh_auction_status.subscribe("/robot_1/auctionStatus", 1000,
    //        &ExplorationPlanner::auctionStatusCallback, this);
    //    }
...
void ExplorationPlanner::Callbacks()
{
    ...
    while (ros::ok())
    {
        ...
        if (robot_name == 1)
        {
            sub_frontiers = nh_frontier.subscribe(
                "/robot_0/frontiers", 10000,
                &ExplorationPlanner::frontierCallback, this);
            sub_visited_frontiers = nh_visited_frontier.subscribe(
                "/robot_0/visited_frontiers", 10000,
                &ExplorationPlanner::visited_frontierCallback, this);
            sub_negotioation = nh_negotiation.subscribe(
                "/robot_0/negotiation_list", 10000,
                &ExplorationPlanner::negotiationCallback, this);
            sub_auctioning = nh_auction.subscribe(
                "/robot_1/auction", 1000, &ExplorationPlanner::auctionCallback,
                this);

            //                sub_frontiers =
            //                nh_frontier.subscribe("/robot_2/frontiers", 10000,
            //                &ExplorationPlanner::frontierCallback, this);
            //                sub_visited_frontiers =
            //                nh_visited_frontier.subscribe("/robot_2/visited_frontiers",
            //                10000,
            //                &ExplorationPlanner::visited_frontierCallback,
            //                this);
            //                sub_negotioation =
            //                nh_negotiation.subscribe("/robot_2/negotiation_list",
            //                10000, &ExplorationPlanner::negotiationCallback,
            //                this);
            //
            //                sub_frontiers =
            //                nh_frontier.subscribe("/robot_3/frontiers", 10000,
            //                &ExplorationPlanner::frontierCallback, this);
            //                sub_visited_frontiers =
            //                nh_visited_frontier.subscribe("/robot_3/visited_frontiers",
            //                10000,
            //                &ExplorationPlanner::visited_frontierCallback,
            //                this);
            //                sub_negotioation =
            //                nh_negotiation.subscribe("/robot_3/negotiation_list",
            //                10000, &ExplorationPlanner::negotiationCallback,
            //                this);
        }
        ...
        {
            sub_frontiers = nh_frontier.subscribe(
                "/robot_1/frontiers", 10000,
                &ExplorationPlanner::frontierCallback, this);
            sub_visited_frontiers = nh_visited_frontier.subscribe(
                "/robot_1/visited_frontiers", 10000,
                &ExplorationPlanner::visited_frontierCallback, this);
            sub_negotioation = nh_negotiation.subscribe(
                "/robot_1/negotiation_list", 10000,
                &ExplorationPlanner::negotiationCallback, this);
            sub_auctioning = nh_auction.subscribe(
                "/robot_0/auction", 1000, &ExplorationPlanner::auctionCallback,
                this);
            //                sub_frontiers =
            //                nh_frontier.subscribe("/robot_2/frontiers", 10000,
            //                &ExplorationPlanner::frontierCallback, this);
...
// void ExplorationPlanner::new_robot_callback(const std_msgs::StringConstPtr
// &msg)
//{
...
void ExplorationPlanner::initialize_planner(
    std::string name, costmap_2d::Costmap2DROS *costmap,
...
bool ExplorationPlanner::clusterFrontiers()
{
    ...
    {
        ROS_DEBUG("Frontier at: %d   and cluster size: %zu", i,
                  clusters.size());
        ...
        {
            ROS_DEBUG("cluster %d contains %zu elements", j,
                      clusters.at(j).cluster_element.size());
            for (int n = 0; n < clusters.at(j).cluster_element.size(); n++)
            {
                ROS_DEBUG("accessing cluster %d  and element: %d", j, n);

                if (fabs(frontiers.at(i).x_coordinate -
                         clusters.at(j).cluster_element.at(n).x_coordinate) <
                        MAX_NEIGHBOR_DIST &&
                    fabs(frontiers.at(i).y_coordinate -
                         clusters.at(j).cluster_element.at(n).y_coordinate) <
                        MAX_NEIGHBOR_DIST)
                {
                    for (int m = 0; m < clusters.at(j).cluster_element.size();
                         m++)
                    {
                        ROS_DEBUG("checking id %d with element id: %d",
                                  frontiers.at(i).id,
                                  clusters.at(j).cluster_element.at(m).id);
                        if (robot_prefix_empty_param == true)
                        {
                            if (frontiers.at(i).id ==
                                    clusters.at(j).cluster_element.at(m).id &&
                                frontiers.at(i).detected_by_robot_str.compare(
                                    clusters.at(j)
                                        .cluster_element.at(m)
                                        .detected_by_robot_str) == 0)
                            {
    ...
    /*
     * To finish the process finally check whether a cluster is close to another
     * one.
     * If so, merge them.
     */
                            ...
                            if (clusters.at(i).id != clusters.at(j).id)
                            {
                                if (clusters.at(j).cluster_element.size() > 1)
                                {
                                    for (int n = 0;
                                         ...
                                         n++)
                                    {
                                        if (fabs(clusters.at(i)
                                                     .cluster_element.at(m)
                                                     .x_coordinate -
                                                 clusters.at(j)
                                                     .cluster_element.at(n)
                                                     .x_coordinate) <
                                                CLUSTER_MERGING_DIST &&
                                            fabs(clusters.at(i)
                                                     .cluster_element.at(m)
                                                     .y_coordinate -
                                                 clusters.at(j)
                                             ...
                                             n++)
                                        {
                                            ...
                                            frontier_to_merge =
                                                clusters.at(j)
                                                    .cluster_element.at(n);
                                            clusters.at(i)
                                                .cluster_element.push_back(
                                                    frontier_to_merge);
                                        }
                                        // ROS_INFO("Erasing cluster %d",
                                        // clusters.at(j).id);
                            ...
                            if (clusters.at(i).id != clusters.at(j).id)
                            {
                                if (clusters.at(j).cluster_element.size() > 1)
                                {
                                         ...
                                         n++)
                                    {
                                        ...
                                        ROS_INFO("x_length: %d    y_length: %d "
                                                 "  resolution: %f",
                                                 x_length, y_length,
                                                 costmap_resolution);
                                        int x_elements =
                                            std::abs((int)(x_length /
                                                           costmap_resolution));
                                        int y_elements =
                                            std::abs((int)(y_length /
                                                           costmap_resolution));
                                        ...
                                        ROS_INFO("atan: %f   x_length: %d    "
                                                 "y_length: %d",
                                                 alpha, x_length, y_length);

                                        for (int x = 0; x <= x_elements; x++)
                                        {
                                            /* increase neighbor size*/
                                            for (int neighbour = 0;
                                                 neighbour < MAX_NEIGHBOURS;
                                                 neighbour++)
                                            {
                                                int y =
                                                    tan(alpha) * x *
                                        ...
                                        ROS_INFO(
                                            "*******************************");
                                        ROS_INFO("elements: %d    obstacle: %d",
                                                 elements_detected,
                                                 obstacle_detected);
                                        ROS_INFO(
                                            "*******************************");
                                        if (elements_detected * 0.25 <
                                            obstacle_detected)
                                        {
                                        ...
                                        break;
                                        // if
                                        // (fabs(clusters.at(i).cluster_element.at(m).x_coordinate
                                        // -
                                        // clusters.at(j).cluster_element.at(n).x_coordinate)
                                        // < CLUSTER_MERGING_DIST &&
                                        //     fabs(clusters.at(i).cluster_element.at(m).y_coordinate
                                        //     -
                                        //     clusters.at(j).cluster_element.at(n).y_coordinate)
                                        //     < CLUSTER_MERGING_DIST)
                                        // {
...
void ExplorationPlanner::visualizeClustersConsole()
{
    ...
    for (int j = 0; j < clusters.size(); j++)
    {
        for (int n = 0; n < clusters.at(j).cluster_element.size(); n++)
        {
            ...
            {
                ROS_DEBUG(
                    "ID: %6d  x: %5.2f  y: %5.2f  cluster: %5d   robot: %s",
                    clusters.at(j).cluster_element.at(n).id,
                    clusters.at(j).cluster_element.at(n).x_coordinate,
                    clusters.at(j).cluster_element.at(n).y_coordinate,
                    clusters.at(j).id, clusters.at(j)
                                           .cluster_element.at(n)
                                           .detected_by_robot_str.c_str());
            }
            else
            {
                ROS_DEBUG(
                    "ID: %6d  x: %5.2f  y: %5.2f  cluster: %5d   dist: %d",
                    clusters.at(j).cluster_element.at(n).id,
                    clusters.at(j).cluster_element.at(n).x_coordinate,
                    clusters.at(j).cluster_element.at(n).y_coordinate,
                    clusters.at(j).id,
...
bool ExplorationPlanner::transformToOwnCoordinates_frontiers()
{
    ...
    for (int i = 0; i < frontiers.size(); i++)
    {
        ...
        if (same_robot == false)
        {
            ...
            for (int j = 0;
                 j < transformedPointsFromOtherRobot_frontiers.size(); j++)
            {
                if (robot_prefix_empty_param == 0)
                {
                    if (transformedPointsFromOtherRobot_frontiers.at(j).id ==
                            frontiers.at(i).id &&
                        frontiers.at(i).detected_by_robot_str.compare(
                            transformedPointsFromOtherRobot_frontiers.at(j)
                                .robot_str) == 0)
                    {
                        transform_flag = true;
                        break;
            ...
            if (transform_flag != true)
            {
                ...
                else
                {
                    ...
                    //                    robo_name =
                    //                    lookupRobotName(frontiers.at(i).detected_by_robot);
                    //                    robo_name2 =
                    //                    lookupRobotName(robot_name);
                    robo_name = frontiers.at(i).detected_by_robot_str;
                    robo_name2 = robot_str;
                    ROS_DEBUG("Robot: %s   transforms from robot: %s",
                              robo_name2.c_str(), robo_name.c_str());
                }

                std::string service_topic = robo_name2.append(
                    "/map_merger/transformPoint"); // FIXME for real scenario!!!
                                                   // robot_might not be used
                                                   // here

                //              ROS_INFO("Robo name: %s   Service to subscribe
                //              to: %s", robo_name.c_str(),
                //              service_topic.c_str());

                client = nh_transform.serviceClient<map_merger::TransformPoint>(
                    service_topic);
                ...
                ROS_DEBUG("Robot name:  %s",
                          service_message.request.point.src_robot.c_str());
                ROS_DEBUG("Old x: %f   y: %f", frontiers.at(i).x_coordinate,
                ...
                if (client.call(service_message))
                {
                    ...
                    if (robot_prefix_empty_param == true)
                    {
                        transform_point.robot_str =
                            frontiers.at(i).detected_by_robot_str;
                    }
                    transformedPointsFromOtherRobot_frontiers.push_back(
                        transform_point);
...
bool ExplorationPlanner::transformToOwnCoordinates_visited_frontiers()
{
    ...
    for (int i = 0; i < visited_frontiers.size(); i++)
    {
        ...
        if (robot_prefix_empty_param == true)
        {
            if (visited_frontiers.at(i).detected_by_robot_str.compare(
                    robot_str) == 0)
            {
                same_robot = true;
                // ROS_ERROR("Same Robot Detected");
        ...
        if (same_robot == false)
        {
            bool transform_flag = false;
            for (int j = 0;
                 j < transformedPointsFromOtherRobot_visited_frontiers.size();
                 j++)
            {
                if (robot_prefix_empty_param == 0)
                {
                    if (transformedPointsFromOtherRobot_visited_frontiers.at(j)
                                .id == visited_frontiers.at(i).id &&
                        visited_frontiers.at(i).detected_by_robot_str.compare(
                            transformedPointsFromOtherRobot_visited_frontiers
                                .at(j)
                                .robot_str) == 0)
                    {
                        transform_flag = true;
            ...
            if (transform_flag != true)
            {
                ...
                {
                    // robo_name =
                    // lookupRobotName(visited_frontiers.at(i).detected_by_robot);
                ...
                std::string service_topic = robo_name2.append(
                    "/map_merger/transformPoint"); // FIXME for real scenario!!!
                                                   // robot_ might not be used
                                                   // here

                client = nh_transform.serviceClient<map_merger::TransformPoint>(
                    service_topic);

                service_message.request.point.x =
                    visited_frontiers.at(i).x_coordinate;
                service_message.request.point.y =
                ...
                if (client.call(service_message))
                {
                    ...
                    {
                        transform_point.robot_str =
                            visited_frontiers.at(i).detected_by_robot_str;
                    }
                    transformedPointsFromOtherRobot_visited_frontiers.push_back(
                        transform_point);
...
bool ExplorationPlanner::check_trajectory_plan()
{
    ...
    startPointSimulated.header.seq =
        start_point_simulated_message++; // increase the sequence number
    ...
    for (int i = 0; i < 10; i++)
    {
        if (frontiers.size() > i)
        {
            ...
            bool backoff_flag = smartGoalBackoff(
                frontiers.at(i).x_coordinate, frontiers.at(i).y_coordinate,
                costmap_global_ros_, &backoffGoal);

            goalPointSimulated.header.seq =
                goal_point_simulated_message++; // increase the sequence number
            ...
            {
                goalPointSimulated.pose.position.x =
                    frontiers.at(i).x_coordinate;
                goalPointSimulated.pose.position.y =
                    frontiers.at(i).y_coordinate;
            }
            ...
            nav.makePlan(startPointSimulated, goalPointSimulated,
                         NAVFN_TOLERANCE, global_plan);
            distance = global_plan.size();
            frontiers.at(i).distance_to_robot = distance;

            ROS_DEBUG("Distance: %d Frontier: %d", distance,
                      frontiers.at(i).id);
        }
        else
        {
...
int ExplorationPlanner::calculate_travel_path(double x, double y)
{
    ...
    std::vector<double> backoffGoal;
    bool backoff_flag =
        smartGoalBackoff(x, y, costmap_global_ros_, &backoffGoal);
    ...
    bool successful = nav.makePlan(startPointSimulated, goalPointSimulated,
                                   NAVFN_TOLERANCE, global_plan);
    ...
    {
        distance = global_plan.size();
        exploration_travel_path_global =
...
int ExplorationPlanner::check_trajectory_plan(double x, double y)
{
    ...
    //    tp.initialize("my_trajectory_planner", &tf_planner,
    //    &costmap_global_ros_);
...
int ExplorationPlanner::estimate_trajectory_plan(double start_x, double start_y,
                                                 double target_x,
                                                 double target_y)
{
    geometry_msgs::PoseStamped goalPointSimulated, startPointSimulated;
    ...
    std::vector<double> backoffGoal;
    bool backoff_flag =
        smartGoalBackoff(target_x, target_y, costmap_global_ros_, &backoffGoal);
...
void ExplorationPlanner::setRobotConfig(int name, double robot_home_position_x,
...
void ExplorationPlanner::home_position_(
...
void ExplorationPlanner::printFrontiers()
{
    for (int i = 0; i < frontiers.size(); i++)
    {
        if (robot_prefix_empty_param == true)
        {
            ROS_INFO("Frontier %d:   x: %f   y: %f   robot: %s",
                     frontiers.at(i).id, frontiers.at(i).x_coordinate,
                     frontiers.at(i).y_coordinate,
                     frontiers.at(i).detected_by_robot_str.c_str());
        }
        else
        {
            ROS_INFO("Frontier %d:   x: %f   y: %f", frontiers.at(i).id,
                     frontiers.at(i).x_coordinate,
                     frontiers.at(i).y_coordinate);
        }
    }
...
bool ExplorationPlanner::storeFrontier(double x, double y,
                                       ...
                                       int id)
{
    ...
    if (robot_prefix_empty_param == true)
    {
        ROS_DEBUG("Storing Frontier ID: %d   Robot: %s", id,
                  detected_by_robot_str.c_str());
        if (id != -1)
        {
            new_frontier.id = id;
                                              ...
                                              std::string detected_by_robot_str)
{
    for (int i = 0; i < frontiers.size(); i++)
    {
        if (robot_prefix_empty_param == true)
        {
            ROS_DEBUG("Removing frontier with id '%d' detected by robot '%s'",
                      frontiers.at(i).id,
                      frontiers.at(i).detected_by_robot_str.c_str());
            ...
            {
                ROS_DEBUG(
                    "Removing Frontier ID: %d  at position: %d  of Robot: %s",
                    frontiers.at(i).id, i,
                    frontiers.at(i).detected_by_robot_str.c_str());
                store_frontier_mutex.lock();
...
bool ExplorationPlanner::storeVisitedFrontier(double x, double y,
                                              int detected_by_robot,
                                              std::string detected_by_robot_str,
                                              int id)
{
    ...
    if (robot_prefix_empty_param == true)
    {
        ROS_DEBUG("Storing Visited Frontier ID: %d   Robot: %s",
                  visited_frontier_id_count, detected_by_robot_str.c_str());
        if (id != -1)
        {
    ...
    else
    {
        ...
        {
            visited_frontier.id =
                (robot_name * 10000) + visited_frontier_id_count++;
        }
...
bool ExplorationPlanner::removeVisitedFrontier(
    int id, std::string detected_by_robot_str)
{
    for (int i = 0; i < visited_frontiers.size(); i++)
    {
        if (robot_prefix_empty_param == true)
        {
            ...
            if (visited_frontiers.at(i).id == id &&
                visited_frontiers.at(i).detected_by_robot_str.compare(
                    detected_by_robot_str) == 0)
            {
                ROS_INFO("Removing Visited Frontier ID: %d  at position: %d  "
                         "of Robot: %s",
                         visited_frontiers.at(i).id, i,
                         visited_frontiers.at(i).detected_by_robot_str.c_str());
                store_visited_mutex.lock();
...
bool ExplorationPlanner::storeUnreachableFrontier(
    double x, double y, int detected_by_robot,
    std::string detected_by_robot_str, int id)
{
    frontier_t unreachable_frontier;

    if (robot_prefix_empty_param == true)
    {
        ROS_DEBUG("Storing Unreachable Frontier ID: %d   Robot: %s",
                  unreachable_frontier_id_count, detected_by_robot_str.c_str());
    ...
    else
    {
        ...
        {
            unreachable_frontier.id =
                (robot_name * 10000) + unreachable_frontier_id_count++;
        }
...
bool ExplorationPlanner::removeUnreachableFrontier(
    int id, std::string detected_by_robot_str)
{
    for (int i = 0; i < unreachable_frontiers.size(); i++)
    {
        if (robot_prefix_empty_param == true)
        {
            if (unreachable_frontiers.at(i).id == id &&
                unreachable_frontiers.at(i).detected_by_robot_str.compare(
                    detected_by_robot_str) == 0)
            {
                ROS_INFO(
                    "Removing Unreachable Frontier ID: %d  at position: %d  of "
                    "Robot: %s",
                    unreachable_frontiers.at(i).id, i,
                    unreachable_frontiers.at(i).detected_by_robot_str.c_str());
...
bool ExplorationPlanner::publish_negotiation_list(
    frontier_t negotiation_frontier, int cluster_number)
{
    ...
    //        negotiation_element.detected_by_robot =
    //        negotiation_list.at(i).detected_by_robot;
    //        negotiation_element.x_coordinate =
    //        negotiation_list.at(i).x_coordinate;
    //        negotiation_element.y_coordinate =
    //        negotiation_list.at(i).y_coordinate;
    ...
    if (cluster_number != -1)
    {
        ...
        for (int i = 0; i < clusters.size(); i++)
        {
            ...
            {
                ROS_DEBUG("Cluster ID: %d   is at vector position: %d",
                          cluster_number, i);
        ...
        for (int i = 0;
             i < clusters.at(cluster_vector_position).cluster_element.size();
             i++)
        {
            adhoc_communication::ExpFrontierElement negotiation_element;
            negotiation_element.x_coordinate =
                clusters.at(cluster_vector_position)
                    .cluster_element.at(i)
                    .x_coordinate;
            negotiation_element.y_coordinate =
                clusters.at(cluster_vector_position)
                    .cluster_element.at(i)
            ...
            negotiation_element.detected_by_robot =
                clusters.at(cluster_vector_position)
                    .cluster_element.at(i)
                    .detected_by_robot;
            negotiation_element.id =
                clusters.at(cluster_vector_position).cluster_element.at(i).id;

            frontier_t new_frontier;
            new_frontier.x_coordinate = clusters.at(cluster_vector_position)
                                            .cluster_element.at(i)
                                            .x_coordinate;
            new_frontier.y_coordinate = clusters.at(cluster_vector_position)
                                            .cluster_element.at(i)
                                            .y_coordinate;
            new_frontier.detected_by_robot =
                clusters.at(cluster_vector_position)
                    .cluster_element.at(i)
                    .detected_by_robot;
            new_frontier.id =
    ...
    else
    {
        adhoc_communication::ExpFrontierElement negotiation_element;
        negotiation_element.detected_by_robot =
            negotiation_frontier.detected_by_robot;
        ...
        negotiation_msg.frontier_element.push_back(
            negotiation_element); // FIXME
    }
...
bool ExplorationPlanner::sendToMulticast(
    ...
    adhoc_communication::ExpFrontier frontier_to_send, std::string topic)
{
    //    ROS_INFO("SENDING frontier id: %ld    detected_by: %ld",
    //    frontier_to_send.id ,frontier_to_send.detected_by_robot);
    adhoc_communication::SendExpFrontier
        service_frontier; // create request of type any+
    ...
    //	std::string destination_name = multi_cast_group + robo_name; //for
    //multicast
    //  std::string destination_name = robo_name; // unicast

    ROS_INFO("sending to multicast group '%s' on topic: '%s'",
             destination_name.c_str(), topic.c_str());
    ...
    if (ssendFrontier.call(service_frontier))
    {
        ...
        {
            ROS_DEBUG("Failed to send to multicast group %s!",
    ...
    {
        ROS_WARN("Failed to call service sendToMulticast [%s]",
                 ssendFrontier.getService().c_str());
        return false;
    }
}
...
bool ExplorationPlanner::sendToMulticastAuction(
    std::string multi_cast_group,
    adhoc_communication::ExpAuction auction_to_send, std::string topic)
{
    adhoc_communication::SendExpAuction
        service_auction; // create request of type any+
    ...
    ROS_INFO("sending auction to multicast group '%s' on topic '%s'",
             destination_name.c_str(), topic.c_str());
    ...
    if (ssendAuction.call(service_auction))
    {
        ...
        {
            ROS_WARN("Failed to send auction to mutlicast group %s!",
                     destination_name.c_str());
            return false;
        }
    }
    ...
    {
        ROS_WARN("Failed to call service sendToMulticastAuction [%s]",
                 ssendAuction.getService().c_str());
        return false;
    }
}
    ...
    const adhoc_communication::ExpFrontier::ConstPtr &msg)
{
    ...
    for (int j = 0; j < msg.get()->frontier_element.size(); j++)
    {
        ...
        for (int i = 0; i < negotiation_list.size(); i++)
        {
            if (robot_prefix_empty_param == true)
            {
                if (negotiation_list.at(i).id == frontier_element.id &&
                    negotiation_list.at(i).detected_by_robot_str.compare(
                        frontier_element.detected_by_robot_str) == 0)
                {
                    entry_found = true;
                }
            }
        ...
        {
            ROS_DEBUG("Negotiation frontier with ID: %" PRId64 "",
                      frontier_element.id);
            frontier_t negotiation_frontier;
            negotiation_frontier.detected_by_robot =
                frontier_element.detected_by_robot;
...
// void ExplorationPlanner::auctionStatusCallback(const
// adhoc_communication::AuctionStatus::ConstPtr& msg)
//{
...
////        }
////        ROS_INFO("Received auction with auction number: %d",
///msg.get()->auction_id);
...
//    clusterFrontiers();
//
//    if(msg.get()->occupied_ids.size() > 0 ||
//    msg.get()->requested_clusters.size() > 0)
//    {
...
//        }
////        ROS_INFO("Requested cluster size: %lu",
///msg.get()->requested_clusters.size());
//        for(int i = 0; i < msg.get()->requested_clusters.size(); i++)
//        {
//            adhoc_communication::Cluster requested_cluster,requested_ids;
//
//            requested_cluster = msg.get()->requested_clusters.at(i);
////            ROS_INFO("IDS_contained:
///%lu",requested_cluster.ids_contained.size());
//
////            for(int j = 0; j < requested_cluster.ids_contained.size(); j++)
////            {
//////                ROS_INFO("received ids: %d",
///requested_cluster.ids_contained.at(j));
////
...
//        thr_auction_status =
//        boost::thread(&ExplorationPlanner::respondToAuction, this,
    ...
    int auction_id_number)
{
    ...
    {
        ROS_INFO("Responding to cluster ids: %d",
                 requested_cluster_ids.at(i).own_cluster_id);
    }
    ...
    for (int n = 0; n < requested_cluster_ids.size(); n++)
    {
        ...
        for (int i = 0; i < clusters.size(); i++)
        {
            if (clusters.at(i).id == requested_cluster_ids.at(n).own_cluster_id)
            {
                ...
                for (int j = 0;
                     j < requested_cluster_ids.at(n).requested_ids.size(); j++)
                {
                    if (robot_prefix_empty_param == true)
                    {
                        cluster_element_msg.id =
                            requested_cluster_ids.at(n).requested_ids.at(j).id;
                        cluster_element_msg.detected_by_robot_str =
                            requested_cluster_ids.at(n)
                                .requested_ids.at(j)
                                .robot_str;
                    }
                    else
                ...
                cluster_msg.bid =
                    calculateAuctionBID(clusters.at(i).id, trajectory_strategy);
...
int ExplorationPlanner::calculateAuctionBID(int cluster_number,
                                            std::string strategy)
{
    //    ROS_INFO("Robot %d  calculates bid for cluster_number %d", robot_name,
    //    cluster_number);
    int auction_bid = 0;
    ...
    if (clusters.size() > 0)
    {
        for (int i = 0; i < clusters.size(); i++)
        {
            if (clusters.at(i).id == cluster_number)
            {
                ...
                //                }else
                //                {
                //                    ROS_ERROR("Cluster found but empty,
                //                    therefore do not calculate LOCAL BID");
                //                    return(-1);
                //                }
            }
        }
    ...
    int distance = -1;
    for (int i = 0;
         i < clusters.at(cluster_vector_position).cluster_element.size(); i++)
    {

        if (strategy == "trajectory")
        {
            distance =
                check_trajectory_plan(clusters.at(cluster_vector_position)
                                          .cluster_element.at(i)
                                          .x_coordinate,
                                      clusters.at(cluster_vector_position)
                                          .cluster_element.at(i)
                                          .y_coordinate);
            //               distance =
            //               estimate_trajectory_plan(robotPose.getOrigin().getX(),
            ...
            //               ROS_INFO("Distance for cluster %d:
            //               %d",clusters.at(cluster_vector_position).id,
            //               distance);
        }
        else if (strategy == "euclidean")
        {
            double x = clusters.at(cluster_vector_position)
                           .cluster_element.at(i)
                           .x_coordinate -
                       robotPose.getOrigin().getX();
            double y = clusters.at(cluster_vector_position)
                           .cluster_element.at(i)
                           .y_coordinate -
                       robotPose.getOrigin().getY();
            distance = x * x + y * y;
        ...
        if (distance > 0)
        {
             ...
             */
            //            ROS_INFO("Euclidean distance: %f   trajectory_path:
            //            %f", sqrt(euclidean_distance), distance*
            //            costmap_ros_->getCostmap()->getResolution());
            if (distance * costmap_ros_->getCostmap()->getResolution() <=
                sqrt(euclidean_distance) * 0.95)
            {
                ROS_WARN("Euclidean distance smaller then trajectory distance "
                         "to LOCAL CLUSTER!!!");
                //                return(-1);
            }
            else
    ...
    if (distance == -1)
    {
        //        ROS_ERROR("Unable to calculate LOCAL BID at position %d  -->
        //        BID: %d", cluster_vector_position, distance);
    }
    //    ROS_INFO("Cluster at position %d  --> BID: %d",
    //    cluster_vector_position, distance);
    return (-1);
}

void ExplorationPlanner::positionCallback(
    const adhoc_communication::MmListOfPoints::ConstPtr &msg)
{
    ...
    const adhoc_communication::ExpAuction::ConstPtr &msg)
{
    ...
    else
    {
        ...
        ROS_INFO("Robot name: %d      int_name: %d", robot_name,
                 robots_int_name);
    ...
    if (same_robot == false)
    {
        ...
        */
        ...
        if (msg.get()->auction_status_message == true)
        {
            ...
            for (int i = 0; i < msg.get()->requested_clusters.size(); i++)
            {
                ...
                for (int j = 0; j < cluster_req.ids_contained.size(); j++)
                {
                    ...
                    requested_clusters.append(NumberToString(
                        (int)cluster_req.ids_contained.at(j).id));
                    requested_clusters.append(", ");
                }
                ROS_INFO("Requested ids: %s from robot: %s",
                         requested_clusters.c_str(),
                         msg.get()->robot_name.c_str());
            }
            ...
            /*
             * Grep all occupied ids and try to convert them into clusters in
             * own
             * coordinate system. This ensures that all robots in the system
             * know
                ...
                msg.get()->requested_clusters.size() > 0)
            {
                for (int i = 0; i < msg.get()->occupied_ids.size(); i++)
                {
                    ...
                    cluster_element.detected_by_robot_str =
                        msg.get()->occupied_ids.at(i).detected_by_robot_str;

                    occupied_ids.ids_contained.push_back(cluster_element);
                }
                int occupied_cluster_id = checkClustersID(occupied_ids);
                ROS_INFO("Check occupied cluster to be the same. %d",
                         occupied_cluster_id);

                if (occupied_cluster_id >= 0)
                {
                    ROS_INFO("Adding occupied cluster: %d",
                             occupied_cluster_id);
                    already_used_ids.push_back(occupied_cluster_id);
                }
                else
                {
                    /* Store undetected Clusters in a struct for later
                     * processing */
                ...
                {
                    int unrecognized_occupied_cluster_id =
                        checkClustersID(unrecognized_occupied_clusters.at(i));
                    if (unrecognized_occupied_cluster_id >= 0)
                    {
                        ROS_INFO("Unrecognized cluster: %d converted",
                                 unrecognized_occupied_cluster_id);
                        already_used_ids.push_back(
                            unrecognized_occupied_cluster_id);
                        unrecognized_occupied_clusters.erase(
                            unrecognized_occupied_clusters.begin() + i);
                ...
                for (int i = 0; i < msg.get()->requested_clusters.size(); i++)
                {
                    ...
                    if (check_cluster_id >= 0)
                    {
                        ...
                        for (int j = 0;
                             j < requested_cluster.ids_contained.size(); j++)
                        {
                            transform_point_t cluster_element_point;
                            if (robot_prefix_empty_param == true)
                            {
                                cluster_element_point.id =
                                    requested_cluster.ids_contained.at(j).id;
                                cluster_element_point.robot_str =
                                    requested_cluster.ids_contained.at(j)
                            ...
                            new_cluster_request.requested_ids.push_back(
                                cluster_element_point);
                        }
            ...
            if (auction_start == true)
            {
                ...
                thr_auction_status =
                    boost::thread(&ExplorationPlanner::respondToAuction, this,
                                  requested_cluster_ids, msg.get()->auction_id);
            }
        }
        else if (msg.get()->auction_status_message == false)
        {
            ...
            {
                if (msg.get()->auction_id ==
                    10000 * robot_name + auction_id_number)
                {
                    continue_auction = true;
                }
            }
            ...
            {
                //            ROS_INFO("auction_id: %d       local_id: %d",
                //            msg.get()->auction_id, 10000*robot_name +
                //            auction_id_number);
                ...
                for (int i = 0; i < robots_already_responded.size(); i++)
                {
                    if (robot_prefix_empty_param == true)
                    {
                        if (msg.get()->robot_name.compare(
                                robots_already_responded.at(i).robot_str) ==
                                0 &&
                            msg.get()->auction_id ==
                                robots_already_responded.at(i).auction_number)
                        {
                    ...
                    {
                        ROS_INFO("Compare msg name: %d  responded: %d       "
                                 "msg auction: %d   responded: %d",
                                 robots_int_name,
                                 robots_already_responded.at(i).robot_number,
                                 msg.get()->auction_id,
                                 robots_already_responded.at(i).auction_number);
                        if (robots_int_name ==
                                robots_already_responded.at(i).robot_number &&
                ...
                if (robot_already_answered == false)
                {
                    if (robot_prefix_empty_param == true)
                    {
                        ROS_INFO("Auction from robot %s received",
                                 msg.get()->robot_name.c_str());
                    }
                    else
                    {
                     ...
                     */
                    for (int i = 0; i < msg.get()->available_clusters.size();
                         i++)
                    {
                        adhoc_communication::ExpCluster cluster_req;
                        ...
                        ROS_INFO("---------------------- %d "
                                 ...
                                 i);
                        std::string requested_clusters;
                        for (int j = 0; j < cluster_req.ids_contained.size();
                             j++)
                        {
                            if (j >= 6)
                            {
                         ...
                         i++)
                    {
                        ...
                        //        ROS_INFO("------------------------------------------------------------------");
                        //        for(int k = 0; k <
                        //        current_cluster.ids_contained.size(); k++)
                        //        {
                        //            ROS_INFO("FRONTIER ID: %d",
                        //            current_cluster.ids_contained.at(k));
                        //        }
                        int current_cluster_id =
                            checkClustersID(current_cluster);

                        ROS_INFO("Received ID converted to cluster ID: %d",
                                 current_cluster_id);
                        if (current_cluster_id >= 0)
                        {
                            auction_pair.cluster_id = current_cluster_id;
                            auction_pair.bid_value = current_cluster.bid;
                            auction_elements.auction_element.push_back(
                                auction_pair);
                            ROS_INFO("BID: %f", current_cluster.bid);
                        }
                    }
                    //    ROS_INFO("Robot %d received all bids for all
                    //    clusters", robot_name);
                    auction_elements.robot_id = robots_int_name;
                    auction_elements.detected_by_robot_str =
                        msg.get()->robot_name;
                    ...
                    /*
                     * A BID is received from one robot. Remember the robot who
                     * send the
...
int ExplorationPlanner::checkClustersID(
    adhoc_communication::ExpCluster cluster_to_check)
{
    ...
    for (int j = 0; j < clusters.size(); j++)
    {
        ...
        {
            //            for(int m= 0; m <
            //            clusters.at(j).cluster_element.size(); m++)
            //            {
            //                ROS_INFO("Ids in cluster to check with:
            //                %d",clusters.at(j).cluster_element.at(m).id);
            //            }
            //            ROS_INFO("Ids in cluster to check with:
            //            %d",clusters.at(j).cluster_element.at(n).id);
            for (int i = 0; i < cluster_to_check.ids_contained.size(); i++)
            {
                if (robot_prefix_empty_param == true)
                {
                    //                    ROS_INFO("Cluster id: %d  robot: %s
                    //                    msg id: %d  robot:
                    //                    %s",clusters.at(j).cluster_element.at(n).id,
                    //                    clusters.at(j).cluster_element.at(n).detected_by_robot_str,
                    //                    cluster_to_check.ids_contained.at(i).id,
                    //                    cluster_to_check.ids_contained.at(i).detected_by_robot_str);
                    if (clusters.at(j).cluster_element.at(n).id ==
                            cluster_to_check.ids_contained.at(i).id &&
                        clusters.at(j)
                                .cluster_element.at(n)
                                .detected_by_robot_str.compare(
                                    cluster_to_check.ids_contained.at(i)
                                        .detected_by_robot_str) == 0)
                    {
                        same_id_found++;
                    }
        ...
        //        ROS_INFO("Same IDs found: %f   elements*0.7: %f",
        //        same_id_found,clusters.at(j).cluster_element.size() * 0.7);

        //            if(same_id_found > clusters.at(j).cluster_element.size() *
        //            0.8)
        //            {
    ...
    //    {
    //
    //        std::sort(compare_clusters.begin(), compare_clusters.end(),
    //        sortCompareElements);
    ////        for(int i = 0; i < compare_clusters.size(); i++)
    ////        {
    ////            ROS_INFO("identical: %d    cluster id: %d",
    ///compare_clusters.at(i).identical_ids, compare_clusters.at(i).cluster_id);
    ////        }
    ////        ROS_INFO("Selected identical: %d    cluster id: %d",
    ///compare_clusters.front().identical_ids,
    ///compare_clusters.front().cluster_id);
...
//{
//    /* Check if manual control or automatic. If automatic, set simulation to
//    false, otherwise set simulation to true in simple_navigation */
//
//    SimpleNavigation.Simulation = true;
//}

void ExplorationPlanner::frontierCallback(
    const adhoc_communication::ExpFrontier::ConstPtr &msg)
{
    ...
    for (int i = 0; i < msg.get()->frontier_element.size(); i++)
    {
        ...
        for (unsigned int j = 0; j < frontiers.size(); j++)
        {
            if (robot_prefix_empty_param == true)
            {
                ...
                if (frontiers.at(j).detected_by_robot_str.compare(
                        frontier_element.detected_by_robot_str) == 0 &&
                    frontier_element.id == frontiers.at(j).id)
                {
        ...
        if (result == true)
        {
            ...
            if (robot_prefix_empty_param == true)
            {
                ROS_DEBUG("Received New Frontier with ID: %" PRId64
                          "  Robot: %s",
                          frontier_element.id,
                          frontier_element.detected_by_robot_str.c_str());
                storeFrontier(frontier_element.x_coordinate,
                              frontier_element.y_coordinate,
                              frontier_element.detected_by_robot,
                              frontier_element.detected_by_robot_str,
                              frontier_element.id);
            }
            else
            {
                ROS_DEBUG("Received New Frontier of Robot %" PRId64
                          " with ID %" PRId64 "",
                          frontier_element.detected_by_robot,
                          frontier_element.id);
                if (frontier_element.detected_by_robot != robot_name)
...
void ExplorationPlanner::visited_frontierCallback(
    const adhoc_communication::ExpFrontier::ConstPtr &msg)
{
    adhoc_communication::ExpFrontierElement frontier_element;
    for (int i = 0; i < msg.get()->frontier_element.size(); i++)
    {
        ...
        if (result == true)
        {
            ROS_DEBUG("Received New Visited Frontier of Robot %" PRId64
                      " with ID %" PRId64 "",
                      frontier_element.detected_by_robot, frontier_element.id);
            if (robot_prefix_empty_param == true)
            {
                ROS_DEBUG("Storing Visited Frontier ID: %" PRId64 "  Robot: %s",
                          frontier_element.id,
                          frontier_element.detected_by_robot_str.c_str());
                storeVisitedFrontier(frontier_element.x_coordinate,
                                     frontier_element.y_coordinate,
                                     frontier_element.detected_by_robot,
                                     frontier_element.detected_by_robot_str,
                                     frontier_element.id);
...
bool ExplorationPlanner::publish_visited_frontier_list()
{
    ...
    for (int i = 0; i < visited_frontiers.size(); i++)
    {
        ...
        visited_frontier_element.detected_by_robot =
            visited_frontiers.at(i).detected_by_robot;
        visited_frontier_element.detected_by_robot_str =
            visited_frontiers.at(i).detected_by_robot_str;
        visited_frontier_element.robot_home_position_x =
            visited_frontiers.at(i).robot_home_x;
        visited_frontier_element.robot_home_position_y =
            visited_frontiers.at(i).robot_home_y;
        visited_frontier_element.x_coordinate =
            visited_frontiers.at(i).x_coordinate;
        visited_frontier_element.y_coordinate =
            visited_frontiers.at(i).y_coordinate;

        visited_frontier_msg.frontier_element.push_back(
            visited_frontier_element);
    }
...
bool ExplorationPlanner::check_efficiency_of_goal(double x, double y)
{
    ...
    if (fabs(diff_home_x) <= MAX_DISTANCE && fabs(diff_home_y) <= MAX_DISTANCE)
    {
        ...
        {
            /*
             * Calculate the distance between all previously seen goals and the
             * new
             * found frontier!!
             */
            ...
            if (fabs(diff_x) <= MAX_GOAL_RANGE &&
                fabs(diff_y) <= MAX_GOAL_RANGE)
            {
                ROS_DEBUG("x: %f  y: %f too close to visited at x: %f   y: %f  "
                          " dif_x: %f   dif_y: %f",
                          x, y, visited_frontiers.at(i).x_coordinate,
                          visited_frontiers.at(i).y_coordinate, diff_x, diff_y);
                return false;
            }
        }
        for (int i = 0; i < unreachable_frontiers.size(); i++)
        {
            ...
            if (fabs(diff_x) <= MAX_GOAL_RANGE &&
                fabs(diff_y) <= MAX_GOAL_RANGE)
            {
                ROS_DEBUG("x: %f  y: %f too close to unreachable at x: %f   y: "
                          "%f   dif_x: %f   dif_y: %f",
                          x, y, unreachable_frontiers.at(i).x_coordinate,
                          unreachable_frontiers.at(i).y_coordinate, diff_x,
                          diff_y);
                return false;
            }
        }
...
void ExplorationPlanner::clearVisitedAndSeenFrontiersFromClusters()
{
    ...
    for (int i = 0; i < clusters.size(); i++)
    {
        for (int j = 0; j < clusters.at(i).cluster_element.size(); j++)
        {
            ...
            {
                if (clusters.at(i).cluster_element.at(j).id ==
                    frontiers.at(m).id)
                {
            ...
            {
                clusters.at(i).cluster_element.erase(
                    clusters.at(i).cluster_element.begin() + j);
                if (j > 0)
                    j--;
            }
        }
    ...
    //            for (int j = 0; j < clusters.size(); j++)
    //            {
    //                for(int n = 0; n < clusters.at(j).cluster_element.size();
    //                n++)
    //                {
    //                    double diff_x = visited_frontiers.at(i).x_coordinate -
    //                    clusters.at(j).cluster_element.at(n).x_coordinate;
    //                    double diff_y = visited_frontiers.at(i).y_coordinate -
    //                    clusters.at(j).cluster_element.at(n).y_coordinate;
    //
    //                    if (fabs(diff_x) <= MAX_GOAL_RANGE && fabs(diff_y) <=
    //                    MAX_GOAL_RANGE)
    //                    {
    //                        ROS_DEBUG("Erasing visited frontier %d from
    //                        cluster",
    //                        clusters.at(j).cluster_element.at(n).id);
    ...
    //                n++)
    //                {
    //                    double diff_x = seen_frontier_list.at(i).x_coordinate
    //                    - clusters.at(j).cluster_element.at(n).x_coordinate;
    //                    double diff_y = seen_frontier_list.at(i).y_coordinate
    //                    - clusters.at(j).cluster_element.at(n).y_coordinate;
    //
    //                    if (fabs(diff_x) <= MAX_GOAL_RANGE*2 && fabs(diff_y)
    //                    <= MAX_GOAL_RANGE*2)
    //                    {
    //                        ROS_DEBUG("Erasing seen frontier %d from cluster",
    //                        clusters.at(j).cluster_element.at(n).id);
    //                        clusters.at(j).cluster_element.erase(clusters.at(j).cluster_element.begin()+n);
    //                        if(n > 0)
    //                        {
...
void ExplorationPlanner::clearVisitedFrontiers()
{
    ...
    for (int i = 1; i < visited_frontiers.size(); i++)
    {
        ...
        {
            double diff_x = visited_frontiers.at(i).x_coordinate -
                            frontiers.at(j).x_coordinate;
            double diff_y = visited_frontiers.at(i).y_coordinate -
                            frontiers.at(j).y_coordinate;
                ...
                fabs(diff_y) <= MAX_GOAL_RANGE)
            {
                if (robot_prefix_empty_param == true)
                {
                    removeStoredFrontier(frontiers.at(j).id,
                                         frontiers.at(j).detected_by_robot_str);
                    if (j > 0)
                    {
                        j--;
                    }
...
void ExplorationPlanner::clearUnreachableFrontiers()
{
    ...
    for (int i = 1; i < unreachable_frontiers.size(); i++)
    {
        for (int j = 0; j < frontiers.size(); j++)
        {
            double diff_x = unreachable_frontiers.at(i).x_coordinate -
                            frontiers.at(j).x_coordinate;
            double diff_y = unreachable_frontiers.at(i).y_coordinate -
                            frontiers.at(j).y_coordinate;
...
void ExplorationPlanner::clearSeenFrontiers(
    costmap_2d::Costmap2DROS *global_costmap)
{
    ...
    //    ROS_INFO("Map origin  x: %f    y: %f",
    //    global_costmap->getCostmap()->getOriginX(),
    //    global_costmap->getCostmap()->getOriginY());
    //    ROS_INFO("Map size    x: %d    y: %d",
    //    global_costmap->getCostmap()->getSizeInCellsX(),
    //    global_costmap->getCostmap()->getSizeInCellsY());
    if (frontiers.size() > 1)
    {
        for (int i = 0; i < frontiers.size(); i++)
        {
            ...
            // ROS_INFO("frontier x: %f   y: %f",
            // frontiers.at(i).x_coordinate,frontiers.at(i).y_coordinate);
            if (!global_costmap->getCostmap()->worldToMap(
                    frontiers.at(i).x_coordinate, frontiers.at(i).y_coordinate,
                    mx, my))
            {
            ...
            for (int j = 0; j < neighbours.size() / 2; j++)
            {
                ...
                //                ROS_INFO("Get Neighbour %d and %d",j*2,
                //                j*2+1);
                new_mx = neighbours.at(j * 2);
                new_my = neighbours.at(j * 2 + 1);
                //                ROS_INFO("got access");

                //                ROS_INFO("Calculating at position x: %d    y:
                //                %d", new_mx, new_my);
                unsigned char cost =
                    global_costmap->getCostmap()->getCost(new_mx, new_my);
                //                ROS_INFO("x position: %d       y position:
                //                %d", new_mx, new_my);
                //                ROS_INFO("Got Cost");
        ...
        //        for(int i= 0; i< goals_to_clear.size(); i++)
        //        {
        //    //        ROS_DEBUG("Frontier with ID: %d already seen and
        //    therefore deleted", goals_to_clear.at(i));
        //            removeStoredFrontier(goals_to_clear.at(i));
        //        }
    }
}
...
bool ExplorationPlanner::smartGoalBackoff(
    double x, double y, costmap_2d::Costmap2DROS *global_costmap,
    std::vector<double> *new_goal)
{
    ...
    for (int j = 0; j < neighbours.size() / 2; j++)
    {
        ...
        if (cost == costmap_2d::FREE_SPACE)
        {
            ...
            {
                global_costmap->getCostmap()->mapToWorld(new_mx, new_my, wx,
                                                         wy);
...
/*
 * searches the occupancy grid for frontier cells and merges them into one
 * target point per frontier.
...
void ExplorationPlanner::findFrontiers()
{
    ...
    for (unsigned int i = 0; i < num_map_cells_; i++)
    {
        ...
        {
            /*
             * If isFrontier() returns true, the point which is checked to be a
             * frontier
    ...
    ROS_INFO("Found %zu frontier cells which are transformed into frontiers "
             "points. Starting transformation...",
             allFrontiers.size());
     ...
     * To limit the amount of frontiers and only pick those which are valuable
     * to
     * drive there, check neighboring frontiers and only remember them if they
     * are
     ...
     * frontier has already been added to the list. If it is already in the
     * list, do
    ...
    for (unsigned int i = 0; i < allFrontiers.size(); ++i)
    {
        ...
        //                ROS_INFO("index: %d   map_x: %d   map_y: %d   world_x:
        //                %f   world_y: %f", allFrontiers.at(i), mx, my, wx,
        //                wy);
        ...
        if (select_frontier == 1)
        {
            ...
            for (unsigned int j = 0; j < frontiers.size(); j++)
            {
                if (fabs(wx - frontiers.at(j).x_coordinate) <= MAX_GOAL_RANGE &&
                    fabs(wy - frontiers.at(j).y_coordinate) <= MAX_GOAL_RANGE)
                {
                    result = false;
                    break;
                }
        ...
        else if (select_frontier == 2)
        {
            ...
            {
                costmap_ros_->getCostmap()->indexToCells(allFrontiers[j], mx2,
                                                         my2);
                costmap_ros_->getCostmap()->mapToWorld(mx2, my2, wx2, wy2);

                if (fabs(wx - wx2) <= MINIMAL_FRONTIER_RANGE &&
                    fabs(wy - wy2) <= MINIMAL_FRONTIER_RANGE &&
                    fabs(wx - wx2) != 0 && fabs(wy - wy2) != 0)
                {
            ...
            {
                costmap_ros_->getCostmap()->indexToCells(neighbour_index[n],
                                                         mx2, my2);
                costmap_ros_->getCostmap()->mapToWorld(mx2, my2, wx2, wy2);
                ...
                while (true)
                {
                    ...
                    {
                        costmap_ros_->getCostmap()->indexToCells(
                            allFrontiers[k], mx3, my3);
                        costmap_ros_->getCostmap()->mapToWorld(mx3, my3, wx3,
                                                               wy3);

                        if (fabs(wx2 - wx3) <= MINIMAL_FRONTIER_RANGE &&
                            fabs(wy2 - wy3) <= MINIMAL_FRONTIER_RANGE &&
                            wx2 != wx3 && wy2 != wy3 && wx != wx3 && wy != wy3)
                        {
                            wx = wx2;
                            wy = wy2;
                            wx2 = wx3;
            ...
            goal_buffer_x.push_back(start_points.at(0) +
                                    (start_points.at(2) - start_points.at(0)) /
                                        2);
            goal_buffer_y.push_back(start_points.at(1) +
                                    (start_points.at(3) - start_points.at(1)) /
                                        2);
        }
    }
    ...
    std::vector<std::string> *robot_str_name)
{
    ...
    {
        ROS_INFO("Waiting %f second because an auction is currently running",
                 wait_if_auction_runs);
        ros::Duration(wait_if_auction_runs).sleep();
        float wait_if_auction_runs = r * 2;
    ...
    for (int i = 0; i < clusters.size(); i++)
    {
        ...
        for (int j = 0; j < clusters.at(i).cluster_element.size(); j++)
        {
            ...
            if (robot_prefix_empty_param == true)
            {
                cluster_request_element.id =
                    clusters.at(i).cluster_element.at(j).id;
                cluster_request_element.detected_by_robot_str =
                    clusters.at(i).cluster_element.at(j).detected_by_robot_str;
            }
            else
            {
                cluster_request_element.id =
    ...
    //            requested_clusters.append(", ");
    //        }
    //        ROS_INFO("Auction with auction number: %d",
    //        auction_msg.auction_id);
...
*/
    ...
    //    nh_robots.param<std::string>("/robots_in_simulation",numbers_of_operating_robots,
    //    "1");
    //    number_of_robots = atoi(numbers_of_operating_robots.c_str());
    //    ROS_INFO("++++++++++ number of robots: %d ++++++++++",
    //    number_of_robots);
...
while(timer_count < 50 && number_of_auction_bids_received < number_of_robots -1)
{
    ...
    ROS_INFO("number of auction bids received: %d",
             number_of_auction_bids_received);
    ...
    bool cluster_selected_flag = selectClusterBasedOnAuction(
        final_goal, clusters_available_in_pool, robot_str_name);
    ...
    if (cluster_selected_flag == true) // && final_goal->size() >= 4)
    {
        ...
        for (int i = 0; i < occupied_ids.size(); i++)
        {
            ...
            auction_element.detected_by_robot_str =
                occupied_ids.at(i).robot_str;
...
bool ExplorationPlanner::clusterIdToElementIds(
    int cluster_id, std::vector<transform_point_t> *occupied_ids)
{
    for (int i = 0; i < clusters.size(); i++)
    {
...
bool ExplorationPlanner::selectClusterBasedOnAuction(
    std::vector<double> *goal, std::vector<int> *cluster_in_use_already_count,
    std::vector<std::string> *robot_str_name_to_return)
{
    ...
    for (int i = 0; i < clusters.size(); i++)
    {
        ...
        ROS_INFO("Own bid calculated for cluster '%d' is '%d'",
                 clusters.at(i).id, my_auction_bid);
    ...
    for (int j = 0; j < clusters.size(); j++)
    {
        for (int i = 0; i < already_used_ids.size(); i++)
        {
            if (already_used_ids.at(i) == clusters.at(j).id)
            {
                ...
                {
                    for (int n = 0; n < auction.at(m).auction_element.size();
                         n++)
                    {
                        if (auction.at(m).auction_element.at(n).cluster_id ==
                            already_used_ids.at(i))
                        {
                            ROS_INFO(
                                "Already used Cluster %d found .... unconsider "
                                "it",
                                auction.at(m).auction_element.at(n).cluster_id);
                            //                            auction.at(m).auction_element.erase(auction.at(m).auction_element.begin()+n);
                            //                            n--;
                            auction.at(m).auction_element.at(n).bid_value =
                                10000;
                        }
                    }
                }
    ...
    */
    ...
    for (int i = 0; i < row; i++)
    {
        ...
        for (int j = 0; j < col; j++)
        {
            ...
            for (int n = 0; n < auction.at(i).auction_element.size(); n++)
            {
                ...
                //                    if(j < clusters.size())
                //                    {
                //                        ROS_INFO("j: %d    smaller then
                //                        clusters.size(): %lu", j,
                //                        clusters.size());
                if (clusters.at(j).id ==
                        auction.at(i).auction_element.at(n).cluster_id &&
                    auction.at(i).auction_element.at(n).bid_value >= 0)
                {
                    //                    /*
                    //                     * Check if duplicates exist, if so
                    //                     change them
                    //                     */
                    ...
                    //                        {
                    //                            if(m(l,k) ==
                    //                            auction.at(i).auction_element.at(n).bid_value)
                    //                            {
                    //                                ROS_INFO("Duplicate
                    //                                found");
            ...
            if (found_a_bid == false) // && cluster_valid_flag == true)
            {
                ...
                ROS_INFO("---- clusters: %zu element size: %zu  robots "
                         "positions: %zu ----",
                         clusters.size(), clusters.at(j).cluster_element.size(),
                         other_robots_positions.positions.size());
                for (int d = 0; d < clusters.at(j).cluster_element.size(); d++)
                {
                    ROS_INFO("Access clusters.at(%d).cluster_element.at(%d)", j,
                             d);
                    position_mutex.lock();
                    /*Check position of the current robot (i)*/
                    for (int u = 0; u < other_robots_positions.positions.size();
                         u++)
                    {
                        adhoc_communication::MmPoint position_of_robot;
                        position_of_robot =
                            other_robots_positions.positions.at(u);

                        if (robot_prefix_empty_param == true)
                        {
                            ROS_INFO(
                                "position of robot: %s   compare with auction "
                                "robot: %s",
                                position_of_robot.src_robot.c_str(),
                                auction.at(i).detected_by_robot_str.c_str());
                            if (position_of_robot.src_robot.compare(
                                    auction.at(i).detected_by_robot_str) == 0)
                            {
                        ...
                        {
                            int robots_int_id =
                                atoi(position_of_robot.src_robot.substr(6, 1)
                                         .c_str());
                            ...
                            if (auction.at(i).robot_id == robots_int_id)
                            {
                                ...
                                ROS_INFO("Robots %d     position_x: %f     "
                                         "position_y: %f",
                                         robots_int_id, other_robots_position_x,
                    ...
                    if (other_robots_position_x != -1 &&
                        other_robots_position_y != -1)
                    {

                        if (trajectory_strategy == "trajectory")
                        {
                            distance = estimate_trajectory_plan(
                                other_robots_position_x,
                                other_robots_position_y,
                                clusters.at(j)
                                    .cluster_element.at(d)
                                    .x_coordinate,
                                clusters.at(j)
                                    .cluster_element.at(d)
                                    .y_coordinate);
                            ROS_INFO("estimated TRAJECTORY distance is: %d",
                                     distance);
                        }
                        else if (trajectory_strategy == "euclidean" &&
                                 j < clusters.size() &&
                                 d < clusters.at(j).cluster_element.size())
                        {
                            double x = clusters.at(j)
                                           .cluster_element.at(d)
                                           .x_coordinate -
                                       other_robots_position_x;
                            double y = clusters.at(j)
                                           .cluster_element.at(d)
                                           .y_coordinate -
                                       other_robots_position_y;
                            distance = x * x + y * y;
                            ROS_INFO("estimated EUCLIDEAN distance is: %d",
                                     distance);
                    ...
                    /*
                     * Check if the distance calculation is plausible.
                     * The euclidean distance to this point need to be smaller
                     * then
                     * the simulated trajectory path. If this stattement is not
                     * valid
                     * the trajectory calculation has failed.
                     */
                    if (distance != -1)
                    {
                        double x =
                            clusters.at(j).cluster_element.at(d).x_coordinate -
                            other_robots_position_x;
                        double y =
                            clusters.at(j).cluster_element.at(d).y_coordinate -
                            other_robots_position_y;
                        double euclidean_distance = x * x + y * y;

                        //                        ROS_INFO("Euclidean distance:
                        //                        %f   trajectory_path: %f",
                        //                        sqrt(euclidean_distance),
                        //                        distance *
                        //                        costmap_ros_->getCostmap()->getResolution());
                        if (distance *
                                costmap_ros_->getCostmap()->getResolution() <=
                            sqrt(euclidean_distance) * 0.95)
                        {
                            ROS_ERROR("Euclidean distance is smaller then the "
                                      "trajectory path at recalculation");
                            distance = -1;
                        }
                        else
                        {
                ...
                {
                    /*
                     * Cluster is definitely unknown. Therefore assign a high
                     * value
    ...
    if (col < row)
    {
        ROS_ERROR("Number of clusters is less the number of robots. Select the "
                  "closest");
        if (col == 0)
        {
    ...
    if (method_used == 0)
    {
         ...
         */
        //        int smallest_bid_value =
        //        auction.back().auction_element.front().bid_value;

        //        ROS_INFO("smallest_bid_value at initialization: %d",
        //        smallest_bid_value);
        ROS_INFO("Columns left: %d", col);
        //        for(int j = 0; j < col; j++)
        //        {
        //            ROS_INFO("col: %d", j);
        //            if(auction.back().auction_element.at(j).bid_value <=
        //            smallest_bid_value &&
        //            auction.back().auction_element.at(j).bid_value != -1)
        //            {
        //                smallest_bid_value =
        //                auction.back().auction_element.at(j).bid_value;
        //                auction_cluster_element_id =
        //                auction.back().auction_element.at(j).cluster_id;
        //            }
        //        }
        if (auction.size() > 0)
        {
            ...
            {
                auction_cluster_element_id =
                    auction.back().auction_element.front().cluster_id;
            }
            else
            {
                return false;
    ...
    {
        /*
         * Use the Ungarische method/ KuhnMunkresAlgorithm in order to figure
         * out which
        ...
        for (int i = 0; i < assignment.size(); i++)
        {
            if (assignment.at(i).at(own_row_to_select_cluster) == 1)
            {
                ...
                ROS_INFO("Selected Cluster at position : %d   %d", i,
                         own_row_to_select_cluster);
                break;
            }
        }
    }
    if (method_used == 2)
    {
        ...
        //                    {
        //                        if(abs(mat(i,j) - mat(n,m)) <= 20 &&
        //                        abs(mat(i,j) - mat(n,m)) != 0 && (mat(i,j) !=
        //                        0 && mat(n,m) != 0))
        //                        {
        //                            mat(i,j) = 0;
        ////                            element_found = true;
        ...
        for (int i = 0; i < mat.columns(); i++)
        {
            if (mat(i, own_row_to_select_cluster) == 1)
            {
                ...
                ROS_INFO("Selected Cluster at position : %d   %d  with BID: %f",
                         i, own_row_to_select_cluster,
                         mat(i, own_row_to_select_cluster));
                break;
            }
        }
    }
    ...
    /*
     * Try to navigate to the selected cluster. If it failes, take the next
     * efficient
    ...
    if (auction_cluster_element_id != -1)
    {
        ...
        while (ros::ok())
        {
            ...
            bool goal_determined =
                determine_goal(6, &new_goal, count, auction_cluster_element_id,
                               &robot_str_name);
            ...
            if (goal_determined == true)
            {
                //                if(new_goal.front() == -1)
                //                {
                //                    ROS_INFO("Unable to access goal points in
                //                    the cluster");
                //                    count++;
                //                }else
                //                {
            ...
            else
            {
                ...
                {
                    ROS_INFO("Not possible to select any goal from the "
                             "available clusters");
                    return false;
                }
                else
    ...
    else
    {
        ...
        ROS_INFO("Auction has failed, no auction elements are existent. "
                 "Choosing nearest cluster");
        std::vector<double> new_goal;
        std::vector<std::string> robot_str_name;
        bool goal_determined =
            determine_goal(4, &new_goal, count, -1, &robot_str_name);
        if (goal_determined == true)
        {
            goal->push_back(new_goal.at(0));
            goal->push_back(new_goal.at(1));
...
bool ExplorationPlanner::negotiate_Frontier(double x, double y, int detected_by,
                                            int id, int cluster_id_number)
{

    ROS_INFO("Negotiating Frontier with id: %d  at Cluster: %d", id,
             cluster_id_number);
    ...
    {
        for (int k = 0;
             k < clusters.at(cluster_vector_position).cluster_element.size();
             k++)
        {
            id_in_actual_cluster = false;
            if (negotiation_list.at(i).id ==
                clusters.at(cluster_vector_position).cluster_element.at(k).id)
            {
                ROS_INFO("         Same ID detected");
                for (int j = 0; j < clusters.at(cluster_vector_position)
                                        .cluster_element.size();
                     j++)
                {
                    for (int m = 0; m < my_negotiation_list.size(); m++)
                    {
                        if (clusters.at(cluster_vector_position)
                                .cluster_element.at(j)
                                .id == my_negotiation_list.at(m).id)
                        {
                            ROS_INFO("         But is in the current working "
                                     "cluster! everything alright");
                ...
                {
                    ROS_INFO("Checking how many goals in the cluster are "
                             "already occupied");
                    for (int n = 0; n < negotiation_list.size(); n++)
                    {
                        for (int m = 0; m < clusters.at(cluster_vector_position)
                                                .cluster_element.size();
                             m++)
                        {
                            if (negotiation_list.at(n).id ==
                                clusters.at(cluster_vector_position)
                                    .cluster_element.at(m)
                                    .id)
                            {
                    ...
                    //                    ROS_INFO("%f goals of %f in the
                    //                    cluster  -> %f",used_cluster_ids,
                    //                    (double)clusters.at(cluster_vector_position).cluster_element.size(),
                    //                    double(used_cluster_ids /
                    //                    (double)clusters.at(cluster_vector_position).cluster_element.size()));
                    if (double(used_cluster_ids /
                               (double)clusters.at(cluster_vector_position)
                                   .cluster_element.size()) >= 0.1)
                    {
                        ROS_INFO("Negotiation failed the other Robot got "
                                 "cluster %d already",
                                 cluster_id_number);
                        entry_found = true;
                        return false;
...
bool ExplorationPlanner::determine_goal(
    int strategy, std::vector<double> *final_goal, int count,
    int actual_cluster_id, std::vector<std::string> *robot_str_name)
{
    if (!costmap_ros_->getRobotPose(robotPose))
    {
        ROS_ERROR("Failed to get RobotPose");
    ...
    if (strategy == 1)
    {
        ...
        {
            if (check_efficiency_of_goal(frontiers.at(i).x_coordinate,
                                         frontiers.at(i).y_coordinate) == true)
            {
                ROS_INFO("-----------------------------------------------------"
                         "-------------");
                ROS_INFO("Determined frontier with ID: %d   at x: %f     y: %f "
                         "  detected by Robot %d",
                         frontiers.at(i).id, frontiers.at(i).x_coordinate,
                         frontiers.at(i).y_coordinate,
                ...
                /*
                 * Solve the Problem of planning a path to a frontier which is
                 * located
                 * directly "in" the obstacle. Therefore back-off 5% of the
                 * targets
                 * coordinate. Since the direction of x and y can be positive
                 * and
                 * negative either, multiply the coordinate with 0.95 to get 95%
                 * of its
                 * original value.
                 */
                final_goal->push_back(
                    frontiers.at(i)
                        .x_coordinate); // final_goal.push_back(frontiers.at(i).x_coordinate*0.95
                                        // - robotPose.getOrigin().getX());
        ...
        while (true)
        {
            if (frontiers.size() > 0)
            {
                    ...
                    true)
                {
                    ROS_INFO("-------------------------------------------------"
                             "-----------------");
                    ROS_INFO("Determined frontier with ID: %d   at x: %f     "
                             "y: %f   detected by Robot %d",
                             frontiers.at(i).id, frontiers.at(i).x_coordinate,
                             frontiers.at(i).y_coordinate,
                             frontiers.at(i).detected_by_robot);
    ...
    else if (strategy == 4)
    {
         ...
         * determination fails at clusters.at(cluster_vector_position).
         * therefore
        ...
        {
            //                    ROS_INFO("Cluster vector: %d  i: %d ",
            //                    cluster_vector_position, i);
            i = i + cluster_vector_position;
            i = i % (clusters.size());
            for (int j = 0; j < clusters.at(i).cluster_element.size(); j++)
            {
                if (check_efficiency_of_goal(
                        clusters.at(i).cluster_element.at(j).x_coordinate,
                        clusters.at(i).cluster_element.at(j).y_coordinate) ==
                    true)
                {
                    ROS_INFO("-------------------------------------------------"
                             "-----------------");
                    ROS_INFO("Robot %d determined Goal: %d  at Clusters: %d",
                             robot_name,
                             (int)clusters.at(i).cluster_element.at(j).id,
                             (int)clusters.at(i).id);
                    ...
                    final_goal->push_back(
                        clusters.at(i).cluster_element.at(j).y_coordinate);
                    final_goal->push_back(
                        clusters.at(i).cluster_element.at(j).detected_by_robot);
                    final_goal->push_back(
                        clusters.at(i).cluster_element.at(j).id);

                    // number of the cluster we operate in
                    final_goal->push_back(clusters.at(i).id);
                    robot_str_name->push_back(clusters.at(i)
                                                  .cluster_element.at(j)
                                                  .detected_by_robot_str);
                    return true;
                }
    ...
    else if (strategy == 5)
    {
        ...
        ROS_INFO("Calculated vector position: %d of cluster %d",
                 cluster_vector_position, actual_cluster_id);
        /*
         * Iterate over all clusters .... if cluster_vector_position is set
         * also the clusters with a lower have to be checked if the frontier
         * determination fails at clusters.at(cluster_vector_position).
        ...
        for (int j = 0; j < clusters.at(position).cluster_element.size(); j++)
        {
            ...
            {
                if (check_efficiency_of_goal(clusters.at(position)
                                                 .cluster_element.at(j)
                                                 .x_coordinate,
                                             clusters.at(position)
                                                 .cluster_element.at(j)
                                                 .y_coordinate) == true)
                {
                    ROS_INFO("-------------------------------------------------"
                             "-----------------");
                    ROS_INFO(
                        "Robot %d determined Goal: %d  at Clusters: %d",
                        robot_name,
                        (int)clusters.at(position).cluster_element.at(j).id,
                        (int)clusters.at(position).id);

                    final_goal->push_back(clusters.at(position)
                                              .cluster_element.at(j)
                                              .x_coordinate);
                    final_goal->push_back(clusters.at(position)
                                              .cluster_element.at(j)
                    ...
                    final_goal->push_back(clusters.at(position)
                                              ...
                                              .detected_by_robot);
                    final_goal->push_back(
                        clusters.at(position).cluster_element.at(j).id);

                    // number of the cluster we operate in
                    final_goal->push_back(clusters.at(position).id);
                    robot_str_name->push_back(clusters.at(position)
                                                  .cluster_element.at(j)
                                                  .detected_by_robot_str);
                    return true;
                }
    ...
    else if (strategy == 6)
    {
        ...
        //                    ROS_INFO("position: %lu", (cluster_vector_position
        //                    +count) % (clusters.size()));
        int position = (cluster_vector_position + count) % (clusters.size());
        for (int j = 0; j < clusters.at(position).cluster_element.size(); j++)
        {
                 ...
                 const ExplorationPlanner::cluster_t &rhs)
{
    ...
    {
        return lhs.cluster_element.front().dist_to_robot <
               rhs.cluster_element.front().dist_to_robot;
    }
    return false;
}
...
void ExplorationPlanner::sort(int strategy)
{
    ...
    if (strategy == 1)
    {
        ...
        if (frontiers.size() > 0)
        {
            for (int i = frontiers.size(); i >= 0; i--)
            {
                ...
                {
                    double x = frontiers.at(j).x_coordinate -
                               robotPose.getOrigin().getX();
                    double y = frontiers.at(j).y_coordinate -
                               robotPose.getOrigin().getY();
                    double x_next = frontiers.at(j + 1).x_coordinate -
                                    robotPose.getOrigin().getX();
                    double y_next = frontiers.at(j + 1).y_coordinate -
                                    robotPose.getOrigin().getY();
                    double euclidean_distance = x * x + y * y;
                    double euclidean_distance_next =
                        x_next * x_next + y_next * y_next;

                    if (sqrt(euclidean_distance) >
                        sqrt(euclidean_distance_next))
                    {
                ...
                else
                {
                    for (int j = 0; j < 10 - 1; j++)
                    {
                        ...
                        else
                        {
                            ...
                            int dist_next =
                                frontiers.at(j + 1).distance_to_robot;
    ...
    else if (strategy == 4)
    {
        ...
        if (clusters.size() > 0)
        {
            for (int cluster_number = 0; cluster_number < clusters.size();
                 cluster_number++)
            {
                if (clusters.at(cluster_number).cluster_element.size() > 0)
                {
                    ROS_DEBUG(
                        "Cluster %d  size: %zu", cluster_number,
                        clusters.at(cluster_number).cluster_element.size());
                    for (int i =
                             clusters.at(cluster_number).cluster_element.size();
                         i > 0; i--)
                    {
                        ROS_DEBUG("Cluster element size: %d", i);
                        for (int j = 0; j < clusters.at(cluster_number)
                                                    .cluster_element.size() -
                                                1;
                             j++)
                        {
                            ROS_DEBUG("Cluster element number: %d", j);
                            double x = clusters.at(cluster_number)
                                           .cluster_element.at(j)
                                           .x_coordinate -
                                       pose_x;
                            double y = clusters.at(cluster_number)
                                           .cluster_element.at(j)
                                           .y_coordinate -
                                       pose_y;
                            double x_next = clusters.at(cluster_number)
                                                .cluster_element.at(j + 1)
                                                .x_coordinate -
                                            pose_x;
                            double y_next = clusters.at(cluster_number)
                                                .cluster_element.at(j + 1)
                                                .y_coordinate -
                                            pose_y;
                            double euclidean_distance = x * x + y * y;
                            ...
                            //                                        int
                            //                                        distance =
                            //                                        check_trajectory_plan(clusters.at(cluster_number).cluster_element.at(j).x_coordinate,
                            //                                        clusters.at(cluster_number).cluster_element.at(j).y_coordinate);
                            //                                        int
                            //                                        distance_next
                            //                                        =
                            //                                        check_trajectory_plan(clusters.at(cluster_number).cluster_element.at(j+1).x_coordinate,clusters.at(cluster_number).cluster_element.at(j+1).y_coordinate);
    ...
    else if (strategy == 5)
    {
        ...
        for (int i = 0; i < clusters.size(); i++)
        {
            if (clusters.at(i).cluster_element.size() > 0)
            {
                for (int j = 0; j < clusters.at(i).cluster_element.size(); j++)
                {
                    ...
                    clusters.at(i).cluster_element.at(j).dist_to_robot =
                        sqrt(distance);

                    /* ********** TRAVEL PATH ********** */
                    //                        int distance =
                    //                        check_trajectory_plan(clusters.at(i).cluster_element.at(j).x_coordinate,
                    //                        clusters.at(i).cluster_element.at(j).y_coordinate);
        ...
        //                    {
        //                            for (int j = 0; j < clusters.size() - 1;
        //                            j++)
        //                            {
        ////                                ROS_ERROR("Compare cluster: %d id:
        ///%d    ||    cluster: %d id: %d", j,
        ///clusters.at(j).cluster_element.front().id, j+1,
        ///clusters.at(j+1).cluster_element.front().id);
        //                                if(clusters.at(j).cluster_element.size()
        //                                > 0 &&
        //                                clusters.at(j+1).cluster_element.size()
        //                                > 0)
        //                                {
        //
        //                                    double x =
        //                                    clusters.at(j).cluster_element.front().x_coordinate
        //                                    - pose_x;
        //                                    double y =
        //                                    clusters.at(j).cluster_element.front().y_coordinate
        //                                    - pose_y;
        //                                    double x_next =
        //                                    clusters.at(j+1).cluster_element.front().x_coordinate
        //                                    - pose_x;
        //                                    double y_next =
        //                                    clusters.at(j+1).cluster_element.front().y_coordinate
        //                                    - pose_y;
        //
        //                                    double euclidean_distance = x * x
        //                                    + y * y;
        //                                    double euclidean_distance_next =
        //                                    x_next * x_next + y_next * y_next;
        //
        ////                                  ROS_INFO("Cluster %d dist: %d
        ///Cluster %d dist: %d",j,(int)euclidean_distance, j+1,
        ///(int)euclidean_distance_next);
        //                                    clusters.at(j).cluster_element.front().dist_to_robot
        //                                    = euclidean_distance;
        //                                    clusters.at(j+1).cluster_element.front().dist_to_robot
        //                                    = euclidean_distance_next;
        //
        ////                                    int distance =
        ///check_trajectory_plan(clusters.at(j).cluster_element.front().x_coordinate,
        ///clusters.at(j).cluster_element.front().y_coordinate);
        ////                                    int distance_next =
        ///check_trajectory_plan(clusters.at(j+1).cluster_element.front().x_coordinate,clusters.at(j+1).cluster_element.front().y_coordinate);
        //
        ...
        //            }else
        //            {
        //                    ROS_INFO("Sorting not possible, no frontiers
        //                    available!!!");
        //            }
    }
    else if (strategy == 6)
    ...
    //		ROS_ERROR("Frontier with ID: %d   at x: %f     y: %f   detected by
    //Robot %d", frontiers.at(i).id, frontiers.at(i).x_coordinate,
    //frontiers.at(i).y_coordinate, frontiers.at(i).detected_by_robot);
    //	}
}
...
void ExplorationPlanner::visualize_visited_Frontiers()
{
    ...
    pub_visited_frontiers_points.publish<visualization_msgs::MarkerArray>(
        markerArray);
}
...
void ExplorationPlanner::setupMapData()
{
        ...
        (this->map_height_ != costmap_ros_->getCostmap()->getSizeInCellsY()))
    {
        ...
        ROS_INFO("Costmap size in cells: width:%d   height: %d   map_cells:%d",
                 map_width_, map_height_, num_map_cells_);

        // initialize exploration_trans_array_, obstacle_trans_array_, goalMap
        // and frontier_map_array_
    ...
    for (unsigned int i = 0; i < num_map_cells_; i++)
    {
        ...
        //    occupancy_grid_array_[i] ==
        //    costmap_2d::INSCRIBED_INFLATED_OBSTACLE ||
        //    occupancy_grid_array_[i] == costmap_2d::LETHAL_OBSTACLE ||
        //    occupancy_grid_array_[i] == costmap_2d::FREE_SPACE)
        //    {
        //    }else
        //    {
...
int ExplorationPlanner::isFrontier(int point)
{
    ...
    if (isFree(point))
    {
         ...
         */
        // int Neighbours = 0;
        // int points[((int)pow(8,Neighbours+1))+8]; // NEIGHBOURS points, each
        // containing 8 adjacent points
        int no_inf_count = 0;
        int inscribed_count = 0;
        int adjacent_points[16];
        /*
         * Now take one point and lookup all adjacent points (surrounding
         * points).
         * The variable adjacentPoints contains all neighboring point (up,
         * right, left ...)
         */
        ...
        if ((int)occupancy_grid_array_[point] ==
            costmap_2d::FREE_SPACE) //<= threshold_free)
        {
            getAdjacentPoints(point, adjacent_points);
            for (int i = 0; i < 16; i++) // length of adjacent_points array
            {

                if (occupancy_grid_array_[adjacent_points[i]] ==
                    costmap_2d::NO_INFORMATION)
                {
                    no_inf_count++;
                    //                                        ROS_DEBUG("No
                ...
                else if (occupancy_grid_array_[adjacent_points[i]] ==
                         costmap_2d::LETHAL_OBSTACLE)
                {
                    /*
                     * Do not break here ... In some scenarios it may happen
                     * that unknown and free space
                     * form a border, but if just a small corridor is available
                     * there, it would not be
                     * detected as frontier, since even one neighboring block is
                     * an inflated block.
                     * Therefore do nothing, if even one unknown block is a
                     * neighbor of an free space
             ...
             * Otherwise, when all surronding blocks --> 64+7 are unknown just
             * the
            ...
            {
                /*
                 * Above a adjacent Point is taken and compared with
                 * NO_INFORMATION.
                 * If the point contains no information, the next surrounding
                 * point from that point
                 * is taken and again compared with NO_INFORMATION. If there are
                 * at least
                 * two points with no information surrounding a no information
                 * point, then return true!
                 * (at least two points with no information means that there is
                 * sufficient space
...
int ExplorationPlanner::backoff(int point)
{
    if (occupancy_grid_array_[up(point)] == costmap_2d::NO_INFORMATION &&
        occupancy_grid_array_[left(point)] == costmap_2d::NO_INFORMATION)
    {
        return (downright(downright(downright(point))));
    }
    if (occupancy_grid_array_[up(point)] == costmap_2d::NO_INFORMATION &&
        occupancy_grid_array_[right(point)] == costmap_2d::NO_INFORMATION)
    {
    ...
    //				if(occupancy_grid_array_[upleft(point)] ==
    //costmap_2d::NO_INFORMATION)
    //				{
    //					return(downright(downright(point)));
    //				}
    //				if(occupancy_grid_array_[upright(point)] ==
    //costmap_2d::NO_INFORMATION)
    //				{
    //					return(downleft(downleft(point)));
    //				}
    //				if(occupancy_grid_array_[downright(point)] ==
    //costmap_2d::NO_INFORMATION)
    //				{
    //					return(upleft(upleft(point)));
    //				}
    //				if(occupancy_grid_array_[downleft(point)] ==
    //costmap_2d::NO_INFORMATION)
    //				{
...
bool ExplorationPlanner::isFrontierReached(int point)
{
    ...
    {
        // ROS_DEBUG("[isFrontierReached]: frontier is within the squared range
        // of distance: %f m", p_dist_for_goal_reached_);
        return true;
    }
...
bool ExplorationPlanner::countCostMapBlocks(int point)
{
    ...
    else if ((int)occupancy_grid_array_[point] ==
             costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
    {
        inflated++;
...
inline bool ExplorationPlanner::isValid(int point) { return (point >= 0); }
...
{
    // only go left if no index error and if current point is not already on the
    // left boundary
